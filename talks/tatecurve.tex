...
\end{block}
The third case of this (much less easy to solve) are cubic Diophantine equations, which are elliptic curves.
}
 \frame{
\frametitle{Curves}
...
An elliptic curve is a compact Riemann Surface (compact one-dimensional manifold over $\mathbb C$) of genus 1 along with a choice of a distinguished point $P \in X$.
\end{definition}
...
If we specify that $\rm{Im} \left( \frac{\lambda_2}{\lambda_1} \right) > 0$ the we have a {\em framed lattice}.
}
...
\begin{theorem}
The set of isomorphism classes is the quotient 
\[
...
\frame{
\frametitle{Weierstrass ${\cal P}$-function}
...
\begin{definition}
The Weierstrass ${\cal P}$-function (relative to $\Lambda$) is defined as
\[ 
{\cal P}(z) = \frac{1}{z^2} + \sum_{\omega \in \Lambda \backslash \{ 0 \}} \left( \frac{1}{(z-\omega)^2}  - \frac{1}{\omega^2} \right)
\]
\end{definition}

\begin{theorem}
The Weierstrass $\cal P$-function converges everywhere on $\mathbb C / \Lambda$ and has an order two pole at each lattice point.
\end{theorem}
}
...
\[
f(z) = {\cal P}'(z) -4{\cal P}(z)^3 + 60 G_4 {\cal P}(z) + 160G_6 \qquad \left( G_{2k} = \sum_{\omega \in \Lambda \backslash \{ 0 \}} \frac{1}{\omega^2}\right)
\]
is constant.
By Liouville's theorem we hence deduce that $f(z)$ is in fact zero, and hence this is a defining function for a variety.
...
Then the map 
\[
\phi: \mathbb C / \Lambda \rightarrow E \qquad \phi \colon z \mapsto [{\cal P}(z): {\cal P}'(z) :1]
\]
