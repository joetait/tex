...
\section{Function fields}\label{Hurwitzsection}
...
Any element $\alpha \in L$ defines a $K$~-~linear map $\mu_{\alpha} \colon  L \rightarrow K$, given by multiplication by $\alpha$.
...
Note that $\tr_{L/K}$ is an additive map, and that for $\alpha \in K$ we have $\tr_{L/K}(\alpha) = n \alpha$.
    \begin{prop}
    The trace map $\tr_{X/Y} \colon K(X) \ra K(Y)$ is non-zero.
    \end{prop}
    \begin{proof}
    If the characteristic of $k$, and hence $K(Y)$, is zero, then $\tr_{X/Y}(1) = n \neq 0$.
    Hence we assume that $\cha(k) = p \neq 0$.
    ...
    We let $h_1, \ldots, h_n$ be a $K(Y)$ basis of $K(X)$, and write $fh_j = \sum_{i=1}^n c_{ij}h_i$ for $c_{ij} \in K(Y)$.
    ...
    Now it follows from the previous paragraph that $\tr_{X/Y}$ is separable if and only if \todo{the next line follows from the previous paragraph, but why does this mean it is separable?}
        \[
        \overline{\tr} = \operatorname{id}_{\overline {K(Y)}} \otimes \tr_{X/Y} \colon \overline{K(Y)}\otimes_{K(Y)} K(X) \ra \overline{K(Y)}\otimes_{K(Y)}K(Y).
        \]
    Since
        \[
    ...
    Now $\overline{\tr}$ is the sum of the trace map on each component.
    But the trace map $\tr_{\overline{K}/\overline{K}}$ is the identity.\todo{define trace for general fields}
    Hence $\overline{\tr}$ is surjective, and so $\tr_{X/Y}$ is too.
    \end{proof}
...
Finally, if $\alpha \in K(X)$ has minimum polynomial 
    ...
    \]
and $s= [K(X):K(Y)(\alpha)]$, then $\tr_{X/Y}(\alpha) = -sa_{r-1}$ as in \cite[Appendix A]{stichtenoth}.\todo{more specific citation}
...
As $\tr_{X/Y}$ is non-zero, then there exists a unique $z\in K(X)$ for every $\lambda \in K(X)^*$ such that $\lambda = z\cdot \tr_{X/Y}$.
...
for any non-zero $\omega \in \Omega_{K(C)}$, and state the following lemma.
    ...
    \begin{defnlem}
    For any non-zero differential $\omega$ on $C$ there is a unique maximal divisor $W$ in $M(\omega)$, which we call the \text{ canonical divisor associated to $\omega$} (we associate the zero divisor to $0\in \Omega_{K(C)}$).
    We denote the divisor associated to $\omega$ by $\di (\omega)$.
    We call a divisor a \text{ canonical divisor} if it is the canonical divisor associated to some $\omega \in \Omega_{K(C)}$.
...
This can then be extended from a single point to a divisor on $Y$, in which case for a divisor $D = \sum_{Q\in Y}n_Q [Q]$ we have
    ...
    \begin{rem}
    It should be noted that in the literature regarding function field theory, what we have denoted by $\pi^*(D)$ is normally called the conorm of $D$ and is denoted $\text{ Con}_{X/Y}(D)$. 
    ...
    We then define the complementary module over $\cO_{X,Q}$ to be
        \[
        C_Q :=\{z\in K(X) | \tr_{X/Y}(z\cdot \cO_{X,Q}') \subseteq \cO_{X,Q}\}.
        \]\todo{is it okay to say $\tr_{X/Y}(z\cdot \cO_{X,Q}')$?}
    \end{defn}
