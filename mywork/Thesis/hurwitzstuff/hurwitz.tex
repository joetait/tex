...
% Stichtenoth  Lemma 3.4.9
    ...
    Then $\psi := \omega\circ \tr_{X/Y}(\alpha) = 0$ for any $\alpha \in \mathcal{A}_{X/Y}(W') + K(X)$.
    \end{lem}
    \begin{proof}
    We start by noting that since $\tr_{X/Y}$ and $\omega$ are $k$-linear, $\psi$ is too.
    Also, note that since $\omega$ is zero on $K(Y)$ then $\psi$ is zero on $K(X)$.
    To show that $\psi(\alpha) = 0$ for any $\alpha \in \mathcal{A}_{X/Y}(W)$ then it is sufficient to show that for any $Q\in Y$ and $P\in \pi^{-1}(Q)$ that
        \[
    ...
    We choose $W$ and $\psi$ as in Lemma \ref{lemmapart1}.
    If $D$ is a divisor on $X$ with $D \nleq W$ then there exists a $\beta \in \cA_{X/Y}(D)$ such that $\psi(\beta) \neq 0$.
    \end{lem}
    \begin{proof}
    We let $Q_0\in Y$ be a point such that there is a $P^*\in \pi^{-1}(Q_0)$ with $v_{P^*}(\pi^*((\omega)) - D) < -\delta_P$.
    We know that such a $Q_0$ exists since $D \nleq W$.
    As before we will denote by $\cO_{Q_0}'$ the integral closure of $\cO_{Q_0}$ in $K(X)$.
    We let 
        \[
        J := \{ z\in K(X) | v_{P^*}(z) \geq v_{P^*}(\pi^*(\di (\omega)) - D)\ \text{for all}\ P^*\in \pi^{-1}(Q_0)\}.
        \]
...
We now prove Theorem \ref{theoremdetailedhurwitz}.
    ...
    We let $W' := \pi^*(\di (\omega)) + R$ $\omega_1:= \omega\circ \tr_{X/Y} \colon \mathcal{A}_{X/Y} \rightarrow k$.
