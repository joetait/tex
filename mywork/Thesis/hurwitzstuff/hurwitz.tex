...
Given a ring $K(C)$ and an $K(C)$-module $M$, we call any $K(C)$-linear map $D\colon K(C)\rightarrow M$ satisfying \[D(ab) = aD(b) + D(a)b\] for all $a,b\in K(C)$ a {\em derivation}.
There is a unique module, denoted $\Omega_{K(C)}$, with a map $d\colon K(C) \rightarrow \Omega_{K(C)}$, which every derivation must factor through; \ie if $D\colon K(C)\rightarrow M$ is a derivation then there is a unique map $f\colon \Omega_{K(C)}\rightarrow M$ such that $D = f\circ d$.
...
A differential $\omega \in \Omega_{K(C)}$ is called holomorphic if its associated divisor, $W$, is non-negative (\ie if $v_P(W) \geq 0$ for all $P\in C$).
...
\end{equation}
for $Q\in C$.
For $\omega \in \Omega_{K(C)}$ we then define $\omega_P\colon K(C) \rightarrow k$ to be the map $\omega_P(x) := \omega(\iota_P(x))$.
