...
Then an {\em \adele on $X$} is a map $\alpha\colon  X \rightarrow K(X)$ such $\alpha(P) \in \cO_{X,P}$ for nearly all $P \in X$ (\ie all but a finite number of points).
...
For any $D\in \cD_X$ we define $v_P(D)$ to be the coefficient of $[P]$ in $D$, and we let
    \[
...
for any non-zero $\omega \in \Omega_{K(X)}$, and state the following lemma.
    ...
    \begin{defnlem}
    For any non-zero differential $\omega$ on $X$ there is a unique maximal divisor $W$ in $M(\omega)$, which we call the \text{ canonical divisor associated to $\omega$} (we associate the zero divisor to $0\in \Omega_{K(X)}$).
    We denote the divisor associated to $\omega$ by $\di (\omega)$.
    We call a divisor a \text{ canonical divisor} if it is the canonical divisor associated to some $\omega \in \Omega_{K(X)}$.
