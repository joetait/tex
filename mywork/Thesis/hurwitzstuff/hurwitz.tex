\chapter{The Hurwitz formula} \label{Chapter:hurwitzformula}
...
We call $\Tr_{X/Y}(\alpha)$ the trace of $\alpha$, and $\Tr_{X/Y}$ is a map $K(X) \ra K(Y)$.\todo{explain last statement}
...
\]
 over $K(Y)$ and $s= [K(X):K(Y)(\alpha)]$, then $\Tr_{X/Y}(\alpha) = -sa_{r-1}$ (see, again, \cite[Appendix A]{stichtenoth}).\todo{clean up this paragraph. Give more explanation}

Fix a basis $z_1,\ldots,z_n$ of $K(X)$ over $K(Y)$.
We now introduce its dual basis with respect to the trace.\todo{clarify this sentence}
...
An {\em \adele on $X$} is a map $\alpha\colon  X \rightarrow K(X)$ such $\alpha(P) \in \cO_{X,P}$ for nearly all $P \in X$ (\ie for all but a finite number of points).
We can view $\alpha$ as an element of $\prod_{P\in X}K(X)$, and we write $\alpha_P := \alpha(P)$.
...
The elements of $K(X)$ have natural valuations; for each $P\in X$ we choose a uniformising parameter $t\in \cO_{X,P}$, and we then define $v_P(x):=n$, where $n$ is the unique integer such that $x=ut^n$ for some unit $u\in \cO_{X,P}$.
We can we can then extend this valuation to elements of $\cA_X$ by defining $v_P(\alpha) := v_P(\alpha_P)$.
\todo[inline]{consider changing \adele notation to $(\alpha_P)_{P \in X}$}

Recall that a divisor on $X$ is a finitely supported formal sum over $X$, with coefficients in $\mathbb Z$, which we write as 
\[
 D = \sum_{P\in X} n_P [P],
\]
and that the degree of such a divisor is 
\[
 \deg(D) := \sum_{P\in X} n_P.
\]
We define the order of $D$ at $P$ to be $v_P(D) := n_P$.

We let $\cD_X$ be the space of divisors on $X$.
For any $x\in K(X)$ we define the divisor of $x$ to be
\[
 \di(x) := \sum_{P\in X} v_P(x) [P].
\]

For any $D\in \cD_X$ we let
\[
 \cA_X(D):=\{\alpha \in \cA_X | v_P(\alpha) \geq -v_P(D)\ \text{for all} \ P\in X\}
\]
be the {\em ad\`ele space associated to $D$}.\todo[inline]{Make some of these proper definitions}
We now define a {\em differential} on $X$ to be a $k$-linear map $\omega\colon  \cA_X \rightarrow k$ such that $\omega$ is zero on $\cA_X(D) +K(X)$ for some divisor $D\in \cD_X$.
Note that via the canonical embedding of $K(X)$ into $\cA_X$, we can view $\cA_X(D) +K(X)$ as a subspace of $\cA_X$.
We will denote the space of differentials on $X$ by $\Omega_{K(X)}$.

Now we define 
\[
 M(\omega) := \{ D\in \cD_X |\ \omega \ \text{is zero on }\ \cA_X(D) + K(X)\}
\]
for any non-zero $\omega \in \Omega_{K(X)}$, and state the following lemma.

\begin{defnlem}
 For any non-zero differential $\omega$ on $X$ there is a unique maximal divisor $W$ in $M(\omega)$, which we call the {\rm canonical divisor associated to $\omega$} (we associate the zero divisor to $0\in \Omega_{K(X)}$).
 We denote the divisor associated to $\omega$ by $\di (\omega)$.
 We call a divisor a {\rm canonical divisor} if it is the canonical divisor associated to some $\omega \in \Omega_{K(X)}$.\\
...
\todo[inline]{Rephrase the above paragraph}
...
\todo[inline]{Sort out clash of notations re $\Omega_X$ and $\Omega_{K(X)}$. Rephrase parts. Consider using something other than ``D'' for differential}
...
Also, for any divisor $D$ on $X$ we define \[H^0(X,\cO(D)) := \{x\in K(X) | v_P(x) \geq -v_P(D)\}\cup \{0\}.\]
...
\begin{proof}
 ...
 Hence if $\phi (T)$ has co-efficients in $\cO_{X,Q}$, then $z$ is integral over $\cO_{X,Q}$ by definition.
