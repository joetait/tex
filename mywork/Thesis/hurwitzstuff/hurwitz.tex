...
Then an {\em ad\`ele on $C$} is a map $\alpha\colon  C \rightarrow K(C)$ such $\alpha(P) \in \cO_P$ for nearly all $P \in C$ (i.e. all but a finite number of points).
...
There is a unique module, denoted $\Omega_{K(C)}$, with a map $d\colon K(C) \rightarrow \Omega_{K(C)}$, which every derivation must factor through; i.e. if $D\colon K(C)\rightarrow M$ is a derivation then there is a unique map $f\colon \Omega_{K(C)}\rightarrow M$ such that $D = f\circ d$.
