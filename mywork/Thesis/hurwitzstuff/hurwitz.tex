...
Then we define a {\em differential} on $X$ to be a $k$-linear map $\omega\colon  \cA_X \rightarrow k$ such that $\omega$ is zero on $\cA_X(D) +K(X)$ for some divisor $D\in \cD_X$.
...
A differential $\omega \in \Omega_{K(X)}$ is called holomorphic if its associated divisor, $W$, is non-negative (\ie if $v_P(W) \geq 0$ for all $P\in X$).
