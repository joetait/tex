...
We now compute the ramification divisor of $\pi$.
    ...
    \end{lem}
    \begin{proof}
    We first show that it will suffice to prove that the coefficient of $[P_i]$ is $2n_i$ for $1\leq i \leq k$.\todo[inline]{explain zero otherwise bit here}
...
The case where $m=1$ can again be found in \cite[Prop. 7.4.26]{liu}.
    ...
    Finally, if $\infty$ is not a branch then, after possibly switching $P_\infty$ and $P_\infty'$, we have
        ...
        & \hspace{1em} - mR + m(g+1)D_\infty \\
        & =  iD_0 + {\displaystyle \sum_{i=1}^l} m_i[Q_i] + (mg - i - m - g - 1)[P_\infty'] + (mg - i - m + g + 1 - \deg(F(x)))[P_\infty],
        & =  iD_0 + {\displaystyle \sum_{i=1}^l} m_i[Q_i] + ((m-1)(g-1) - 2 - i)[P_\infty'] + (mg - i - m + g + 1 - \deg(F(x)))[P_\infty],
        \end{align*}
    by Proposition \ref{propdivisorofypis2}, \eqref{equationdivxpis2}, \eqref{equationdifferentialdivisor} and Lemma \ref{h(x)char2}.
    Since $0 \leq 0 \leq (g-1)(m-1)- 2$ then the coefficient of $[P_\infty']$ is clearly non-negative.
