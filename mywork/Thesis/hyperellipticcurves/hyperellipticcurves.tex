...
In the following lemma we compute the divisor of $y \in K(X)$.
    ...
    So for any $P\notin \pi^{-1}(\infty)$ then $\ord_P(y) =  \frac{1}{2}\ord_P(f(x)) = 1$ if $P$ is a ramification point, and $\ord_P(y) = \frac{1}{2}\ord_P(f(x)) = 0$ otherwise.
...
In this case the function field $K(X)$ is $k(x,y)$, a degree two extension of the function field of one variable over $k$, $k(x) = k(\PP_k^1)$, where
    ...
    y^2 - H(x)y = F(x)
    \end{equation}
for some polynomials $H(x), F(x)\in k[x]$, such that $H(x)$ and $H'(x)^2F(x) + F'(x)^2$ have no common roots in $k$ \cite[Prop.\ 7.4.24]{liu}.
...
We let $k$ be the number of distinct roots that $H(x)$ has and $d_H$ the degree of $H(x)$. Then 
...
We now compute the ramification divisor of $\pi$.
    ...
    To see this, note that the maximal ideal $\mathfrak m_{P,X}$ of the local ring $\cO_{X,P}$ at $P$ is generated by $x-a_i$ and $y-b_i$.
...
The case where $m=1$ can again be found in \cite[Prop. 7.4.26]{liu}.
    ...
    Then a basis of $H^0(X,\Omega_X^{\otimes m})$ is given by\par
        {\centering
        \begin{tabular}{c c}
        $\omega, x\omega, \ldots , x^{g-1}\omega$ &  if $m=1$, \\
        $\omega, x\omega, x^2\omega$ & if $m=g=2$, \\
