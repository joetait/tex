...
Before going in to the details of hyperelliptic curves, which vary depending on whether the characteristic of $k$ is two or not, we recall that a curve $X$ is hyperelliptic if there exists a finite separable morphism $\pi \colon X \ra \PP_k^1$ of degree two.
We let $X$ be a hyperelliptic curve of genus $g$ throughout the chapter, and $\pi$ be the corresponding map.  
Throughout the chapter we let $P_a$ and $P_a'$ denote the unique elements of $\pi^{-1}(a)$ for any point $a \in \mathbb P_k^1$ that is not a branch point.
...
and
    ...
    \di(dx) = R - 2D_\infty.
    \end{equation}\todo{reference Riemann-Hurwitz formula for this}
