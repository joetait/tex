...
Clearly, for any $f, g \in K(X)^*$ and $P \in X$, it is true that $\ord_P(fg) = \ord_P(f) + \ord_P(g)$.\todo{inequality}
...
We say that $\omega \in \Omega_{K(X)}$ is a \emph{meromorphic differential} on $X$.
    ...
    \end{proof}
\todo[inline]{mention that $\Omega_{X,P}$ is module of differentials of $\cO_{X,P}$. $dt$ generates as a $\co_{X,P}$ module}
...
We will use these descriptions almost exclusively for the rest of the thesis, and as such the reader may take as their definition if he or she wishes.
    ...
    Note that a sketch of the proof below can be found in \cite[Pg.\ 248]{hart}.
        \end{rem}
    \begin{proof}
    We let $\underline{\Omega}_{K(X)}$ and $\underline{K}(X)$ denote the constant sheaves of $\Omega_{K(X)}$ and $K(X)$ respectively, and $\Omega_X$ denote the sheaf of differentials on $X$.
    ...
    These adjunction maps give a map $\underline{K(X)}/\cO_X \ra \prod_{P \in X}\left( K(X)/\cO_{X,P} \right)$, whose image is actually in the sum $\oplus_{P \in X} i_*\left(K(X)/\c_{X,P} \right)$.
