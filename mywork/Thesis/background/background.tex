...
All definitions and results should be available in any elementary text on algebraic curves or algebraic geometry in general, such as \cite{hart}, \cite{fulton} or \cite{griffithsfirstcourse}.

In the first two sections we define precisely what we mean by a curve, and by functions and differentials on a curve.
...
We then reach the highlight of the section with the statement of the Riemann-Roch theorem, which we follow with some corollaries, to show its applications.
...
Note that any $f\in \cO_{X,P}$ will have non-negative order at $P$, since $df = dut^n = nt^n-1dt$\todo{finish this}
...
We now give two corollaries to the Riemann-Roch theorem.
    ...
    The statement follows by rearranging
        ...
        \end{align*}
    where the first equality is stated prior to the Riemann-Roch theorem, and the second equality follows from the Riemann-Roch theorem itself.\todo{check references here}
    \end{proof}
...
The majority of the topics considered in this thesis will focussed on situations where we have such a projection.\todo{to be edited}
