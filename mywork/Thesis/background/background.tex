...
We now show that if $f\in \cO_{X,P}$ then $\ord_P(df) \geq 0$.
First we assume that $f$ is a unit in $\cO_{X,P}$.
Then $df  = d (f^2/f) = f^2d(1/f) + 2df$ and hence $f^2df = -df$.
We conclude that $\ord_P(df) = \ord_P(d(1/f)) = -\ord_P(df)$, and hence $\ord_P(df) = 0$.
In general, if $f = ut^n$ for some unit $u \in \cO_{X,P}$ and uniformising parameter $t\in \cO_{X,P}$, then we have $df = d(ut^n) = t^ndu + unt^{n-1}dt$, and the order of the right hand side is clearly positive.
