...
When we refer to an \emph{algebraic curve}\index{Algebraic curve|see {curve}} (or often just a \emph{curve}\index{Curve}) we will mean a smooth, connected, projective variety of dimension one over $k$.
In particular, we let $\PP_k^1$ be the projective line\index{Projective line}.
Similarly, when we refer to an \emph{affine curve}\index{Affine curve} we mean a smooth, connected, affine variety of dimension one over $k$. % Add if needed
...
In this section we recall basic results pertaining to functions\index{Function} and differentials\index{Differential} on a curve $X$. 

A \emph{meromorphic function}\index{Function!meromorphic} on $X$ is any morphism $f \colon X \ra \PP_k^1$, other than the morphism mapping all points to infinity.
The collection of meromorphic functions on $X$ is denoted $K(X)$, and called the \emph{function field}\index{Function field} of $X$.
...
Returning to our study of functions on $X$, we recall that a meromorphic function $f$ on $X$ is \emph{regular}\index{Function!regular!on $U$} on an open set $U \subseteq X$ if the image $f(U)$ lies in $k = \AA_k^1 \subset \PP_k^1$.
We let $H^0(U,\cO_X)$ denote the space of functions in $K(X)$ which are regular on $U$.
Moreover, if $f \in K(X)$ is regular on $X$ we say that $f$ is \emph{regular}\index{Function!regular}, and then $H^0(X,\cO_X)$ is the space of regular functions.
...
Given $P \in X$ we say that a meromorphic function $f \in K(X)$ is \emph{regular at $P$}\index{Function!regular!at $P$} if $f(P) \in k \subset \PP_k^1$.
...
We then define \emph{the order of $f$ at $P$}\index{Order!of function}\index{Function!order} to be $\ord_P(f) := n$ (note that this is independent of the choice of $t$).
...
If $\ord_P(f) = n >0$ we say that $f$ has a \emph{zero of order $n$}\index{Function!zero} at $P$, whilst if $\ord_P(f) = n < 0$ then we say that $f$ has a \emph{pole of order $n$}\index{Pole of function}\index{Function!pole} at $P$.
...
We call any element $t \in \cO_{X,P}$ which has order $1$ at $P$ a \emph{uniformising parameter at $P$}\index{Uniformising parameter}.
...
Then a $k$-linear map $D \colon R \ra M$ satisfying $D(fg) = fD(g) + gD(f)$ is called a \emph{derivation}\index{Derivation} of $R$ in to $M$ over $k$.

There exists a unique module $\Omega_k(R)$, called the \emph{module of differentials of $R$ over $k$}\index{Module of differentials}, and a derivation $d \colon R \ra \Omega_k(R)$ through which all derivations of $R$ over $k$ must factor.
...
In this case we call the map $d \colon K(X) \ra \Omega_{K(X)}$ the \emph{differential map}\index{Differential map} and we let $df := d(f)$.
We say that $\omega \in \Omega_{K(X)}$\index{Differential} is a \emph{meromorphic differential}\index{Differential!meromorphic} on $X$.
...
We define the \emph{order of $\omega$ at $P$}\index{Order!of differential} to be $\ord_P(\omega) := \ord_P(f)$, and remark that this is independent of the choice of $t$.
The set of differentials regular at $P$ form a module over $\cO_{X,P}$, which we call the \emph{module of differentials regular at $P$}\index{Module of differentials!regular at $P$}, and denote by $\Omega_{X,P}$.
...
For any $f \in \cO_{X,P}$ we have $df \in \Omega_{X,P}$.
...
We call $\omega \in \Omega_{K(X)}$ \emph{holomorphic on $U$}\index{Differential!holomorphic!on $U$} if $\ord_P(\omega) \geq 0$ for all $P \in U$, and we let
    \[
    H^0(U, \Omega_X) := \{ \omega \in \Omega_{K(X)} | \ord_P(\omega) \geq 0\ \text{for all}\ P \in X\} \cup \{ 0 \}
    \]
be \emph{the space of holomorphic differentials on $U$}\index{Differentials!holomorphic!space of}.
If $\omega \in \Omega_{K(X)}$ is holomorphic on $X$ we say that $\omega$ is \emph{holomorphic}\index{Differentials!holomorphic}, and so $\hzero$ is the space of global holomorphic differentials.
As in \cite[Chap.\ 8, \S 2, Prop.\ 3]{fulton}, the $k$-vector space $\hzero$ is finite dimensional.

    \begin{defn}\label{definitiongenus}
    We define the \emph{genus}\index{Genus} of $X$ to be
        \[
...
We now briefly recall the notion of a \emph{polydifferential}\index{Polydifferential}.
...
Finally, for any open $U \subseteq X$ we define 
    ...
    \]
to be the \emph{space of holomorphic polydifferentials}\index{Polydifferentials!holomorphic!space of} on $U$.
We call the elements of $H^0(X,\Omega_{K(X)}^{\otimes m})$ \emph{global holomorphic polydifferentials}\index{Polydifferentials!holomorphic } on $X$.
...
We first recall that a \emph{divisor}\index{Divisor} on $X$ is a finitely supported formal sum 
    \[
    D = \sum_{P \in X} n_P[P],
    \]
with coefficients in $\ZZ$.
The set off all divisors on $X$ forms an additive group, denoted $\Di (X)$.
The \emph{degree}\index{Degree!divisor of} of the divisor $D$ is $\deg(D) := \sum_{P \in X} n_P$, which lies in $\ZZ$.


Given any function $f \in K(X)$ we define the \emph{divisor associated to $f$}\index{Divisor!associated to $f$} to be
    \[
...
We call any divisor $D$ which is equal to $\di(f)$ for some $f \in K(X)$ a \emph{principal divisor}\index{Principal divisor}.
Is is clear that for any $f, g \in K(X)$ we have $\di(fg) = \di(f) + \di(g)$.
Also, for any $f \in K(X)$ we define $\di_0(f)$ and $\di_\infty (f)$, the \emph{divisor of zeroes}\index{Divisor!of zeroes} and the \emph{divisor of poles}\index{Divisor!of poles} of $f$ respectively, as follows:
    \[
...
Now for any differential $0 \neq \omega \in \Omega_{K(X)}$ we define the \emph{divisor associated to $\omega$}\index{Divisor!associated to $\omega$} to be
    \[
...
If $W$ is a divisor on $X$ and $W = \di(\omega)$ for some $ 0 \neq \omega \in \Omega_{K(X)}$ then we say that $W$ is a \emph{canonical divisor}\index{Canonical divisor} on $X$.

The principal divisors of $X$ form a subgroup of $\Di(X)$, and two divisors $D, D' \in \di(X)$ are \emph{equivalent}\index{Divisor!equivalence of}, denoted $D \sim D'$, if their image in the quotient of $\Di(X)$ by the group of principal divisors is the same; \ie if there exists $f \in K(X)$ such that $ D = D' + \di(f)$.
...
Both of the spaces mentioned above are also referred to as \emph{Riemann--Roch spaces}\index{Riemann--Roch spaces}.
...
It follows from this lemma and the definition of genus that $\dim_k(H^0(X,\cO_X(W))) = g_X$ for any canonical divisor $W$.

We now state the celebrated Riemann--Roch theorem.

    \begin{thm}[Riemann--Roch theorem]\label{theoremriemannroch}\index{Riemann--Roch theorem}
...
We now give some corollaries to the Riemann--Roch theorem.
    ...
    \todo[inline]{Change next corollary?}
    \begin{cor}
...
Using the Riemann--Roch theorem and Corollary \ref{cordegreeofcanonicaldivisor} we can compute the dimension of the space of \emph{holomorphic polydifferentials}\index{Polydifferentials!holomorphic} of order $m$, denoted $H^0(X,\Omega_X^{\otimes m})$, where $m \in \ZZ_{> 0}$.
...
We then define the \emph{degree}\index{Degree!morphism} of $\phi$, denoted $\deg(\phi)$, to be the degree of the extension $K(X)/K(Y)$, which is always finite.
...
Recall that we have
    ...
    We define\index{Ramification} the \emph{ramification index}\index{Ramification!index} $e_P$ of $\phi$ at $P$ to be
        \[
        e_P := \ord_P(\phi^*(t)).
        \]
    \end{defn}
...
We say that the point $Q \in Y$ is a \emph{branch point}\index{Branch point} of $\phi$ if there exists some $P \in \phi^{-1}(Q)$ for which $e_P >1$.
We say that $P \in X$ is a \emph{ramification point}\index{Ramification!point} of $X$ if $e_P >1$.
...
Then if $p = \cha(k)$ divides $e_P$ we say that $P$ is \emph{wildly ramified}\index{Ramification!wild}.
If $p$ does not divide $e_P$ we say that $P$ is \emph{tamely ramified}\index{Ramification!tame}.

    \begin{defn}
    Let $D = \sum_{Q \in Y}n_Q [Q]$ be a divisor on $Y$.
    Then the \emph{pull back}\index{Pull back!of divisor} of $D$ with respect to $\phi$ is
        \[
...
Now we describe the different exponent, which we require to define the ramification divisor.
    ...
    Then we define the \emph{different exponent}\index{Different exponent} at $P$ to be 
        \[
...
Note that since $\phi^*(t)$ is regular at $P$ it follows that $\delta_P$ is non-negative for all $P \in X$.
    ...
    The \emph{ramification divisor}\index{Ramification!divisor}\index{Divisor!ramification} of $\phi \colon X \ra Y$ is 
        \[
...
The following theorem has the classical Riemann--Hurwitz formula as a corollary, but also goes further, actually relating the canonical divisors on $X$ and $Y$.
    \begin{thm}\label{theoremdetailedhurwitz}\index{Riemann--Hurwitz formula}
...
We now introduce the higher ramification groups, which we will use to state Hilbert's formula, which computes the coefficients of the ramification divisor.
   ...
   \begin{defn}
    ...
    Then for $i\geq -1$ we define the \emph{$i^{th}$ ramification group}\index{Ramification!group} at $P$, denoted $G_i(P)$, to be the subgroup of $s\in G$ such that $i_G(s) := \ord_P(s(t)-t)$ is at least $i+1$.
...
vanishes on the image of $\Omega_{K(X)}$, which allows us to make the following definition.
    ...
    Then we define the \emph{trace map}\index{Trace map} to be 
        \[
...
We now combine the product map in \eqref{equationproductmap} with the trace map $t$ to get a map 
    ...
    \begin{thm}\label{theoremserreduality}\index{Serre duality}
