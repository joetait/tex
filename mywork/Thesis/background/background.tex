...
Since $ut$ is also a uniformising parameter, both terms are regular at $P$, and by the triangle inequality their sum is too.\todo{rephrase? Mention triangle inequality earlier}
...
Also, it follows immediately from Proposition \ref{propfinitelymanyzeroesandpoles} that if $D \in Di(X)$ is a divisor with negative degree then $H^0(X,\cO_X(D)) = \{0\}$.
    ...
    Given any divisor $D$ on $X$ we have the following isomorphism,
        ...
        \]\todo{check this is the right way round, and also whether I need to add minuses to coefficients}
...
Using the Riemann-Roch theorem and Corollary \ref{dim=gc} we can compute the dimension of the space of \emph{holomorphic polydifferentials} of degree $m$\todo{Check if degree is correct}, denoted $H^0(X,\Omega_X^{\otimes m})$, where $m \in \ZZ_{> 0}$.
    ...
    \begin{proof}
    Since $g_X\geq 2$ it follows from Corollary \ref{dimg=c} that $\deg(W)\geq1$, and hence $\deg(mW)>\deg(W)$.
