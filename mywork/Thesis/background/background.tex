...
Now for any differential $0 \neq \omega \in \Omega_{K(X)}$ we define the \emph{divisor associated to $\omega$} to be
    ...
    \]
To show that does indeed have finite support we start by recalling for any $P \in X$ and $f \in K(X)$ then $\ord_P(df) = 0$ if $f$ is a unit in $\cO_{X,P}$, which is the case for all but finitely many points $P$ in $X$.
...
Note that by Lemma \ref{lemisomorphismofdifferentialsandfunctions} we can essentially replace $\cO_X$ by $\Omega_X$ in the statement of the Riemann-Roch theorem.
