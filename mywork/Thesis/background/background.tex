...
We now introduce the higher ramification groups, which we will use to state Hilbert's formula, which computes the coefficients of the ramification divisor.
   ...
   \begin{defn}
    ...
    Then for $i\geq -1$ we define the \emph{$i^{th}$ ramification group}\index{Ramification!group} at $P$, denoted $G_i(P)$, to be the subgroup of $s\in G_{-1}(P)$ such that $i_G(s) := \ord_P(s(t)-t)$ is at least $i+1$.
...
This definition implies that the following explicit standard description of the residue map.
