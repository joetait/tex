...
Conversely, the following lemma shows that Hortsch's theorem implies Theorem \ref{theoremsplittingtheorem} if $\deg(f(x)) = p$.
    ...
    Further, the map $x \mapsto ax$, $y \mapsto a^{\frac{p}{2}}y$ is also an isomorphism of $K(X')$ to the hyperelliptic curve given by $y^2 = f(x) = x^p - x$.
    ...
    Then the combined polynomial $f(x) = g(x^p - x)$ obviously has no repeated roots, and it is also clear that $f(x + 1) = f(x)$, and hence we have an automorphism $\tau$, as described in Theorem \ref{theoremsplittingtheorem}.
...
We conclude this chapter with an example which demonstrates that the requirement in Theorem \ref{theoremsplittingtheorem} for $\infty$ to be a branch point $X$ is a necessary condition.
    ...
    Then we can define a map of $k$ vector spaces 
        ...
        by
        \[
        \bar \gamma_3 \mapsto \gamma_3 \quad \text{and} \quad \bar \gamma_4 \mapsto \gamma_4 + \gamma_2.
        \]
    ...
    Using \eqref{equationderhamcommutativediagram} it is easy to verify that
        \[
    ...
    Furthermore, we note that
        \[
        \tau^*(\gamma_1) = \gamma_1 \quad \text{and} \quad \tau^*(\gamma_2) = \gamma_2 + \gamma_1.
        \]

    Finally we conclude that
        \[
        s(\tau^*(\bar\gamma_3)) = s(\bar\gamma_3 + 2\bar \gamma_4) = \gamma_3 + 2\gamma_2 + 2\gamma_4 = \tau^*(\gamma_3) = \tau^*(s(\bar\gamma_3))
        \]
    and
        \[
