...
The following lemma gives us useful descriptions of $\hone$ and $H^1(X,\Omega_X)$.
    ...
    As $i_*$ and $i^{-1}$ are adjoint \cite[Chap.\ II, \S 5]{hart} then the above map is an isomorphism at the level of stalks.
...
We now give the general form of curves with an automorphism $\tau \colon (x,y) \mapsto (x+a, y)$, for $g_X = 4$ and $g_X = 7$.

    \begin{ex}
    It is easy to verify that if 
        \begin{equation}\label{equationgenusfourgeneralcurve}
        f(x) = x^9 + a_6x^6 + a^2a_6x^4 + a_3x^3 + a^4a_6x^2 + 2(a^8 + a^2a_3)x + a_0
        \end{equation}
    satisfies the condition $f(x+a) = f(x)$ for any $a, a_6, a_3, a_0 \in k$.
    Now we fix $a_6, a_3, a \in k$.
    ...
    Then we can compute 
        ...
        \]
    which again does not depend on $a_0 \in k$.
    It is then clear that if we choose $a_0 \in k \backslash {-\beta_1', -\beta_2', -\beta_3'}$ then $f'(x)$ and $f(x)$ will not share any roots, and hence $f(x)$ has no repeated roots.
