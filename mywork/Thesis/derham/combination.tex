...
We now give the general form of curves with an automorphism $\tau \colon (x,y) \mapsto (x+a, y)$ when $g_X = 4$.
    ...
    Note that if
        ...
        f(x) = h(x^p - x)
        \]
    for some polynomial $h \in k[x]$ then clearly $f(x + a) = f(x)$ for any $ a \in k$.
    Such an equation can define a hyperelliptic curve of any genus.
...
Throughout this section we assume that the characteristic of $k$ is $3$.
