...
The following lemma gives us a useful description of $H^1(X,\Omega_X)$.
    ...
    The short exact sequence
        \begin{equation}\label{equationseconadydualitysesfunctions}
    ...
    To explain the isomorphism in \eqref{equationsheafisomorphism} we first construct a map from $\underline{K}(X)/\cO_{X,P}$ in to the product $\prod_{P \in X} i_*\left({K(X)}/\cO_{X,P}\right)$, and then show that this map has finite support.\todo{B: Not the whole map}
    ...
    From this the isomorphism in \eqref{equationsheafisomorphism} follows.\todo{Add more details - mention adjoint}
    
    Replacing $\underline{K}(X)/\Omega_X$ by $\bigoplus_{P\in X} i_*\left(K(X)/\cO_{X,P}\right)$ in \eqref{equationsecondarydualitysesfunctions} yields
        \begin{equation}\label{equationresolutionofox}
        0 \ra \cO_X \ra \underline{K}(X) \ra \bigoplus_{P \in X}i_* \left( K(X)/\cO_{X,P} \right)
        \end{equation}
    Taking cohomology then yields the exact sequence \eqref{equationinitialdualitysesfunctions}.
    
    We can also tensor \eqref{equationresolutionofox} with $\Omega_X$ to get\todo{replace tensoring with repeating above argument again}
        \begin{equation}\label{equationsecondarydualitysesdifferentials}
...
We now describe how the trace map acts on $H^1(X,\Omega_X)$ via the presentation \eqref{equationcechhoneisomorphismdifferentials}.
    ...
    Let $ \omega \in \Omega_X(U_0 \cap U_\infty)$ and let $[\omega]$ denote its residue class in $\hone$ via the isomorphism \eqref{equationcechhoneisomorphismdifferentials}.
    Then we have
        \[
    ...
    \end{lem}
    \begin{proof}
    We take the \cech complex of \eqref{equationsecondarydualitysesfunctions} over the cover $\cU$, yielding the following bicomplex
        \begin{equation}\label{equationdualitycommutativediagramdiffientials}
    ...
    In fact, this sequence is isomorphic to the sequence \eqref{equationinitialdualitysesdifferentials} found in Lemma \ref{lemmaexactsequenceofhoneandhzero}.
    However, we will only exhibit isomorphisms between $\ker(d_3)$ and $\operatorname{coker}(d_1)$ and, respectively, the third and fourth terms of \eqref{equationinitialdualitysesdifferentials}, since this is all we need to examine the trace map.
...
We then recall from \cite[Pg. 351]{grothendiecklettertoatiyah} that the de Rham cohomology of $X$ is precisely the hypercohomology of \eqref{equationderhamcomplex}.
...
Via the isomorphism \eqref{equationcechhoneisomorphismfunctions}, as well as the isomorphism above, we can define the maps
...
We recall that the assumptions made about the polynomials, $f(x)$ and $F(x)$ and $H(x)$ mean that the affine plane curve defined by \eqref{equationdefiningequationpnot2} and \eqref{equationdefiningequationpis2}, respectively, are smooth.
...
Also, we denote the order of the zero of $F(x)$ at $\gamma_i \in k$ by $m_i \in \NN$.
    ...
    From \eqref{equationdivxpis2} and \eqref{equationdivisorofypnot2} we see that
        \begin{align}\label{equationdivisorofyoverx}
    ...
    It then follows from Theorem \ref{theoremserreduality} that the residue classes of $ yx^{-1}|_{U_0\cap U_\infty},\ldots,yx^{-g}|_{U_0\cap U_\infty}$ form a basis of $\hone$.
    ...
    Hence it follows from Theorem \ref{theoremserreduality} that the residue classes of $\frac{y}{x}|_{U_0 \cap U_\infty}, \ldots, \frac{y}{x^g}|_{U_0 \cap U_\infty}$ form a basis of $\hone$.
    
    
    
    If $P_\infty$ is a branch point then we compute the divisor of $ \frac{y}{x^j} \cdot \frac{x^i}{H(x)}dx$, using \eqref{equationdivxpis2}, \eqref{differentialdivisor}, \eqref{equationdivisorofH} and Proposition \ref{propdivisorofypis2}:
        \begin{align*}
...
We now give a corollary to Theorem \ref{theorembasisofhone}, which is of the same style as the Mittag-Leffler theorem, but for hyperelliptic curves.
    ...
    By Theorem \ref{theorembasisofhone} the residue classes $ \gamma_ 1= \left[ \frac{x}{y}\right], \ldots, \gamma_n = \left[\frac{x^g}{y}\right]$ form a basis of $\hone$, it follows that there exist unique $\alpha_1, \ldots, \alpha_g$ such that
        \[
        \delta\left( (f_P)_{P \in X} \right) - \left( \alpha_1\gamma_1 + \ldots + \alpha_n\gamma_n \right) = 0.
        \]
    We can derive the exact sequence \eqref{equationinitialdualitysesfunctions} by applying the snake lemma to the \cech complex of \eqref{equationsecondarydualitysesfunctions} over $\cU$, which is
        \begin{equation}
    ...
    By the exactness of \eqref{equationinitialdualitysesfunctions} it follows that there exists an $f \in K(X)$ which has Laurent tail $f_P - g_P$ at each $P \in X$, as required in the statement of the corollary.
    \end{proof}
...
Viewing $\derhamhone$ as a quotient of \eqref{equationderhamspace}, we now give a $k$-vector space basis of $\derhamhone$.

    \begin{thm}\label{theorembasisofderham}
    If $p \neq 2$ then the residue classes of 
        \begin{equation}\label{equationhonebasislementofderhampnot2}
    ...
    together with the residue classes of 
        \begin{equation}\label{equationzerobasiselementofderhampis2}
...
Before proving this theorem we use it to prove the following corollary.
    ...
    Now we can split $\frac{H(x)}{x^i}$ as follows, 
        ...
        \end{equation*}
    where $B_j$ are as in \eqref{equationcapitalh}.
    ...
    We now consider the action of $\sigma$ on the entries in \eqref{equationhonebasiselementofderhamis2}.
    Firstly we see that
        \begin{align*}
    ...
    We can now describe exactly how sigma acts on the elements of \eqref{equationhonebasiselementofderhamis2} using $H_{1,i}(x)$ and $H_{2,i}(x)$:
        \begin{multline*}
        ...
        \end{multline*}
    So the action of $\sigma$ on the basis elements in \eqref{equationhonebasiselementofderhamis2} amounts to adding 
        \[
        \left( d\left(\frac{H(x)_{1,i}}{x^i}\right), d\left(\frac{H(x)_{2,i}}{x^i}\right), \frac{H(x)}{x^i} \right),
        \]
    which clearly satisfies the conditions of \eqref{equationderhamquotient} and hence is zero.
    So the action of the involution $\sigma$ on $\derhamhone$ is trivial and hence the action of the group $G$ is not faithful.
    \end{proof}

    \begin{rem}
    We briefly study the action of $\sigma$ on the elements \eqref{equationhonebasisofderhampnot2} (when $p\neq 2$).
...
We now prove Theorem \ref{thoerembasisofderham}.

    \begin{proof}
    We make use of the fact that the short exact sequence in Lemma \ref{propshortexactsequence} splits as a sequence of vector spaces over $k$, and that we know bases of the outer two terms.
    
    It is clear that the elements in \eqref{equationhzerobasiselementoderhampnot2} and \eqref{equationhzerobasiselementofderhampis2} are elements of the space \eqref{equationderhamspace}. 
    ...
    Moreover, it is obvious that if the elements in \eqref{equationhonebasisofderhampnot2} and \eqref{equationhonebasiselementofderhamis2} are well defined elements of the space \eqref{equationderhamspace} then they will map to the basis of $\hone$ given in Theorem \ref{theorembasisofhone}.
    So we need only show that the terms in \eqref{equationhonebasisofderhampnot2} and \eqref{equationhonebasiselementofderhamis2} satisfy the conditions stated in \eqref{equationderhamspace}.
    ...
    This shows that the elements in \eqref{equationhonebasisofderhampnot2} satisfy $df_{0, \infty} = \omega_0 - \omega_\infty$, one of the conditions of \eqref{equationderhamspace}.
    ...
    So in order to prove that for $i\in \{1, \ldots, g\}$ the elements of \eqref{equationhonebasiselementofderhamis2} are satisfy the conditions of \eqref{equationderhamspace} it only remains to show that the differentials $\frac{\Phi_i(x,y)}{x^{i+1}H(x)}dx$ and $\frac{\Psi_i(x,y)}{x^{i+1}H(x)}dx$ are regular on $U_\infty$ and $U_0$ respectively.
    ...
    By \eqref{equationsplittingphiandpsi} $\Phi_i(x,y) = \Phi_i^x(x) + y\Phi_i^y(x)$ and $\Psi_i (x,y)= \Psi_i^x(x) + y\Psi_i^y(x)$, and we will use these splittings to show that $\frac{ \Phi_i(x,y) }{x^{i+1}H(x)}dx$ and $\frac{\Psi_i(x,y) }{x^{i+1}H(x)}dx$ are regular on $U_\infty$ and $U_0$ respectively.\todo{note that some stuff was removed before this para}
    ...
    This yields
        ...
        \end{align*}
    using \eqref{differentialdivisor}, \eqref{equationdivisorofH} and \eqref{equationdivxpis2}.
    ...
    We now compute the divisor of the other half of $\frac{\Phi_i(x,y)}{x^{i+1}H(x)}dx$, namely
        ...
        \end{align*}
    From Proposition \ref{propdivisorofypis2} we see that $y$ only has poles at points in $\pi^{-1}(\infty)$, and hence this completes the proof that $\frac{\Phi_i(x,y) }{x^{i+1}H(x)}dx$ is regular on $U_\infty = X \backslash \pi^{-1}(\infty)$.
...
In the previous section (Theorem \ref{thoerembasisofderham}) we found a basis for the de Rham cohomology of any hyperelliptic curve using \cech cohomology, with respect to the cover $\cU = \{ U_0 , U_\infty\}$.
...
splitting the polynomial $(x-a)^g$ in to two parts.
    ...
    In order to be able to refer to the entries in $\nu_i$ we let
        ...
        \]
    First, note that it follows from the proof of Theorem \ref{thoerembasisofderham} that $d(f_{0 \infty i}) = \omega_{0 i} - \omega_{\infty i}$, and that $f_{0 \infty i}, \omega_{0 i}$ and $\omega_{\infty i}$ are regular on the appropriate open sets.
    ...
    Firstly, by \eqref{equationdivxpis2} and \eqref{differentialdivisor},
        \begin{align*}
    ...
    On the other hand, by \eqref{equationdivxpis2} and \eqref{differentialdivisor},
        \begin{align*}
    ...
    is regular on $U_a$ we first compute the divisor
        ...
        \end{align*}
    using \eqref{equationdivxpis2}, \eqref{differentialdivisor} and \eqref{equationdivisorofypnot2}.
...
Here $\rho$ and $\rho'$ are the projections on the first, third and fifth coordinates and on the second, third and sixth coordinates respectively.
    ...
    \end{lem}
    \todo[inline]{Change statement to match that of following theorem}
    \begin{proof}
...
%    \end{rem}
    ...
    Recall from Proposition \todo{find refernece} that we have a canonical short exact sequence
    ...
    Then let $G = \langle \tau \rangle$ be the subgroup of $\aut(X)$ generated by $\tau$.
    Then sequence \eqref{equationderhamcohomologyshortexactseqeunce} does not split when as a sequence of $k[G]$-modules.\todo{possibly rephrase}
    \end{thm}
    \begin{proof}
    By Lemma \ref{lemmatauactsbyplusminusoneony} we have $\tau^*(y) = y$ or $\tau^*(y) = -y$.
    Without loss of generality we may as well assume that $\tau(y) = y$ since, if $\tau(y) = -y$, we may replace $tau$ by $\tau \circ \sigma$ (where $\sigma$ is the hyperelliptic involution of $X$) and notice the sequence splits as a sequence of $K[G]$-modules if and only if it splits as a  $K[\langle \tau \circ \sigma \rangle]$-modules (see remark after Corollary).\todo{reference corollary, check statement} 
    ...
    Then it follows that for all $\alpha \in \hone$ then
        \begin{equation}\label{equationcommutativityoftauandsplittingmap}
        s(\tau^*(\alpha)) = \tau^*(s(\alpha)) \in \derhamhone.
        \end{equation}
    We will show that this equality gives rise to a contradiction when $\alpha$ is the residue class of $\frac{y}{x^g}|_{U_0 \cap U_\infty}$ in $\hone$ (see \todo{find references - $\hone$ isomorphism } and Theorem \ref{theorembasisofhone}).  
    It will then follow that no splitting map (and hence no splitting map) can exist.
    ...
    In order to do this we consider the following obvious commutative diagram of isomorphisms:
        ...
        \end{array}
        \]
    From the proof of Proposition \todo{label and reference sixtuple proposition} we know that $\rho^{-1} \left(\left[\frac{y}{x^g}\right]\right)$ is the residue class of
        \[
        \left( \frac{r_g(x)y}{x^g(x-a)^g}, \frac{y}{x^g}, \frac{t_g(x)y}{x^g(x-a)^g} \right) = \left( \frac{((x-a)^g-x^g)y}{x^g(x-a)^g}; \frac{y}{x^g}, \frac{y}{(x-a)^g} \right).
        \]  \todo{State where this lives?}
    Therefore 
        \begin{align*}
        \tau^* \left( \left[ \frac{y}{x^g} \right] \right)  & = \tau^* \left( \rho' \left(\rho^{-1}\left(\left[\frac{y}{x^g}\right)\right)\right) \\
        & = \tau^*\left( \rho' \left( \left[ \frac{((x-a)^g-x^g)y}{x^g(x-a)^g}, \frac{y}{x^g}, \frac{y}{x^g}, \frac{y}{(x-a)^g} \right] \right) \right) \\
        & = \tau^*\left( \left[ \frac{y}{(x-a)^g} \right] \right) \\
        & = \left[ \frac{y}{x^g} \right],
        \end{align*}
    \ie the element $\left[ \frac{y}{x^g} \right] \in \hone$ is fixed by $\tau^*$.

    Since the residue class of $\left[ \frac{y}{x^g} \right]$ maps to the basis element $\gamma_g \in derhamhone$ it follows that
        \[
    ...
    Now the action of $\tau^*$ on $\lambda$ is easily seen to be given by
        \begin{align*}
        \tau^*(\lambda_i) & = \tau^*\left( \left[ \frac{x^i}{y}dx, \frac{x^i}{y}dx, 0\right] \right) \\
        & = \left[ \frac{(x+a)^i}{y}dx, (x+a)^i}{y}dx, 0 \right] \\
    ...
    Then, by \eqref{equationcommutativityoftauandsplittingmap}, it follows that
        ...
        & = \left( \tau^*\left( \left[ \frac{y}{x^g} \right] \right) \right) \\
        & = \tau^* \left( \gamma_g + \sum_{i=0}^{g-1} d_i \lambda_i \right) \\
        & = \left( \gamma_g + \sum_{i=0}^{g-1} d_i \lambda_i \right) + \sum_{i=0}^{g-1} d_i \left( \sum_{k = 0}^i \binom{k}{i} a^{i-k} \lambda_i \right),
        \end{align*}
    and hence, by comparing coefficents of the basis elements $\lambda_{g-1}$, we see that $c_{g-1} = 0$.
    We now show that the defining equation
        \[
        \tau^*(\gamma_g) = \gamma_g + \sum_{i=0}^{g-1} c_i \lambda_i
        \]
    implies that $c_{g-1} = a/4$.
    Since we assumed that $a \neq 0$ this will give us the contradiction we desire.

    To compute $\tau^*(\gamma_g)$ we consider the following commutative of isomorphisms
        \[
        \begin{array}{ccc}
        \derhamhone \cong \cechderhamhone(\cU)  & \xleftarrow{\rho} & \cechderhamhone(\cU'')  \\
                \tau^*\downarrow & ~ & \rho'\downarrow  \\
        ...
        \end{array}
        \]\todo{have both $\rho$ and $\rho'$ been defined?}
...
%    used in the proof of Proposition \ref{propbasisoftriplecoverderham}.
    Then 
        \begin{align*}
        \tau^*(\gamma_g) & = \tau^*(\rho'(\rho^{-1}(\gamma_g))) aa
        & = \tau^*\left[ \left( \ldots, \frac{- \phi_g(x)}{2yx^{g+1} dx, \frac{y}{(x-a)^g} \right) \right]
        & = \left[ \left( \tau^*(\ldots) , \frac{-\phi_g(x+a)}{2y(x+a)^{g+1}}dx, \frac{y}{x^g} \right) \right].
        \end{align*}
    On the other hand, we have 
        ...
        \end{equation*}
    Since any element of the form $(\omega_0, \omega_\infty, 0)$ in the subspace \todo{get reference} of the space \todo{getreference} is in fact zero, we conclude that
        \[
    ...
    Finally, since
        ...
        \]
    for some $m\in \ZZ$ coprime to $p$ (by Lemma \todo{reference}), we obtain
        \[
    ...
    In particular, it follows from \cite{canonicalrepresentation} that the short exact sequence in Proposition \ref{propshortexactsequence} doesn't split when the above conditions hold.
    \end{ex}
