...
The following lemma gives us useful descriptions of $\hone$ and $H^1(X,\Omega_X)$.
    ...
    For each $P \in X$, which has a natural embedding $i\colon \{P\} \hookrightarrow X$, we view the module $K(X)/\cO_{X,P}$ as a sheaf on the singleton $\{P\}$.
    ...
    Also, since each $P \in X$ has a corresponding map $i$, we can actually produce a map in to the product $\prod_{P \in X} i_*(K(X)/\cO_{X,P}$.
    For any $f \in K(X)$ then $f$ lies in $\cO_{X,P}$ for all but a finite number of points $P \in X$, and hence the image of $f$ in $\prod_{P \in X} i_*\left(K(X)/\cO_{X,P}\right)$ is zero in almost all factors.
    ...
    Replacing $\underline{K}(X)/\cO_X$ by $\bigoplus_{P\in X} i_*\left(K(X)/\cO_{X,P}\right)$ in \eqref{equationsecondarydualitysesfunctions} yields
        ...
        \end{equation}
    Taking cohomology we arrive at the exact sequence \eqref{equationinitialdualitysesfunctions}.
    ...
    We have the following equalities:
        ...
        \end{align*}
    Then for any $P \in X$ we have the map $i_* \circ i^{-1} \colon \underline{\Omega}_{K(X)}/\Omega_X \ra i_*\left( \Omega_{K(X)}/\Omega_{X,P} \right)$, which is an isomorphism at the level of stalks.
    Finally, the image of any $f \in \underline{\Omega}_{K(X)}/\Omega_{X,P}$ in $\prod_{P \in X} i_*\left( \Omega_{K(X)}/\Omega_{X,P}\right)$ is zero in almost all factors, and from this we derive the isomorphism in \eqref{equationsheafisomorphismdifferentials}.
...
We fix such a map $\pi \colon X \rightarrow \mathbb P_k^1$ of degree two, which is unique up to an automorphim of $\mathbb P_k^1$ (see \cite[Rem. 7.4.30]{liu}).
...
We now describe how the trace map acts on $H^1(X,\Omega_X)$ via the presentation \eqref{equationcechhoneisomorphismdifferentials}.
    ...
    \end{lem}
    \begin{proof}
    We take the \cech complex of \eqref{equationsheafisomorphismdifferentials} over the cover $\cU$, yielding the following bicomplex
    ...
    In fact, this sequence is isomorphic to the sequence \eqref{equationinitialdualitysesdifferentials} found in Lemma \ref{lemmaexactsequenceofhoneandhzero}, though we do not prove this here.
    ...
    Then given any element $\omega_P)_{U_0 \cap U_\infty} \in \bigoplus \Omega_{K(X)}/\Omega_{X,P}$ we can define
        ...
        (\omega_P') := 
            ...
            \end{cases}
        \]  
    Then clearly $d_3((\omega_P')_{P \in U_0}, 0) = (\omega_P)_{P \in U_0 \cap U_\infty}$.
    ...
    To show the isomorphism $\ker(d_3) \cong \bigoplus_{P \in X} \Omega_{K(X)}/\Omega_{X,P}$ we first observe that the kernel of $d_3$ is formed of pairs $ ((\omega_P)_{P \in U_0}, (\omega_P')_{P \in U_\infty})\in \left( \bigoplus_{P \in U_0} \Omega_{K(X)}/\Omega_{X,P} \right) \times \left( \bigoplus_{P \in  U_\infty} \Omega_{K(X)}/\Omega_{X,P} \right)$ such that $\omega_P = \omega_P'$ for $ P \in U_0 \cap U_\infty$.
...
Again applying an automorphism of $\mathbb P_k^1$ if necessary, we can assume that $H(x)$ is monic.
...
We now recall the details of some divisors discussed in Chapter \todo{cite}, which we will need later.
...
As above, the $A_i$ are the branch points of $\pi$ and we let $P_i \in X$ be the corresponding ramification points.
...
and from this it follows that
    ...
    \end{equation}
\todo[inline]{check use of sign in char 2}
...
Also, we denote the order of the zero of $F(x)$ at $\gamma_i \in k$ by $m_i \in \NN$.
    ...
    The hyperelliptic involution $\sigma$ interpolates these two points.
...
We now give a corollary to Theorem \ref{theorembasisofhone}, which is of the same style as the Mittag-Leffler theorem\todo{reference}, but for hyperelliptic curves.
    ...
    Now $\delta$ is the differential map $\ker(d_3) = \bigoplus_{P \in X}K(X)/\cO_{X,P} \ra \operatorname{coker}(d_1) = \hone$ in the statement of the snake lemma.\todo{reference snake lemma}
