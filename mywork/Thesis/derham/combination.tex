...
The following lemma gives us a useful description of $H^1(X,\Omega_X)$.
    ...
    To explain the isomorphism in \eqref{equationsheafisomorphism} we first construct a map from $\underline{K}(X)/\cO_{X,P}$ in to the product $\prod_{P \in X} i_*\left({K(X)}/\cO_{X,P}\right)$, and then show that the image of each element under this map has finite support.
    ...
    Given $i\colon \{P\} \hookrightarrow X$ we have the following equalities
        ...
        & = K(X)/\cO_{X,P}.
        \end{align*}
    It follows that for any $P \in X$ we have a map $\underline{K}(X)/\cO_X \ra i_* \left( K(X)/\cO_{X,P} \right)$.
    ...
    Moreover, for any $f \in K(X)$ then $f$ lies in $\cO_{X,P}$ for all but a finite number of points $P \in X$, and hence the image of $f$ in $\prod_{P \in X} i_*\left(K(X)/\cO_{X,P}\right)$ is zero in almost all factors.
...
We now give a corollary to Theorem \ref{theorembasisofhone}, which is of the same style as the Mittag-Leffler theorem, but for hyperelliptic curves.
    ...
    Now $\delta$ is the differential map $\ker(d_3) = \bigoplus_{P \in X}K(X)/\cO_{X,P} \ra \operatorname{coker}(d_1) = \hone$ in the statement of the snake lemma.
    Hence we can perform a diagram chase to find the element in $\ker(d_3)$ which maps $\left( \gamma_1 + \ldots + \gamma_g \right) \in \hone$ via this differential.
    Firstly, it is clear that $\gamma_1 + \ldots + \gamma_g$ pulls back to 
        ...
        \end{equation}
    Since $\alpha_ix^i/y$ is regular on $U_\infty \cap U_0$, \eqref{immediate} is equal to $\left( (g_P)_{P \in U_0}\, 0\right)$, where
        \[
        g_P =
            \begin{cases}
            ...
            \end{cases}
        \]
    Clearly $\left( (g_P)_{P \in U_0},0\right) \in \ker (d_3) = \bigoplus_{P \in X} K(X)/\cO_{X,P}$, and $\delta \left( (g_P)_{P \in U_0}, o \right) = \gamma_1 + \ldots + \gamma_g$.
    Hence $\delta((f_P)_{P \in X} - (g_P)_{P \in X}) = 0$, and by the exactness of \eqref{equationinitialdualitysesfunctions} it follows that there exists an $f \in K(X)$ which has Laurent tail $f_P - g_P$ at each $P \in X$, as required in the statement of the corollary.
    \end{proof}
...
We now prove Theorem \ref{theorembasisofderham}.
    ...
    By \eqref{equationsplittingphiandpsi} $\Phi_i(x,y) = \Phi_i^x(x) + y\Phi_i^y(x)$ and $\Psi_i (x,y)= \Psi_i^x(x) + y\Psi_i^y(x)$, and we will use these splittings to show that $\frac{ \Phi_i(x,y) }{x^{i+1}H(x)}dx$ and $\frac{\Psi_i(x,y) }{x^{i+1}H(x)}dx$ are regular on $U_\infty$ and $U_0$ respectively.
...
%    \end{rem}
    ...
    From the proof of Proposition \ref{propbasisoftriplecoverderham} we know that $\rho^{-1} \left(\left[\frac{y}{x^g}\right]\right)$ is the residue class of
        \[
        \left( \frac{r_g(x)y}{x^g(x-a)^g}, \frac{y}{x^g}, \frac{t_g(x)y}{x^g(x-a)^g} \right) = \left( \frac{((x-a)^g-x^g)y}{x^g(x-a)^g}, \frac{y}{x^g}, \frac{y}{(x-a)^g} \right) \in \cechhone(\cU'').
        \]  
    Therefore 
        \begin{align*}
