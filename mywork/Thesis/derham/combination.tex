...
We then use this to prove that the sequence of $k[G]$-modules
    ...
    \end{equation}
is exact, see Proposition \ref{propshortexactsequence}.
The rest of the chapter will then build towards showing that for a particular class of hyperelliptic curves this sequence does not split.
...
As an application of this we then give a Mittag-Leffler style theorem for hyperelliptic curves, see Corollary \ref{cormittagleffler}.

In the next section we compute a $k$ vector space basis of $\derhamhone$, which features the bases of $\hzero$ and $\hone$ already mentioned, as well as other components, see Theorem \ref{theorembasisofderham}.
...
Throughout this chapter we assume that $X$ is hyperelliptic of genus $g \geq 2$.
We recall from Chapter \ref{chapterhyperellipticcurves} that a curve is hyperelliptic if there exists a finite, separable morphism of degree two from the curve to $\mathbb P_k^1$.\index{Hyperelliptic curve}
...
Via this differential we have the following cochain complex
    ...
    \end{equation*}
The first cohomology group of this chain complex is $\check{H}^1({\cU},\cO_X) = \frac{\cO_X(U_0 \cap U_\infty)}{\Ima(\check{d})}$ and hence
...
We now describe how the trace map acts on $H^1(X,\Omega_X)$ via the presentation \eqref{equationcechhoneisomorphismdifferentials}.
    ...
    \end{lem}
    \begin{proof}
    We take the \cech complex of \eqref{equationshortexactsequencedifferentialsdirectsum} over the cover $\cU$, yielding the following bicomplex, with exact rows
        ...
        \end{equation}
    The exactness of the rows can be derived from \ref{lemmaexactsequenceofhoneandhzero}, by replacing $X$ by $U_0$ and $U_\infty$, and noting that in this case the first cohomology group will vanish, by Serre's affineness criteria \cite[Thm.\ 5.2.23]{liu}.
    ...
    Now recall that $d_3$ is defined by $((\omega_P)_{P \in U_0}, (\omega_P')_{P \in U_\infty}) \mapsto (\omega_P - \omega_P')_{P \in U_0 \cap U_\infty}$.
    ...
    This then maps to 
        ...
        \]
    By commutativity of the diagram $\psi \in \ker(d_3) \cong \bigoplus_{P \in X}\Omega_{K(X)}/\Omega_{X,P}$.
    This means that $\omega_P$, and hence $\psi$, is zero for any $P \in U_0 \cap U_\infty$.
    Since $\psi$ is also zero for $P \in \pi^{-1}(\infty)$ it follows that 
        \[
...
We now give a corollary to Theorem \ref{theorembasisofhone}, which is of the same style as the Mittag-Leffler theorem. For a description of the classical Mittag-Leffler problem see \cite[Pgs.\ 180-181]{miranda}.\index{Mittag-Leffler problem}
    ...
    \begin{cor}\label{cormittagleffler}
    ...
    By Theorem \ref{theorembasisofhone} the residue classes $ \gamma_ 1= \left[ \frac{y}{x}\right], \ldots, \gamma_g = \left[\frac{y}{x^g}\right]$ form a basis of $\hone$, and it follows that there exist unique $\alpha_1, \ldots, \alpha_g \in k$ such that
        \[
    ...
    Since $\alpha_ix^i/y$ is regular on $U_\infty \cap U_0$, then \eqref{immediate} is equal to $\left( (g_P)_{P \in U_0}, 0\right)$, where
        ...
        g_P =
            ...
            \end{cases}
        \]
    Clearly $\left( (g_P)_{P \in U_0},0\right) \in \ker (d_3) = \bigoplus_{P \in X} K(X)/\cO_{X,P}$, and $\delta \left( (g_P)_{P \in U_0}, 0 \right) = \alpha_1\gamma_1 + \ldots + \alpha_g\gamma_g$.
...
We now define the following polynomials:
    ...
    \Phi_i^y(x) & =  B_{g+1,i}x^{g+1} + \ldots B_{i+1,i}x^{i+1}, \\
...
Before proving this theorem we use it to prove the following corollaries.
    ...
    Let $p \neq 2$.
    Then the hyperelliptic involution acts on $\derhamhone$ by multiplication with $-1$.
    \end{cor}
    \begin{proof}
...
We now prove Theorem \ref{theorembasisofderham}.
    ...
    If we assume that $\infty$ is a branch point then  using Proposition \ref{propdivisorofypis2} we see that
        \begin{align*}
        \di\left(\frac{y\Psi_i^y(x) }{x^{i+1}H(x)}dx \right)  =  & \di_0(y) - (2g+1)[P_\infty] + \di(\Psi_i^y(x)) - (i+1)D_0 + 2(g+i)[P_\infty] \\
         =  & \di_0(y) + \di(\Psi_i^y(x)) -(i+1)D_0 + (2i -1)[P_\infty] \\
         = &  \di_0(y) + \di_0(\Psi_i^y(x)) - 2d_\Psi^y[P_\infty] - (i+1)D_0 + (2i-1)[P_\infty] \\
...
We keep the assumptions of the previous section, and we also assume that $\cha(k) = p \geq 3$.
...
These split the polynomial $(x-a)^g$ in to two parts.
    ...
    We begin this by computing $df_{0 a i}$, which is
        ...
        & = \frac{xf'(x)r_i(x)(x-a) + 2f(x)(xr_i'(x)(x-a) - i(x-a)r_i(x) - gxr_i(x))}{2yx^{i+1}(x-a)^{g+1}} dx.
        \end{align*}
    ...
    Note that the last relation ($df_{a \infty i} = \omega_{a i} - \omega_{\infty i}$) holds, since 
        \[
        df_{a \infty i} = df_{0 \infty i} - df_{0 a i} = \omega_{0 i} - \omega_{\infty i } - \omega_{0 i} + \omega_{a i} = \omega_{a i} - \omega_{\infty i}.\qedhere 
        \]\end{proof}
...
Hence the following diagram commutes 
    ...
    \mathbb P^1_k & \xrightarrow[\bar\tau] & \mathbb P_k^1
    \end{array}
    \]
...
\begin{lem}\label{lemmatauactsbyplusminusoneony}
    ...
    We derived above that $f(x) = f(x+a)$.
...
%    \end{rem}
    ...
    In order to do this we consider the following obvious commutative diagram of isomorphisms:
        \[
        \begin{array}{cccccc}
        \hone & \xleftarrow{\sim} & \cechhone{\cU}  & \xleftarrow{\rho} & \cechhone{\cU''}  \\
        \tau^*\downarrow & ~ & ~ & ~ & \rho'\downarrow  \\
        \hone & \xleftarrow{\sim} & \cechhone{\cU}  & \xleftarrow{\tau^*} & \cechhone{\cU'}
        \end{array}
        \]
    ...
    Now the action of $\tau^*$ on $\lambda_i$ for $0 \leq i \leq g-1$ is easily seen to be given by
        ...
        & = \sum_{k=0}^i \binom{i}{k} a^{i-k} \lambda_k.
        \end{align*}
    Then, by \eqref{equationcommutativityoftauandsplittingmap}, it follows that
        ...
        & = \left( \gamma_g + \sum_{i=0}^{g-1} c_i \lambda_i \right) + \sum_{i=0}^{g-1} d_i \left( \sum_{k = 0}^i \binom{i}{k} a^{i-k} \lambda_k \right).
        \end{align*}
    ...
    To compute $\tau^*(\gamma_g)$ we consider the following commutative diagram of isomorphisms
        \begin{equation}\label{equationderhamcommutativediagram}
        \begin{array}{ccccc}
        \derhamhone & \xleftarrow{\sim} & \cechderhamhone(\cU)  & \xleftarrow{\rho} & \cechderhamhone(\cU'')  \\
        \tau^*\downarrow & ~ & ~ & ~ & \rho'\downarrow  \\
        \derhamhone & \xleftarrow{\sim} & \cechderhamhone(\cU)  & \xleftarrow{\tau^*} & \cechderhamhone(\cU')
        \end{array}
        \end{equation}
...
%    used in the proof of Proposition \ref{propbasisoftriplecoverderham}.
    ...
    Since any element of the form $(\omega_0, \omega_\infty, 0)$ in the subspace \eqref{equationderhamquotient} of the space \eqref{equationderhamspace} is in fact zero, we conclude, by comparing the second entries of \eqref{equationactionoftauongamma} and \eqref{equationimageofgammaunders}, that
        \[
        - \frac{\phi_g(x+a)}{2y(x+a)^{g+1}} dx = -\frac{\phi_g(x)}{2yx^{g+1}}dx + \sum_{i = 0}^{g-1} c_i \frac{x^i}{y}dx
        \]
    in $\Omega_{K(X)}$.
...
Conversely, the following lemma shows that Hortsch's theorem implies Theorem \ref{theoremsplittingtheorem} if $\deg(f(x)) = p$.
    ...
    If $b \in k$ is a root of $f(x)$ then the map $x \mapsto x+b$, $y \mapsto y$ is an isomorphism of $K(X)$ to the function field of the hyperelliptic curve $X'$ given by $y^2 = f(x) = x^p - a^{p-1}x$.
    Further, the map $x \mapsto ax$, $y \mapsto a^{\frac{p}{2}}y$ is an isomorphism of $K(X')$ to the function field of the hyperelliptic curve given by $y^2 = f(x) = x^p - x$.
    Combining these isomorphisms we see that our original curve is isomorphic to that defined by $y^2 = x^p - x$, concluding the proof.
    \end{proof}

    Since we do not, a priori, enforce any conditions on the degree of $f(x)$, it is plausible that Theorem \ref{theoremsplittingtheorem} is much more general that Hortsch's theorem. In fact the following example is a simple and general method to obtain polynomials $f(x)$ of any odd degree, and hence any genus, such that Theorem \ref{theoremsplittingtheorem} applies to the curve given by $y^2 = f(x)$. Of course this will only hold in a finite number of characteristics for any fixed genus.


    \begin{ex}
    Let $g(x) \in k[x]$ be a polynomial of odd degree without repeated roots.
    Then the combined polynomial $f(x) = g(x^p - x)$ obviously has no repeated roots, and it is also clear that $f(x + 1) = f(x)$, and hence we have an automorphism $\tau$, as in the statement of Theorem~\ref{theoremsplittingtheorem}.

    Moreover, if $g \geq 2$ and $p$ divides $2g+1$, and if we choose $h(x)$ to be a polynomial of degree $n := (2g+1)/p$ then the curve defined by $y^2 = f(x) := h(x^p - x)$ is of genus $g$, and satisfies the criteria of Theorem \ref{theoremsplittingtheorem}.
    \end{ex}
...
We now examine hyperelliptic curves that satisfy the requirements of Theorem \ref{theoremsplittingtheorem} of genus 4 in full generality.
    ...
    Now we fix $a_6, a_3 \in k$, such that $a_6$ or $a_3 + a^6$ is non-zero.
    
    Then $f'(x) = a^2a_6x^3 + 2a^4a_6x + 2(a^8 + a^2a_3)$ is non-zero.
...
We conclude this chapter with an example which demonstrates that the requirement in Theorem \ref{theoremsplittingtheorem} for $\infty$ to be a branch point $X$ is a necessary condition.
    ...
    By \cite[Table 1]{automorphismshyperellipticmodular}, the modular curve $X_0(22)$ is the hyperelliptic curve of genus 2 defined by
        ...
        \]
    and the automorphism group of $X_0(22)$ is $D_6$.
    ...
    We first notice that the map $x \mapsto x-1$, $y \mapsto y$ is an isomorphism of $K(X_0(22))$ to the function field of the curve defined by $y^2 = f(x) = x^6 + 2x^4 + 2x^2 + 2$.
    We now apply a further isomorphism to this curve.
    ...
    By Theorem \ref{theorembasisofderham} a basis of $\cechderhamhone(\cU)$ is given by
        \begin{align*}
        \lambda_1 & =\left[ \left( \frac{1}{y}dx, \frac{1}{y}dx, 0 \right) \right] \\
        \lambda_2 & =\left[ \left(\frac{x}{y}dx, \frac{x}{y}dx, 0\right) \right] \\
        \gamma_1 & =\left[\left( \frac{1}{yx^2}dx, \frac{x^4+2x^2}{y}dx, \frac{y}{x} \right) \right] \\
        \gamma_2 & =\left[\left( \frac{x^2 +1}{2yx^3}dx, \frac{2x^3}{y}dx, \frac{y}{x^2}\right) \right],
        \end{align*}
    and we let $\bar\gamma_i$ be the image in $\hone$ of $\gamma_i$ under the projection $p \colon \derhamhone \ra \hone$.
    In particular, $\bar \gamma_1$ and $\bar \gamma_2$ form a basis of $\hone$.

    Then we can define a map of $k$ vector spaces 
        \[
        s \colon \hone \ra \derhamhone
        \]
        by
        \[
        \bar \gamma_1 \mapsto \gamma_1 \quad \text{and} \quad \bar \gamma_2 \mapsto \gamma_2 + \lambda_2.
        \]
    Clearly $p \circ s$ is the identity map on $\hone$, and hence if $s$ is $k[\langle \tau \rangle]$-linear the sequence in Proposition \ref{propshortexactsequence} does split as a sequence of $k[\langle \tau \rangle]$-modules.

    We now show that $s$ is $k[\langle \tau \rangle]$-linear.
    Applying Proposition \ref{propbasisoftriplecoverderham} to the basis above we see that the pre-images of $\gamma_1$ and $\gamma_2$ in $\cechderhamhone(\cU'')$ are the residue classes of
        \[
        \nu_1 = \left(\frac{1}{yx^2}dx, \frac{x^4+2x^3+2x^2}{2y(x-1)^3}dx, \frac{x^4+2x^2}{y}dx, \frac{y}{x(x-1)^2}, \frac{y}{x}, \frac{y(x+1)}{(x-1)^2} \right)
        \]
    and
        \[
        \nu_2 = \left( \frac{1+x^2}{2yx^3}dx, \frac{x^3+x^2+x+1}{2y(x-1)^3}dx, \frac{2x^3}{y}dx, \frac{y(x+1)}{x^2(x-1)^2}, \frac{y}{x^2}, \frac{y}{(x-1)^2} \right).
        \]
    
    Using a computation similar to \eqref{equationderhamcommutativediagram} it is easy to verify that
        \[
        \tau^*(\gamma_1) = \tau^* (\rho'(\nu_1)) = 2\lambda_2 + 2\gamma_2 + \gamma_1
        \]
    and that
        \[
        \tau^*(\gamma_2) = \tau^*(\rho'(\nu_2)) = \gamma_2 + 2 \lambda_1.
        \]
    Furthermore, we note that
        \[
        \tau^*(\lambda_1) = \lambda_1 \quad \text{and} \quad \tau^*(\lambda_2) = \lambda_2 + \lambda_1.
        \]

    Finally we conclude that
        \[
        s(\tau^*(\bar\gamma_1)) = s(\bar\gamma_1 + 2\bar \gamma_2) = \gamma_1 + 2\lambda_2 + 2\gamma_2 = \tau^*(\gamma_1) = \tau^*(s(\bar\gamma_1))
        \]
    and
        \[
        s(\tau^*(\bar\gamma_2)) = s(\bar\gamma_2) = \gamma_2 + \lambda_2 = \tau^*(\gamma_2 + \lambda_2)  = \tau^*(s(\bar\gamma_2)).
        \]
    Hence $s$ is $k[\langle \tau \rangle]$-linear, and the sequence in Proposition \ref{propshortexactsequence} splits.
    \end{ex}



%
%\section{Example of split sequence}
%
%As in \todo{citation}, the curve defined by the equation
%    \begin{equation}\label{equation22modularcurve}
%    y^2 = \tilde{f}(x) = (x^3 + 4x^2 + 8x + 4)(x^3 + 8x^2 + 16x + 16)
%    \end{equation}
%is the modular curve $X_0(22)$.
%By \todo{cite table 1}, this curve has an automorphism of order 3 when $\cha(k) = 3$, and we assume that this is the case throughout this section.
%
%We first perform an automorphism on the curve defined by \eqref{equation22modularcurve}.
%As on \todo{cite pg 150}, if $h(x) = a_nx^n + \ldots + a_0 \in k[x]$ has no repeated roots and $a_0 \neq 0$ then the curves defined by $y^2 = h(x)$ and $y^2 = h^*(x) := a_0^{-1}x^nh(1/x)$ are isomorphic.
%We let $X$ be the curve defined by 
%    \[
%    y^2 = f(x) = \tilde{f}^*(x-1) = x^6 + x^4 + x^2 + 2,
%    \]
%and it is easy to check that the map $x \mapsto x+1$ is an automorphism of $X$.
%
%In this case we can compute the elements in Proposition \todo{reference}, which are
%    \[
%    v_1 = \left( \frac{1}{yx^2}dx, \frac{x^5 + x^4 + 2x^2 + x + 2}{2y(x-1)^3}dx, \frac{x^4+2x^2}{y}dx, \frac{y}{x(x-a)^2}, \frac{y}{x}, \frac{y(x+1)}{(x-1)^2} \right)
%    \]
%and
%    \[
%    v_2 = \left( \frac{x^2+1}{2yx^3}dx, \frac{x^3+x^2+x+1}{2y(x-1)^3}dx, \frac{x^3}{2y}dx, \frac{y(x+1)}{x^2(x-1)^2}, \frac{y}{x^2}, \frac{y}{(x-1)^2} \right)
%    \]
%
%
