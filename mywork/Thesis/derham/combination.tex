...
The following lemma gives us a useful description of $H^1(X,\Omega_X)$.
    ...
    The short exact sequence
        \begin{equation}\label{equationsecondarydualitysesfunctions}
    ...
    To explain the isomorphism in \eqref{equationsheafisomorphism} we first construct a map from $\underline{K}(X)/\cO_{X,P}$ in to the product $\prod_{P \in X} i_*\left({K(X)}/\cO_{X,P}\right)$, and then show that this map has finite support.\todo[inline]{B: Not the whole map}
    ...
    Since $i_*$ and $i^{-1}$ are adjoint then the above map is an isomorphism at the level of stalks --- \ie $i_* \circ i^{-1} \colon K(X)/\cO_{X,P} \ra i_*\left( K(X)/\cO_{X,P} \right)$ is an isomorphism.
    Moreover, for any $f \in K(X)$ then $f$ lies in $\cO_{X,P}$ for all but a finite number of points $P \in X$ hence the image of $f$ in $\prod_{P \in X} i_*\left(K(X)/\cO_{X,P}\right)$ is zero in almost all factors.
    From this the isomorphism in \eqref{equationsheafisomorphism} follows.
    
    Replacing $\underline{K}(X)/\cO_X$ by $\bigoplus_{P\in X} i_*\left(K(X)/\cO_{X,P}\right)$ in \eqref{equationsecondarydualitysesfunctions} yields
    ...
    For each $P \in X$ we again have a natural injection $i \colon \{ P \} \ra X$, giving rise to the induced sheaf $i_*\left( K(X)/\cO_{X,P} \right)$ on $X$.
    We now construct an isomorphism
        ...
        \end{equation}
    similar to that in \eqref{equationsheafisomorphismdifferentials}.
    ...
    We have the following equalities
        ...
        \end{align*}
    Then for any $P \in X$ we have the map $i_* \circ i^{-1} \colon \underline{\Omega}_{K(X)}/\Omega_X \ra i_*\left( \Omega_{K(X)}/\Omega_{X,P} \right)$, which as an isomorphism of stalks.
    Finally, the image in  $\prod_{P \in X} i_*\left( \Omega_{K(X)}/\Omega_{X,P}\right)$ is zero in almost all factors, and from this we derive the isomorphism in \eqref{equationsheafisomorphismdifferentials}.
...
In addition to the assumptions in section 1, we now assume that $X$ is hyperelliptic of genus $g \geq 2$.\todo[inline]{Do we require the genus to be two?}
...
We now describe how the trace map acts on $H^1(X,\Omega_X)$ via the presentation \eqref{equationcechhoneisomorphismdifferentials}.
    ...
    This means that $\bar \omega_P$, and hence also $\psi$, is zero for any $P \in U_0 \cap U_\infty$.
...
The following lemma shows that $\derhamhone$ fits in to a short exact sequence.
    ...
    %\todo[inline]{Should this really be $f \in K(X)$ and not $f \in \cO_X(U_0 \cap U_\infty)$. Affects whether $t$ is applied to $df$ or $[df]$ below.}
    Hence $t\left([df]\right) = 0$ by Lemma \ref{lemmatracemaplemma}, and so the residue isomorphism factors through $\alpha$, and $p$ is surjective.
    \end{proof}

\section{Bases of $\hone$ and $\derhamhone$}
\subsection{basis of $\hone$}

\todo[inline]{if remove genus assumption in last section add it again here}
...
whilst if $p=2$ then the divisor of $H(x)$ is
    ...
    \end{equation}\todo[inline]{reference in other part of thesis}
...
Also, we denote the order of the zero of $F(x)$ at $\gamma_i \in k$ by $m_i \in \NN$.
    ...
    Next we recall from \cite[Prop. 7.4.26]{liu} that if $p=2$ a basis of $\hzero$ is given by $\frac{1}{H(x)}dx, \ldots, \frac{x^{g-1}}{H(x)}dx$.\todo[inline]{change reference in thesis}
    ...
    Hence the residue classes of $\frac{y}{x}|_{U_0 \cap U_\infty}, \ldots, \frac{y}{x^g}|_{U_0 \cap U_\infty}$ form a basis of $\hone$ when $p = 2$ and $\infty$ is not ramified, by Theorem \ref{theoremserreduality}.
    ...
    Since $X \backslash U_\infty = \{P_\infty\}$ it follows from the Theorem \ref{theoremresiduetheorem} the residue of $\frac{yx^{i-j}}{H(x)}dx$ at $P_\infty$ is also zero, and hence the residue classes of the elements $\left[\frac{y}{x}\right], \ldots, \left[\frac{y}{x^g}\right]$ form a basis of $\hone$, in all cases.
    \end{proof}
...
We now give a corollary to Theorem \ref{theorembasisofhone}, which is of the same style as the Mittag-Leffler theorem, but for hyperelliptic curves.
    ...
    Via a diagram chase on $\left( \gamma_1 + \ldots + \gamma_g \right) \in \hone$ we can find a corresponding element \todo[inline]{B: not precise enough, rather write $\delta$ maps ... to ...}
...
\subsection{Basis of $\derhamhone$}
...
Before proving this theorem we use it to prove the following corollary.
    ...
    We can now describe exactly how sigma acts on the elements of \eqref{equationhonebasiselementofderhampis2} using $H_{1,i}(x)$ and $H_{2,i}(x)$:
        ...
        \end{multline*}
    So the action of $\sigma$ on the basis elements in \eqref{equationhonebasiselementofderhampis2} amounts to adding 
        \[
    ...
    \begin{rem}
    We briefly study the action of $\sigma$ on the elements \eqref{equationhonebasiselementofderhampnot2} (when $p\neq 2$).
...
We now prove Theorem \ref{theorembasisofderham}.
    ...
    This yields
        ...
        \end{align*}
    using \eqref{equationdifferentialdivisor}, \eqref{equationdivisorofH} and \eqref{equationdivxpis2}.
...
In the previous section (Theorem \ref{theorembasisofderham}) we found a basis for the de Rham cohomology of any hyperelliptic curve using \cech cohomology, with respect to the cover $\cU = \{ U_0 , U_\infty\}$.
...
splitting the polynomial $(x-a)^g$ in to two parts.
    ...
    Firstly, by \eqref{equationdivxpis2} and \eqref{equationdifferentialdivisor},
        \begin{align*}
    ...
    On the other hand, by \eqref{equationdivxpis2} and \eqref{equationdifferentialdivisor},
        \begin{align*}
...
We note that for any $\tau \in \aut(X)$ the following diagram commutes,
    ...
    \end{array}
    \]
since the hyperelliptic involution $\sigma$ is in the centre of $\aut(X)$.
...
Here $\rho$ and $\rho'$ are the projections on the first, third and fifth coordinates and on the second, third and sixth coordinates respectively.
    ...
    Suppose that $\tau \in \aut(X)$ is such that the induced automporhism $\bar \tau \colon \mathbb P_k^1 \ra \mathbb P_k^1$ is given by $x \mapsto x+ a$.
...
%    \end{rem}
    ...
    Then it follows that for all $\alpha \in \hone$ then
        ...
        \end{equation}
    We will show that this equality gives rise to a contradiction when $\alpha$ is the residue class of $\frac{y}{x^g}|_{U_0 \cap U_\infty}$ in $\hone$ (see \eqref{equationcechhoneisomorphismfunctions} and Theorem \ref{theorembasisofhone}).  
    ...
    In order to do this we consider the following obvious commutative diagram of isomorphisms:
        ...
        \end{array}
        \]
    From the proof of Proposition \ref{propbasisoftriplecoverderham} we know that $\rho^{-1} \left(\left[\frac{y}{x^g}\right]\right)$ is the residue class of
        \[
        ...
        \]  \todo[inline]{State where this lives?}
    Therefore 
        \begin{align*}
    ...
    Now the action of $\tau^*$ on $\lambda$ is easily seen to be given by
        ...
        & = \left[ \frac{(x+a)^i}{y}dx, \frac{(x+a)^i}{y}dx, 0 \right] \\
    ...
    To compute $\tau^*(\gamma_g)$ we consider the following commutative of isomorphisms
        ...
        \end{array}
        \]\todo[inline]{have both $\rho$ and $\rho'$ been defined?}
...
%    used in the proof of Proposition \ref{propbasisoftriplecoverderham}.
    ...
    On the other hand, we have 
        ...
        \end{equation*}
    Since any element of the form $(\omega_0, \omega_\infty, 0)$ in the subspace \eqref{equationderhamquotient} of the space \eqref{equationderhamspace} is in fact zero, we conclude that
        \[
