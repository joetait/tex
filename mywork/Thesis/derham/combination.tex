...
Before proving this theorem we use it to prove the following corollary.
    ...
    Firstly we see that
        ...
        & = \frac{-\Psi_i(x,y)}{x^{i+1}H(x)}dx + \frac{H(x)\left(xH_{1,i}'(x) + iH_{1,i}(x)\right)}{x^{i+1}H(x)}dx\\
    ...
    \begin{rem}
    We briefly study the action of the hyperelliptic involution $\sigma$ on the elements \eqref{equationhonebasiselementofderhampnot2} (when $p\neq 2$).
...
splitting the polynomial $(x-a)^g$ in to two parts.
    ...
    Note that the second and third line are not necessarily equal, since the coefficient of $x^{i-1}$ in $r_1(x)$ may be divisible by $p$.
    On the other hand, by \eqref{equationdivxpis2} and \eqref{equationdivisorofypnot2},
        \begin{align*}
...
%    \end{rem}
    ...
    Without loss of generality we can and will assume that $\tau(y) = y$ since, if $\tau(y) = -y$, we can replace $\tau$ by $\tau \circ \sigma$ (where $\sigma$ is the hyperelliptic involution of $X$). N
    ...
    Now the action of $\tau^*$ on $\lambda_i$ for $1 \leq i \leq g$ is easily seen to be given by
        \begin{align*}
...
%    used in the proof of Proposition \ref{propbasisoftriplecoverderham}.
    Then 
        \begin{align}\label{equationactionoftauongamma}
        \tau^*(\gamma_g) & = \tau^*(\rho'(\rho^{-1}(\gamma_g))) \\  
        & = \tau^*\left( \left[ \omega_a, \frac{- \phi_g(x)}{2yx^{g+1}} dx, \frac{y}{(x-a)^g} \right) \right] \\
        & = \left[ \left( \tau^*(\omega_a) , \frac{-\phi_g(x+a)}{2y(x+a)^{g+1}}dx, \frac{y}{x^g} \right) \right],
        \end{align}
    where $\omega_a$ is the second entry in $\nu_g$, as in the proof of Proposition \ref{propbasisoftriplecoverderham}.
