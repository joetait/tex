...
We now give a method of forming curves which have an automorphism of the form described in Theorem \ref{theoremsplittingtheorem} of any genus.
    ...
    Note that if
        ...
        f(x) = h(x^p - x)
        \]
    for some polynomial $h \in k[x]$ then clearly $f(x + a) = f(x)$ for any $ a \in \mathbb F_p \subset k$.
    Such an equation can define a hyperelliptic curve of any genus if $h(x)$ has no repeated roots.
    Indeed, given any $g \geq 2$ we can fix some prime $p$ which divides $2g+1$, and if we choose $h(x)$ to be any polynomial of degree $n := (2g+1)/p$ then $f(x) = h(x^p - x)$ defines a curve of genus $g$.
    \end{ex}

We finally examine the case of genus 4.
