...
Of course, any group that acts on a curve $X$ also acts on spaces of functions and differentials of $X$, such as $\hzero$, and it is this intersection of group actions and Riemann-Roch spaces that the main focus of our study will lie.
In particular, we will consider the $k[G]$-module structure of various spaces of differentials on $X$, and related spaces, for a subgroup $G$ of the automorphism group $\aut(X)$.
Moreover, we will pay special attention to what happens in positive characteristic.
...
In the following section we begin by defining polynomials $\phi_i(x)$, $\psi_i(x)$, $\Phi_i(x,y)$ and $\Psi_i(x,y)$ for $ 1 \leq i \leq g$ in terms of  $i$, $y$, $f(x)$, $F(x)$ and $H(x)$, with which we describe a basis of $\derhamhone$ in Theorem \ref{theorembasisofderham}.
...
Given a smooth, projective curve $X$ over an algebraically closed field $k$, a significant open problem is to completely determine the $k[G]$-module structure of $\hzero$, for any subgroup $G$ of $\aut(X)$.
