...
We also assume that $g_X \geq 2$.
    ...
    \begin{rem}
    In Section \ref{sectionvalentinimadan} we show that it is possible to give an alternative, but rather involved, proof of the above lemma, using the deep and intricate results of \ref{valmadan}
    \end{rem}
...
By \cite[App.\ 5, Lem.\ 5.1]{quaddiffequi} we can choose $y$ and $a$ above such that for each $P_i$ the maximum $m_i \in \NN$ such that the ramification group $G_{m_i}(P_i)$ is non-trivial is co-prime to $p$.
From Hilbert's formula (Theorem \ref{hilbertsformula}) we can conclude that
    \[
    \delta_i = \sum_{j=0}^\infty (\ord(G_j(P_i))-1 = \sum_{j=0}^{m}(p-1) = (p-1)(m+1).
    \]
In particular, the sum after the second equality is to $m$ since $G_j(P_i)$ is trivial for $j > m$, and hence $\ord(G_j(P_i)) -1 = 0$ for such $j$.
Now we set
    \[
    \gamma_{i,k} = \left\lfloor \frac{\delta_i - k m}{p} \right\rfloor, \qquad k = 0, \ldots, p-1,
    \]
...
Using the above theorem we now give an alternative proof of Lemma \ref{m=1}.
    ...
    \begin{proof}[Proof of Lemma \ref{m=1}]
