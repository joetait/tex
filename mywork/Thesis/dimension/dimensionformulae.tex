...
By \cite[App.\ 5, Lem.\ 5.1]{quaddiffequi} we can choose $y$ and $a$ above so that for each $P_i$ the maximum $m_i \in \NN$ for which the ramification group $G_{m_i}(P_i)$ is non-trivial is co-prime to $p$.
...
We now state the main theorem of \cite{valmadan}.
    ...
    The indecomposable representation $M_{p-1}$ occurs $\Gamma_{p-2} - \Gamma_{p-1} - 1$ times, whilst for $k=1, \ldots, p-2$, the indecomposable representation $M_k$ of degree $k$ occurs $\Gamma_{k-1} - \Gamma_k + \alpha_k$ times, with $\alpha_k = -1$ if $k = p-1$, and zero otherwise.
    \end{thm}
    \begin{proof}
...
Using the above theorem we now give an alternative proof of Lemma \ref{m=1}.
    ...
    Since
        ...
        \]
    we it follows that $m_i \neq 0$ for exactly one $i$, and that we have $1 \leq m_i \leq p$.
    Furthermore, since we chose all $m_i$ to be coprime to $p$, we actually have $1 \leq m_i \leq p-1$.
    Note that $s=1$, since if the only non-trivial ramification group of $P \in X$ is $G_0(P)$ then $P$ is not ramified, and we let $m = m_1$ and $\delta = \delta_1$.
    ...
    On other hand, from the Hurwitz Formula (Corollary \ref{corhurwitzformula}) and Hilbert's Formula (Theorem \ref{hilbertsformula}), we can conclude that
        \begin{align*}
        2g_X - 2 = -2p + (p-1)(m+1),\\
        \intertext{which we can rearrange to}
        g_X(p-3) = 0.
        \end{align*}
