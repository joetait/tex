...
Now if we let $\alpha_i = -v_i(a) = m$\todo[inline]{why are they all equal to $m$? I think it is because the valuation is being divided by the ramification index, which is $p$ in this case. Need to check} then we can compute the exponent of the different \todo[inline]{check terminology} at $P_i$, which is
    \[
    \delta_i = (p-1)(\alpha_i + 1) = (p-1)(m+1).
    \]\todo[inline]{check that this is clear}
Now we set
    \[
    \gamma_{i,k} = \left\lfloor \frac{\delta_i - k \alpha_i}{p} \right\rfloor, \qquad k = 0, \ldots, p-1,
    \]\todo[inline]{make clear that and why this is the coefficient of the ramification divisor}
where $\lfloor c \rfloor$ denotes the largest integer less than $c$, for any $c \in \mathbb R$.
Then $\gamma_{i,k}$ is the coefficient of $P_i$ in $R$ the ramification divisor. \todo[inline]{give references for this}
...
Using the above theorem we now give an alternative proof of Lemma \todo[inline]{citation, here and below}.

    \begin{proof}[Proof of Lemma]
    Note that $g_Y= 0$ by assumption.\todo[inline]{check this}
    ...
    Now since we are assuming that the action of $G$ is trivial, it must follow that $d_k = 0$ for all $k \neq 1$.\todo[inline]{check this is already mentioned}
    By Theorem \todo[inline]{citation} it follows that $\Gamma_{p-2} - \Gamma_{p-1} -1 = 0$, and hence $\Gamma_{p-2} = 1$.
    We can then show inductively that for $2 \leq k \leq p-2$ then $\Gamma_k = 1$, using the relation $\Gamma_{k-1} - \Gamma_k = 0$ from theorem \todo[inline]{citation}.
    ...
    Combining this with \eqref{equationdifferenceofgamma0andgamma1} and the action being trivial \todo[inline]{is this second part actually needed?} we conclude that
        \[
        1 = - \left \lfloor \frac{\alpha_i}{p} \right \rfloor = \Gamma_0 - \Gamma_1 = g_X.
        \]  \todo[inline]{why is this equal to $g_X$?}
