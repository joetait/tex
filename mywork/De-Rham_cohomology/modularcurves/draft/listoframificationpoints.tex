...
\]
\todo[inline]{show that the relations hold}
...
It is clear that $df_{0\infty} - df_{0 a} + df_{a \infty} = 0$.\todo{explain more}.
It is clear that the functions are regular on the correct open sets. \todo{show this}
This just leaves us to check that $--- : = \omega_{0} - df_{a \infty}$ is regular on $U_a$.
We first compute $df_{0 a }$ as follows
\begin{align}
df_{0 a} & = d \left( \frac{y\left( \sum_{i=0}^{k-1}(-1)^{g-i}\binom{g}{i}a^{g-i}x^i \right)}{x^k(x-a)^g} \\
& = \frac{f'}{2y} \left( \frac{\sum_{i=0}^{k-1}(-1)^{g-i}\binom){g}{i}x^i\right)}{x^k)x-a)^g} + \frac{ y \left( \sum_{i=0}^{k-1}(-1)^{g-i}i \binom{g}{i}a^{g-i}x^{i-1}\right)}{x^k(x-a)^g} + \frac{y\left(\sum_{i=0}^{k-1}(-1)^{g-i}\binom{g}{i}a^{g-i}x^i\right)(-gx-k(x-a))}{x^{k+1}(x-a)^{g+1}} 
\end{align}
...
\end{align}
\todo[inline]{Give binomial expansion a name. Use it to shorten statements.}
\end{proof}
\bibliography{biblio}
