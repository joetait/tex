...
\begin{lem}
The following is a legitimate element of $\cechderhamhone(\cU '')$:
\[
\left(  \frac{x^3 + x^2 + 1}{yx^2}dx, \frac{x^3}{2y}dx, \frac{1}{2y(x-1)^3} \cdot (2x^3 - x^2 - x -1 )dx, \frac{y}{x^2}, \left(\frac{y}{x^2} - \frac{y}{(x-1)^2} \right) dx, \frac{-y}{(x-1)^2} \right).
\]
\end{lem}
\begin{proof}
We start by checking that each element is regular on the appropriate open set (recall that $U_1 = X \backslash \pi^{-1}(0), U_2=X \backslash \pi^{-1}(\infty) $ and $U_3 = X \backslash \pi^{-1}(1)$.
...
\[
\di_0(x^3+x^2+x+1) - 3D_\infty - R + 3D_\infty + 3D_\infty - 3D_1 -2D_\infty + R = \d_0(x^3+x^2+x+1) + D_\infty - 3D_1.
\]
...
Next we check that $-\left(\frac{y}{x^2} - \frac{y}{(x-1)^2}$ is regular on $U_1 \cap U_3$ by computing the divisor of its square, which is
\[
...
\]
Since the divisor of $-\left(\frac{y}{x^2} - \frac{y}{(x-1)^2}$ is half of the divisor above, we see it is regular on the desired set.
...
\begin{align*}
\left( \frac{y}{x} - \frac{y(x-2)}{(x-1)^2} \right)^2 & = f \left( \frac{1}{x^2} - \frac{(x-2)^2}{(x-1)^4} \right) - \frac{2(x-2)}{(x-1)^2x} \\
& = frac{f}{x^2(x-1)^4}.
\end{align*}
