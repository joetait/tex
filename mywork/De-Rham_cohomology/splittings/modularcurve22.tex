...
We observe that since $r_{i,g}+t_{i,g}$ is just the binary expansion of $(x-a)^g$ then
\begin{align*}
...
Firstly
\begin{align*}
\frac{r_{i,g}y}{x^i(x-a)^g} & = \di(r_{i,g}) + \di(y) - i\di(x) - g\di(x-a) \\
...
On the other hand
\begin{align*}
\frac{t_{i,g}y}{x^i(x-a)^g} & = \di\left(\frac{t_{i,g}}{x^i}\right) + \di(y) - g\di(x-a) \\
...
\end{align*}
Since $t_{i,g}/x^i$ is regular on $U_\infty$ we conclude that $\di_0\left( \frac{t_{i,g}}{x^i} \right)$ is non-negative, and hence $({t_{i,g}y})/({x^i(x-a)^g})$ is regular on $U_a \cap U_\infty$.
...
If $\deg(f) = 2g+1$, then the degree of both $\phi_i(x)r_{i,g}(x-a)$ and $2if(-1)^{g-i+1}\binom{g}{i} a^{g-i+1}x^i$ is clearly at most $2g+1+i$.
...
and
\[
\psi_i(x)t_{i,g}(x-a) - a\cdot(-2ia_0)\cdot \left( (-1)^{g-i}\binom{g}{i}a^{g-i}x^i \right)
\]
