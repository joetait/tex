...
\[
r_{i,j} : = \sum_{k=0}^{i-1} (-1)^{j-k}\binom{j}{k} a^{j-k} x^k
\]
and
\[
t_{i,j} := \sum_{k=i}^{j} (-1)^{j-k}\binom{j}{k} a^{j-k} x^k.
\]
...
\di_0 \left( \frac{t_{i,g}}{x^i} \right) +R - gD_a -(g+1-i)D_\infty.
\end{align*}
...
& = \frac{(xf'r_{i,g}(x-a) - 2if(x-a)r_{i,g}) + 2ixf(r_{i,g}'(x-a)-gr_{i,g})}{2yx^{i+1}(x-a)^{g+1}}dx.
\end{align*}
We first show that $r_{i,g}'-gr_{i,g}$ is a monomial of degree $i-1$. 
It will then follow that $\frac{2ixf(r_{i,g}' - gr_{i,g}}{2yx^{i+1}(x-a)^{g+1}}dx$ can only have poles at $\pi^{-1}{a}$ and $\pi^{-1}(\infty)$.
Since 
\begin{align*}
r_{i,g}' & = \sum_{k=0}^{i-1} k (-1)^{g-k} \binom{g}{k} a^{g-k} x^{k-1} \\
& = \sum_{k=0}^{i-2} (k+1) (-1)^{g-k-1} \binom{g}{k+1}a^{g-k-1} x^k
\end{align*}
it follows that
\begin{align*}
r_{i,g}'(x-a) & = x \sum_{k=0}^{i-1} k (-1)^{g-k} \binom{g}{k} a^{g-k} x^{k-1} - a \sum_{k=0}^{i-2} (k+1) (-1)^{g-k-1} \binom{g}{k+1}a^{g-k-1} x^k \\
& = \sum_{k=0}^{i-1} k (-1)^{g-k} \binom{g}{k} a^{g-k} x^k  + \sum_{k=0}^{i-2} (k+1) (-1)^{g-k} \binom{g}{k+1}a^{g-k} x^k,
\end{align*}
and so the coefficient of $x^k$ is $(-1)^{g-k}\left(k\binom{g}{k} + (k+1)\binom{g}{k+1}\right) a^{g-k}$ for $0 \leq k \leq i-2$.
