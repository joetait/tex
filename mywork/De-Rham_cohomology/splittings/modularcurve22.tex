...
\begin{equation*}
\omega_{0 1} - \omega_{\sigma 1}  = \frac{1}{2yx^2(x-a)^3} \left( (a_3x^3 -a_1x-2a_0)(x-a)^3 - x^2\r_1\right )dx,
\end{equation*}
...
On the other hand
\begin{equation*}
\omega_{0 2} - \omega_{\sigma 2} = \frac{1}{2yx^3(x-a)^3}\left( -(a_3x^3+2a_2x^2+3a_1x+4a_0)(x-a)^3 + x^3\r_2 \right) dx,
\end{equation*}
...
Hence
\begin{multline*}
\sigma(\r_1) = \\ \frac{(4a^3a_6+a^2a_5+a_3)x^4+(6a^4a_6+2a^3a_5+aa_3)x^3+(2a^2a_2-a_1)x^2+(3aa_1-2a_0)x+4aa_0}{2yx^3}dx.
\end{multline*}
We also have
...
We compute the coefficient of $\omega_{\infty}$\todo{check that this is defined} in $\sigma(\rho'(\nu_1)) - \gamma_3 + a\gamma_4$, which is
\begin{multline*}
...
We now use this to compute the $d_1$ and $d_2$ above, by computing the coefficient of $\omega_\infty$\todo{As above, check this is defined} of $\sigma(\rho'(\nu_1)) - \gamma_4$, as follows
\begin{multline*}
...
\end{equation*}
splits if $\deg(f) = 5$.\todo{equivalently, if $\cha (k) = 5$, other equivalent conditions.}
\end{prop}
\begin{rem}
Note that $\deg(f)=5$ is equivalent to saying that $\cha(k)=5$.
\end{rem}
\begin{proof}
