...
We let $X$ be a genus $2$ curve over an algebraically closed field $k$ (characteristic not 2).
Since any genus two curve is hyperelliptic, we may suppose that $X$ is defined by 
\begin{equation}\label{definingequation}
y^2 = f(x) = a_6x^6 + a_5x^5 + a_4x^4 + a_3x^3 + a_2x^2 + a_1x + a_0 \in k[x]
\end{equation}
...
\begin{multline*}
\nu_1 = \left(\frac{a_3x^3-a_1x-2a_0}{2yx^2}dx, \frac{r_1}{2y(x-a)^3}dx, \frac{-(4a_6x^4+3a_5x^3+2a_4x^2)}{2y}dx,  \right. \\ \left. \frac{a^2y}{x(x-a)^2}, \frac{y}{x},  \frac{y(x-2a)}{(x-a)^2} \right)
\end{multline*}
and $\rho^{-1}(\gamma_4)$ is the residue class of
\begin{multline*}
\nu_2 = \left( \frac{-(a_3x^3+2a_2x^2+3a_1x+4a_0)}{2yx^3},  \frac{r_2}{2y(x-a)^3}dx, \frac{-(2a_6x^3+a_5x^2)}{2y}dx, \right. \\ \left.  \frac{y(a^2-2ax)}{x^2(x-a)^2},\frac{y}{x^2}, \frac{y}{(x-a)^2} \right),
\end{multline*}
where
\begin{multline*}
r_1  := (4a^3a_6+a^2a_5+a_3)x^4 + (3a^3a_5+2a^2a_4-3aa_3)x^3 \\
 + (2a^3a_4 + 6a^2a_3 - a_1)x^2 + (4a^2a_2 +3aa_1-2a_0)x+(2a^2a_1+6aa_0)
\end{multline*}
and
\begin{multline*}
r_2  := (-6a^2a_6-2aa_5)x^4 + (2a^3a_6-3a^2a_5-4aa_4-a_3)x^3 \\ + (a^3a_5-3aa_3-2a_2)x^2 + (-2a_2a-3a_1)x +(-aa_1-4a_0).
\end{multline*}
\end{prop}
\begin{proof}
...
When $i=1$ we have
\begin{align*}
f_{0 \sigma i} - f_{0 \infty i}+ f_{\sigma \infty i} & = \frac{a^2y}{x(x-a)^2} -\frac{y}{x} + \frac{y(x-2a)}{(x-a)^2} \\
...
Secondly, the divisor of  $f_{\sigma \infty 1}$ is
\begin{align*}
...
\end{align*}
and the divisor of $f_{\sigma \infty 2}$ is
\begin{align*}
...
& = R - 2D_a - D_\infty,
\end{align*}
both of which are clearly positive on $U_\sigma \cap U_\infty$.

Now we show that for $i = 1, 2$ the differential $\omega_{\sigma i}$ is regular on $U_\sigma$.
In both cases it is clear that there can only be poles at $a$ and $\pi^{-1}(\infty)$.
Since $a$ is not in $U_\sigma$, we need only check that there are no poles in $\pi^{-1}(\infty)$, or equivalently that the coefficient of $D_\infty$ in $\di( \omega_{\sigma i})$ is non-negative.
Since $\deg(r_i) = 4$ for both $i=1$ and $i=2$ it follows that when $i=1$ this coefficient is $-4 - 4 + 3 + 3 = 0$ and when $i=2$ the coefficient is $-4 -2 +3 + 3=0$.
...
\begin{equation*}
\omega_{0 1} - \omega_{\sigma 1}  = \frac{1}{2yx^2(x-a)^3} \left( (a_3x^3 -a_1x-2a_0)(x-a)^3 - x^2r_1\right )dx,
\end{equation*}
...
On the other hand
\begin{equation*}
\omega_{0 2} - \omega_{\sigma 2} = \frac{1}{2yx^3(x-a)^3}\left( -(a_3x^3+2a_2x^2+3a_1x+4a_0)(x-a)^3 + x^3r_2 \right) dx,
\end{equation*}
...
\end{equation*}
We use this to prove the following proposition regarding the action of $\sigma^*$ on $\cechderhamhone(\cU)$.

\begin{prop}
We have the following actions of $\sigma^*$ on the basis elements of $\cechderhamhone(\cU)$:
\begin{align*}
...
\begin{align*}
\sigma^*( \omega_{\sigma}) & = \sigma^* \left( \frac{r_1}{2y(x-a)^3} dx \right) \\
& = \frac{\sigma^*(r_1)}{2yx^3}dx.
\end{align*}
Now we compute $\sigma^*(r_1)$, which is
\begin{align*}
\sigma^*(r_1) & = (4a^3a_6+a^2a_5+a_3)x^4 + (16a^4a_6+7a^3a_5+2a^2a_4+aa_3)x^3 \\
...
\end{enumerate}
Hence the constant coefficient of $\sigma^* (r_1)$ simplifies to
\begin{equation*}
...
Hence
\begin{multline*}
\sigma^*(r_1) = \\ \frac{(4a^3a_6+a^2a_5+a_3)x^4+(2a^3a_5+aa_3)x^3+(2a^2a_2-a_1)x^2+(3aa_1-2a_0)x+4aa_0}{2yx^3}dx.
\end{multline*}
We also have
...
\begin{align*}
\sigma^*(r_2) & = -(6a^2a_6+2aa_5)x^4 - (22a^3a_6+11a^2a_5+4aa_4+a_3)x^3 \\
...
by \ref{6} the constant coefficient of $\sigma^*(r_2)$ is
\[
...
\end{align*}
We also define the following polynomials for $j \geq i$
\[
r_{i,j} : = \sum_{k=0}^{i} (-1)^{j-k}\binom{j}{k} a^{j-k} x^k
\]
and
\[
t_{i,j} := \sum_{k=i+1}^{j} (-1)^{j-k}\binom{j}{k} a^{j-k} x^k.
\]
We will mostly consider these polynomials when $j=g$, when they will split the polynomial $(x-a)^g$ in to two parts.
We now present a basis of $\cechderhamhone (\cU'')$.

\begin{prop}\label{basis22}
The pre-image $\rho^{-1}(\tau_i)$ is the residue classes of
\begin{equation*}
\nu_i = \left(\frac{\psi_i(x)}{2yx^{i+1}}dx, ---, \frac{-\phi_i(x)}{2yx^{i+1}}dx,  \frac{r_{i,g}y}{x^i(x-a)^g}, \frac{y}{x^i},  \frac{t_{i,g}y}{x^i(x-a)^g} \right).
\end{equation*}
\end{prop}
\begin{proof}
...
Firstly, since $r_{i,g}+t_{i,g}$ is the binary expansion of $(x-a)^g$ then
\begin{align*}
f_{0 \sigma i} - f_{0 \infty i}+ f_{\sigma \infty i} & = \frac{r_{i,g}y}{x^i(x-a)^g} - \frac{y}{x^i} + \frac{t_{i,g}y}{x^i(x-a)^g} \\
& = \frac{y(r_{i,g} + t_{i,g} - (x-a)^g)}{x^i(x-a)^g} \\
& = 0.
\end{align*}
...
Firstly
\begin{align*}
\di \left( f_{0 \sigma i} \right) & = \di \left( \frac{r_{i,g}y}{x^i(x-a)^g} \right) \\
&  = \di(r_{i,g}) + \di(y) - i\di(x) - g\di(x-a) \\
& = \di_0(r_{i,g}) - (i-1)D_\infty +R - (g+1)D_\infty - iD_0 + iD_\infty - gD_a + gD_\infty \\
& = \di_0(f_{i,g}) +R -iD_0 - gD_a + D_\infty,
\end{align*}
which is regular on $U_0 \cap U_\sigma$.
On the other hand
\begin{align*}
\di \left( f_{\sigma \infty i} \right) & = \di \left( \frac{t_{i,g}y}{x^i(x-a)^g} \right) \\
& = \di\left(\frac{t_{i,g}}{x^i}\right) + \di(y) - g\di(x-a) \\
& = \di_0 \left( \frac{t_{i,g}}{x^i} \right) - (g-i)D_\infty +R - (g+1)D_\infty - gD_a + gD_\infty\\
& = \di_0 \left( \frac{t_{i,g}}{x^i} \right) +R - gD_a -(g-i+1)D_\infty.
\end{align*}
Since $t_{i,g}/x^i$ is regular on $U_\infty$ we conclude that $({t_{i,g}y})/({x^i(x-a)^g})$ is regular on $U_a \cap U_\infty$.

To show that
\begin{equation}\label{longequation}
\omega_{\sigma i} =  \frac{(\psi_i(x)t_{i,g} - \phi_i(x)r_{i,g})(x-a) - 2if(-1)^{g-i+1}\binom{g}{i} a^{g-i+1}x^i}{2yx^{i+1}(x-a)^{g+1}}dx
\end{equation}
...
{(\psi_i(x)t_{i,g} - \phi_i(x)r_{i,g})(x-a) - 2if(-1)^{g-i+1}\binom{g}{i} a^{g-i+1}x^i}
\end{equation}\todo{should be plus $-2if\ldots$?}
...
The order of $\psi_i(x)t_{i,g}(x-a)$ is at most $2g+2$, which is less than $2g+2+i$ for all $i \geq 1$.
If $\deg(f) = 2g+1$, then clearly
\[
\deg\left( \phi_i(x)r_{i,g}(x-a) \right) = \deg(\phi_i) + \deg(r_{i,g}) + \deg((x-a)) \leq 2g+1 i-1 +1 = 2g+i+1
\]
and
\[
...
\end{cases}
\]
Lastly, if $\deg(f) = 2g+2$ then the leading term of $-\phi_i(x)r_{i,g}(x-a)$ is
\begin{align*}
...
Since these terms cancel, we again have the that the degree of the numerator of \eqref{longequation} is at most $2g+i+1$, and \eqref{longequation} has no pole at the point(s) in $\pi^{-1}(\infty)$.

Finally, we show that the numerator of \eqref{longequation} is divisible by $x^{i+1}$.
Note that
\[
...
and
\[
\psi_i(x)t_{i,g}(x-a) - a(-2ia_0)\left( (-1)^{g-i}\binom{g}{i}a^{g-i}x^i \right)
\]
are both divisible by $x^{i+1}$.
Moreover, 
\[
a\cdot(-2ia_0)\cdot \left( (-1)^{g-i}\binom{g}{i}a^{g-i}x^i \right) = 2ia_0(-1)^{g-i+1}\binom{g}{i}a^{g-i+1}x^i,
\]
so the terms cancel and we see that the numerator is divisible by $x^{i+1}$.


Finally, we show that $\omega_{\sigma i} = \omega_{0 i} -df_{0 \sigma i}$.
We begin this by computing $df_{0 \sigma i}$, which is
\begin{align*}
df_{0 \sigma i} & = d \left( \frac{y r_{i,g}}{x^i(x-a)^g} \right) \\
& = \frac{r_{i,g}}{x^i(x-a)^g}dy + y d\left( \frac{r_{i,g}}{x^i(x-a)^g} \right) \\
& = \frac{f'r_{i,g}}{2yx^i(x-a)^g}dx + \left( \frac{yr_{i,g}'}{x^i(x-a)^g} -\frac{iy r_{i,g}}{x^{i+1}(x-a)^g} - \frac{gyr_{i,g}}{x^i(x-a)^{g+1}}\right) dx \\
& = \frac{xf'r_{i,g}(x-a) + 2f(xr_{i,g}'(x-a) - i(x-a)r_{i,g} - gxr_{i,g})}{2yx^{i+1}(x-a)^{g+1}} dx.
\end{align*}
Hence $\omega_{0 i} - df_{o \sigma i}$ expands to
\[
\frac{\psi_i(x)(x-a)^{g+1} - xf'r_{i,g}(x-a) - 2f(xr_{i,g}'(x-a)-i(x-a)r_{i,g}-gxr_{i,g})}{2yx^{i+1}(x-a)^{g+1}}.dx
\]
Now
\[
(x-a)^{g+1} = (x-a)^g(x-a)  = (r_{i,g} + t_{i,g})(x-a)
\]
and
\[
xf'r_{i,g}(x-a) - 2ifr_{i,g}(x-a) = r_{i,g}(x-a)(xf'-2if) = r_{i,g}(x-a)(\psi_i(x) + \phi_i(x)).
\]
So
\[
\psi_i(x)(x-a)^{g+1} - xf'r_{i,g}(x-a) - 2ifr_{i,g}(x-a) = (\psi_i(x)t_{i,g} - \phi_i(x) r_{i,g})(x-a).
\]


We now compute $r_{i,g}'-gr_{i,g}$.
Since 
\begin{align*}
r_{i,g}' & = \sum_{k=0}^i k (-1)^{g-k} \binom{g}{k} a^{g-k} x^{k-1} \\
& = \sum_{k=0}^{i-1} (k+1) (-1)^{g-k-1} \binom{g}{k+1}a^{g-k-1} x^k
\end{align*}
it follows that
\begin{align*}
r_{i,g}'(x-a) & = x \sum_{k=0}^i k (-1)^{g-k} \binom{g}{k} a^{g-k} x^{k-1} - a \sum_{k=0}^{i-1} (k+1) (-1)^{g-k-1} \binom{g}{k+1}a^{g-k-1} x^k \\
...
On the other hand, the coefficient of $x^k$ in $gr_{i,g}$ is $(-1)^{g-k}g\binom{g}{k}a^{g-k}$.
Since 
\begin{align*}
...
\[
\omega_{0 i } - df_{0\sigma i} =  \frac{(\psi_i(x)t_{i,g} - \phi_i(x)r_{i,g})(x-a) - 2if(-1)^{g-i+1}\binom{g}{i} a^{g-i+1}x^i}{2yx^{i+1}(x-a)^{g+1}}.
\]
