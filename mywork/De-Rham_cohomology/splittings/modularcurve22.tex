...
We now show that $r_{i,g}'-gr_{i,g}$ is a monomial of degree $i$. 
It will then follow that $\frac{2ixf(r_{i,g}'(x-a) - gr_{i,g}}{2yx^{i+1}(x-a)^{g+1}}dx$ is regular on $U_\sigma cap U_\infty$.
Since 
\begin{align*}
...
& = g \binom{g}{k}
\end{align*}
it follows that $r_{i,g}' (x-a) - gr_{i,g}$ is divisible by $x^i$, and since $\deg(r_{i,g}'(x-a) - gr_{i,g}) \leq i$ we know that $r_{i,g}'(x-a) - gr_{i,g}$ we know it is in fact a monomial of order $i$.
In particular, $ r_{i,g}'(x-a) - gr_{i,g} = (g+i-1)(-1)^{g-i+1}\binom{g}{i-1}a^{g-i+1}x^i$, and hence
\begin{align*}
\frac{2ixf(r_{i,g}'(x-a)-gr_{i,g})}{2yx^{i+1}(x-a)^{g+1}}dx & = \frac{2if(g+i-1)(-1)^{g-i+1}\binom{g}{i-1}a^{g-i+1}x^{i+1}}{2yx^{i+1}(x-a)^{g+1}} dx \\
& = \frac{2if(g+i-1)(-1)^{g-i+1}\binom{g}{i-1}a^{g-i+1}}{2y(x-a)^{g+1}} dx,
\end{align*}
which is clearly regular on $U_\sigma \cap U_\infty$.
