...
& = \di_0(r_{i,g}) - iD_\infty +R - (g+1)D_\infty - iD_0 + iD_\infty - gD_a + gD_\infty \\
...
& = \di_0 \left( \frac{t_{i,g}}{x^{i+1}} \right) - (g-i-1)D_\infty +R - (g+1)D_\infty - gD_a + gD_\infty\\
\di_0 \left( \frac{t_{i,g}}{x^i} \right) +R - gD_a -(g-i)D_\infty.
\end{align*}
Since $\frac{t_{i,g}}{x^i}$ is regular on $U_\infty$ we conclude that $\di_0\left( \frac{t_{i,g}}{x^i} \right)$ is non-negative, and hence $\frac{t_{i,g}y}{x^i(x-a)^g}$ is regular on $U_a \cap U_\infty$.


We now show that $\omega_{0 i} - df_{0 \sigma i}$ is regular on $U_\sigma$.\todo{check whether I should be using $U_\sigma$ or $U_a$}
We begin by computing $df_{0 \sigma i}$, as follows:\todo{add ``$(x)$'' to $r_{ij,}$ and $t_{i,j}$ terms}
\begin{align*}
...
\[
\frac{\psi_i(x)(x-a)^{g+1} - xf'r_{i,g}(x-a) - 2f(xr_{i,g}'(x-a)-i(x-a)r_{i,g}-gxr_{i,g})}{2yx^{i+1}(x-a)^{g+1}}.
\]
Now
\[
...
This term is
\begin{align*}
(i-1)(-1)^{g-i+1}\binom{g}{i-1}a^{g-i+1}x^i - g(-1)^{g-i+1}\binom{g}{i-1}a^{g-i+1}x^i & = (-1)^{g-i+1}\left( (i-1-g)\frac{g!}{(i-1)!(g-i+1)!} \right) a^{g-i+1}x^i \\
...
\[
\omega_{0 i } - df_{0\sigma i} =  \frac{(\psi_i(x)t_{i,g} - \phi_i(x)r_{i,g})(x-a) - 2f(-1)^{g-i+1}\binom{g}{i} a^{g-i+1}x^i}{2yx^{i+1}(x-a)^{g+1}}.
\]
The last observation we make is that there are only two terms that have degree less $i+1$, namely the smallest terms in $\psi_i(x)t_{i,g})(x-a)$, which is 
\[
-2ia_0\cdot (-1)^{g+i}\binom{g}{i}a^{g-i}x^i\cdot (-a) = 2ia_0(-1)^{g+i+2}\binom{g}{i}a^{g-i+1}x^i,
\]
 and the smallest term in $- 2f(-1)^{g-i+2}\binom{g}{i} a^{g-i+1}x^i}{2yx^{i+1}(x-a)^{g+1}$, which is $-2a_0\cdot (-1)^{g-i+1}i \binom{g}{i}a^{g-i+1}x^i$.
These terms clearly cancel, and we see that the whole numerator is divisible by $x^{i+1}$.

It only remains to show that there is no pole at infinity.
