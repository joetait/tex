...
We fix a degree two map $\phi \colon X \ra \mathbb{P}^1_k$, which is unique up to isomorphism of $\mathbb{P}_k^1$.
...
We suppose that $X$ has an automorphism $\sigma \colon X \ra X$, such that the action of $\sigma^*$ on $x$ in the function field $K(X) = k(x,y)$ is $\sigma^* \colon x \to x+a$ for some $a\in k$.
...
\begin{align*}
\lambda_i  = & \left( \frac{x^i}{y}dx, \frac{x^i}{y}dx, 0\right) , i=0, \ldots, g-1 \\
\intertext{and}
\gamma_i = & \left ( \frac{\psi_i(x)}{2yx^{i+1}}, \frac{-\phi_i(x)}{2yx^{i+1}}, x^{-i}y \right), i=1,\ldots ,g.
\end{align*}
...
The pre-image $\rho^{-1}(\gamma_i)$ is the set of residue classes of
\begin{multline*}
...
\[
df_{\sigma \infty i} = df_{0 \infty i} - df_{0 \sigma i} = \omega_{0 i} - \omega{_\infty i } - \omega_{0 i} + \omega_{\sigma i} = \omega_{\sigma i} - \sigma_{\infty i}.
\]

\end{proof}


We now describe the action of $\sigma^*$ on $\lambda_i$ in the following lemma.
\begin{lem}
