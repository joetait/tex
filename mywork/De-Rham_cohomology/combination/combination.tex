...
\end{equation}
where $(f\omega)_P := f_p\omega_p$ is the product of the germ of $\omega$ in $\Omega_{X,P}$ with the germ of $f$ in $\underline{K(X)}_{P}$.
...
\todo[inline]{check to see what is repeated in the first paragrpah from before}
...
We can assume, without loss of generality, that $f(x)$ is monic.
...
On the other hand, the lowest degree terms of $2if(-1)^{g-i+1}\binom{g}{i}a^{g-i+1}x^i$ and $\psi_i(x)t_i(x)(x-a)$ which can be non-zero are, respectively,
\[
...
\]
it follows that these terms cancel when adding $\psi_i(x)t_i(x)(x-a)$ and $2if(-1)^{g-i+1}\binom{g}{i}a^{g-i+1}x^i$.
