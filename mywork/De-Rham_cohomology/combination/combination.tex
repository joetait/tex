...
Then there exist unique $\alpha_1, \ldots, \alpha_n \in k$ such that after replacing $f_P$ by $f_P - \left( \frac{\alpha_1 x}{y} + \ldots + \frac{\alpha_g x^g}{y}\right)$ for $P \in \pi^{-1}(\infty)$, we can find an $f \in K(X)$ such that the laurent tail of $f$ at $P$ in $X$ is $f_P$.
\end{cor}
\begin{proof}
Since $f_P = 0$ for almost all $P \in X$ then $(f_P)_{P \in X} \in \bigoplus_{P \in X} K(X)/\cO_{X,P}$.
From Lemma \ref{exactsequencelemma} we have
\begin{equation*}
0 \ra H^0(X,\cO_X) \ra K(X) \ra \bigoplus_{P \in X} K(X)/\cO_{X,P} \xrightarrow{\delta} \hone \ra 0.
\end{equation*}
...
By the exactness of \eqref{dualitysesfunctions} it then follows that there exists an $f \in K(X)$ such that the image of $f$ in $\bigoplus_{P \in X} K(X)/\cO_{X,P}$ is $(f_P)_{P \in X}$, and hence $f$ has $f_P$ as a laurent tail at each $P \in X$.
\end{proof}
