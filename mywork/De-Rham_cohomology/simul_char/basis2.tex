...
\end{align*}
It follows that for each $P \in X$ we can map $f \in \underline{\Omega}_{K(X)}/\Omega_X$ to $i_*i^{-1}(f) \in \left( \Omega_{K(X)}/\Omega_{X,P}$.
Recall that for any $f \in \Omega_{K(X)}$then $f$ lies in $\Omega_{X,P}$ for all but a finite number of points $P \in X$.
Hence the image of $f$ in $\Prod_{P \in X} i_*\left(\Omega_{K(X)}/\Omega_{X,P}$ is zero in almost all factors.
