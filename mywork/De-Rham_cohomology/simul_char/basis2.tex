...
Since $\Omega_{X,P} \subseteq \ker (\res_P)$, it follows that $\res_P$ is well defined on the quotient $\Omega_{K(X)}/\Omega_{X,P}$.
...
\[
\bigoplus_{P \in X} \Omega_K(X)/\Omega_{X,P} \ra k, \quad (\omega_P)_{P \in X} \mapsto \sum_{P\in X} \res_P(\omega_P)
\]
vanishes on the image of $\Omega_{K(X)}$.
It follows that given some $\overline{(\omega_P)_{P \in X}} \in H^1(X,\Omega_X)$, which is mapped to by some $(\omega_P)_{P \in X} \in \bigoplus_{P \in X} \Omega_K(X)/\Omega_{X,P}$, we can define the trace map $t$:
\[
...
\end{equation}
where $(f\omega)_P := f_p\omega_p$ is just the product of the germ of $\omega$ in $\Omega_{X,P}$ with $f_P$.
