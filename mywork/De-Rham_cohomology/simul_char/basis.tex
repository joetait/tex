...
We can extend this embedding to the restrictions of elements of $K(X)$.
Let $f \in K(X)$ and $U \subseteq X$.
Then let $r_P = f$ if $P \in U$, and $r_P = 1$ otherwise.
...
Since the differential has no other poles on $U_0$ it follows that $\sum_{P \in U_0} \res_P(x^{-1}dx) = \res_{P_\infty}(x^{-1}dx) \neq 0$.
As described in the paragraph preceding this lemma, the map $ \langle , \rangle \colon \hzero \times \hone \ra k$ is a dual map.
...
On the other hand, if $\infty$ is not a branch point then we see from \eqref{residuedivisor} that $x^{-1}dx$ has poles at both $P_\infty$ and $P_\infty'$.
Hence $\sum_{P \in U_0}\res_P(x^{-1}dx) = \res_{P_\infty}(x^{-1}dx) + \res_{P_\infty'}(x^{-1}dx)$.
Since we assumed $p=2$ it is sufficient to show that $\res_{P_\infty}(x^{-1}dx) = \res_{P_\infty'}(x^{-1}dx)$ in order to prove that $\langle x^iy^{-1}dx, yx^{-(i+1)}|_{U_0} \rangle \neq 0$, and hence that $\frac{y}{x^j}$, $1 \leq j \leq g$, is a dual basis of $\frac{x^i}{y}dx$, $0 \leq i \leq g-1$.

We can easily show this using the hyperelliptic involution $\sigma$.
Recall that \todo{finish this}
...
So we see that there is a pole of order one at $P_\infty$ if $2j - 3 - 2i = -1$, and hence if $j = i+1$.
...
This shows that the elements \eqref{one} satisfy the condition of \eqref{derhamconditions} that $df_{0, \infty} = \omega_0 - \omega_\infty$.
...
In order to do this we define ${}_i\alpha_j \in k$ for $0\leq j \leq 2g+1$ by the equation
\[
...
We will use the fact that we can change signs as is convenient.
We see that
\begin{align*}
