...
\todo[inline]{Explanation of Serre Duality}
We start by noting that we have a short exact sequence of sheaves
...
\end{equation}
where $\underline{\Omega}_{K(X)}$ is the constant sheaf of the module of differentials of $K(X)$ over $k$, which we denote $\Omega_{K(X)}$.
By \cite[Chap II, ex. 1.16]{hart} we see that \eqref{serredualityses} is a flasque resolution of $\Omega_X$.

Clearly the sheaf $\Omega_{K(X)}/\Omega_P$\todo{is $\Omega_P$ standard enough notation?} is a sheaf on the singleton $\{P\}$, which has a natural embedding $i\colon \{P\} \rightarrow X$.
Hence for each $P$ we have the induced sheaf $i_*\left(K_X/\cO_P\right)$ on $X$.
...
\underline{\Omega}_{K(X)}/\Omega_X\cong \bigoplus_{P\in X} i_*(K_X/\cO_P).
\end{equation}
We construct this by first constructing a map from $\underline{\Omega}_{K(X)}/\Omega_P$ in to the product $\prod_{P \in X} i_*\left(\Omega_{K(X)}/\Omega_P\right)$, and then showing that this map has finite support.
...
& = \Omega_{K(X)}/\Omega_P.
\end{align}
Hence for each $P\in X$ we can map $f \in \underline{\Omega}_{K(X)}/\Omega_X$ to $i_*i^*(f) \in i_*\left( \Omega_{P(X)}/\Omega_P\right)$, and this map is an isomorphism.
We then recall that for any $f \in \Omega_{K(X)}$ we have $f \in \Omega_P$  for all but a finite number of points $P \in X$.
Combining the above yields the isomorphism in \eqref{sheafisomorphism} is 

It is now clear that tensoring the short exact sequence above by $\Omega_X$ gives us
\[
0 \rightarrow \Omega_X \rightarrow \Omega_X \otimes {\cal K}_X \rightarrow \bigoplus_{P\in X} i_*(\Omega_K/\Omega_P) \rightarrow 0,
\]
another flasque resolution, this time of $\Omega_X$.\todo{define $\Omega_K$, justify last term}

Now we can take the \cech complex of this over our cover $\cU$, which is
\begin{equation}\label{dualitydiagram}\begin{array}{ccccc}
 \Omega_X(U_0)\times \Omega_X(U_\infty) & \hookrightarrow & \Omega_K\times \Omega_K & \twoheadrightarrow & \left( \bigoplus \limits_{P \in U_0} \Omega_K/\Omega_P\right) \times \left( \bigoplus \limits_{P\in U_\infty} \Omega_K / \Omega_P \right) \\
d_1 \downarrow & ~ & d_2 \downarrow & ~ & d_3 \downarrow \\
\Omega_X(U_0 \cap U_\infty) & \hookrightarrow & \Omega_K & \twoheadrightarrow & \bigoplus\limits_{P \in U_0 \cap U_\infty} \Omega_K/\Omega_P.
\end{array}
\end{equation}\todo{define $\Omega_K$}
By the snake lemma we have differential map from $\delta \colon \ker (d_3) \rightarrow {\rm coker}(d_1)$.

The entire exact sequence given by the snake lemma is
\[
0 \rightarrow \hzero \ra \Omega_K \ra \bigoplus_{P \in X}\Omega_K/\Omega_P \ra \hone \ra 0.
\]
...
The map $d_2\colon \Omega_K \times \Omega_K \ra \Omega_2$ is the \cech differential given by $(\omega_1,\omega_2) \mapsto \omega_1 - \omega_2$, and hence the kernel is formed precisely by the elements $(\omega, \omega)$.
This shows that $\ker (d_2) \cong \Omega_K$.
Lastly, $d_3$ is similarly defined by $d_3\colon (\omega, \psi) \mapsto \omega|_{U_0 \cap U_\infty} - \psi|_{U_0 \cap U_\infty}$.
Hence the kernel of $d_3$ is formed of those elements which agree on $U_1 \cap U_3$.

Given a point $P\in X$ we have the a map $\res_P \colon \Omega_K \ra k$\todo{define properly}.
This also gives a well defined map on the quotient $\res_P \colon \Omega_K/\Omega_P \ra k$, since by definition elements of $\Omega_P$ do not have a pole at $P$, and hence have zero residue at $P$.
Therefore $\Omega_P$ is contained in the kernel of $\res\colon \Omega_K \ra k$.
Since we know that $\sum_{P\in X}\res_P(\omega) = 0$ for any $\omega \in \Omega_K$ (by \cite[Chap. III, Thm. 7.14.2]{hart}) the map $\res\colon \bigoplus_{P \in X} \Omega_K/\Omega_P \ra k$ given by $\sum_{P\in X} \res_P(\omega_P)$ vanishes on the the image of $\Omega_K$.\todo{check that the res map is defined properly}
Hence this map passes to the quotient and we can define
\[
\res\colon \hone \ra k.
\]

Using \eqref{dualitydiagram} we can study this map.
...
This then maps to $(f|_{U_0}, 0)$, which is why we need only consider residue on $U_0$ to compute the resolution.\todo{tidy this up}
\todo[inline]{End of Serre duality}
