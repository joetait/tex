...
Moreover we let $\cO$ be the complex that formed from the first (non-trivial) column of \eqref{bicomplex} and $\Omega$ be the complex formed from the second (non-trivial) column of \eqref{bicomplex}, shifted by one.\todo{check phrasing}
Then we obtain a short exact sequence of complexes 
\[
\Omega \hookrightarrow T \twoheadrightarrow \cO.
\]
...
\end{equation}\todo{rewrite, along the lines of "WLOG"}
\end{prop}
\begin{proof}
...
We now suppose that $\ord_{P_\infty} (y) = -(g+1)$. Since $y(y+H(x)) = F(x)$ it then follows that $-(g+1) + \ord_{P_\infty}(y+H(x)) = \ord_{P_\infty}(F(x))$, and hence $\ord_{P_\infty}(y+H(x)) = -\deg(F(x)) + g + 1$.\todo{wording}
...
\end{align*}
Hence for each $P\in X$ we can map $f \in \underline{\Omega}_{K(X)}/\Omega_X$ to $i_*i^{-1}(f) \in i_*\left( \Omega_{K(X)}/\Omega_{X,P}\right)$.\todo{sort out $i_*i^*$ issue}
...
Combining the above yields the isomorphism in \eqref{sheafisomorphism}.\todo{justify isomorphism. Probably due to isomorphism at the level of sheaves.}
...
By the snake lemma we have a connecting homomorphism $\delta \colon \ker (d_3) \rightarrow \operatorname {coker}(d_1)$.
...
\]
\todo[inline]{form above in to proposition}
which we examine below.\todo{refer to Hartshorne}

By Serre's affineness criteria (see \cite[Thm 5.2.23]{liu}) we know that $H^i(X,\cO_X) = 0$ for $i\geq 2$.
...
Hence the map $\ker(d_3) \ra \bigoplus_{P \in X} \Omega_{K(X)}/\Omega_{X,P}$ given by $(\omega_1, \omega_2) \mapsto \omega:= \omega_1|_{U_\infty} = \omega_2|_{U_0}$, where 
\begin{equation}
\omega(x) = \begin{cases}
