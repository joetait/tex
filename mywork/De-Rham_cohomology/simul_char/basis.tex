...
But it follows from \cite[Chap. III, Thm. 7.14.1 (b)]{hart} that $\res_P(df)=0$ for any $f \in \cO_X(U_0 \cap U_\infty)$, and hence $\res_P$ is zero on the image of the map $\cO_X(U_0 \cap U_\infty) \ra \Omega_X(U_0 \cap U_\infty)$.
From this it follows that the residue isomorphism factors through $\alpha$, and hence $p$ is surjective.\todo{repetition (last and 2nd line of para).}\todo{could give more detail. May need more notation though?}
\end{proof}
...
\end{equation*}
is an isomorphism.\todo{move to proof of next lemma}
...
We will show that the $\langle x^iy^{-1}dx, yx^{-j} \rangle = \sum_{P \in \pi^{-1}(\infty)}\res_P(x^{i-j}dx)$ (the equality following from Lemma \ref{tracemaplemma} is non-zero if and only if $j = i+1$.
It will then follow from Theorem \ref{serredualitytheorem} that the collection of elements $\{ yx^{-j}|_{U_0\cap U_\infty}\}_{ 1 \leq j \leq g}$ form a basis of $\hone$.
...
By the residue theorem \cite[Chap. III, Thm. 7.14.2]{hart} $\res_{P_\infty} (x^{i-j}dx) = -\sum_{P \in U_\infty}\res_P(x^{i-j}dx)$, so $\res_{P_\infty}(x^{i-j}dx) = 0$.
...
Hence 
\[
\langle x^iy^{-1}dx, yx^{-(i+1)} \rangle = \res_{P_\infty}(x^{-1}dx) + \res_{P_\infty'}(x^{-1}dx).
\]
As we assumed $p\neq 2$ it is sufficient to show that $\res_{P_\infty}(x^{-1}dx) = \res_{P_\infty'}(x^{-1}dx)$ in order to prove that $\langle x^iy^{-1}dx, yx^{-(i+1)}\rangle \neq 0$.
...
Hence $\langle x^iy^{-1}dx, yx^{-(i+1)}\rangle \neq 0$ and so the residue classes of the elements $yx^{-1}|_{U_0\cap U_\infty}, \ldots, yx^{-g}|_{U_0 \cap U_\infty}$ form a basis of $\hone$.
...
On the other hand, if $j-i \leq 0$ then the differential $\frac{yx^{i-j}}{H(x)}dx$ is regular on $U_\infty$.
It then follows from the residue theorem that its residue at $P_\infty$ is also zero.

If $\infty \in \mathbb P_k^1$ is not a branch point then using \eqref{divxp=2}, \eqref{differentialdivisor}, \eqref{divisorofH} and Proposition \ref{divyp=2}, the divisor of $\frac{y}{x^j} \cdot \frac{x^i }{H(x)}dx$ is
\begin{align*}
\di\left( \frac{yx^{i-j}}{H(x)}dx \right) & = \di(y) + \di(x^{i-j}) + \di( dx) - \di(H(x)) \\
...
Moreover, if this is the case and $2g+(j-i) = 2g+1 \geq \deg(F(x))$ then this is the only pole in $U_0$\todo{adapt reasoning?}, and hence the residue of $\frac{yx^{i-j}}{H(x)}dx$ is non-zero.
