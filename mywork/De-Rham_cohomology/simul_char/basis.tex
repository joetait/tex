...
\begin{displaymath}\label{bicomplex}
...
Hence the map 
\[
\left( \underline{\Omega}_{K(X)}/\Omega_X \right)_P \ra i_*\left(\Omega_{K(X)}/\Omega_{X,P} \right) \qquad f \mapsto i_*i^*(f)
\]
 is an isomorphism of stalks, and so the above yields an isomorphism at the level of sheaves in \eqref{sheafisomorphism}.\todo{check more}


Now we can take the \cech complex of \eqref{serredualityses} over our cover $\cU$, yielding
\begin{displaymath}\label{dualitydiagram}
...
We now recall the definition of the trace map $t\colon \hone \ra k$.
...
This then injects in to $\Omega_{K(X)}$, and since $d_2$ is surjective we know it is in the image of $d_2$, and hence we can choose an element of $\Omega_{K(X)} \times \Omega_{K(X)}$ mapping to $\omega$.
...
So to compute $t(\bar \omega)$ it suffices to compute the residue of $\omega$ (or equivalently $\psi$) at all points in $\pi^{-1}(\infty)$.\todo{make this in to a lemma}
\todo[inline]{rephrase last few lines}
...
\end{equation}\todo{explain what the product is}
...
Serre's duality theorem \cite[Thm. 2, Chap. II]{algebraicgroupsandclassfields} states that $\psi$ and $\theta$ are isomorphisms and are dual to each other,\todo{explain what it means for $\psi$ and $\theta$ to be dual} and hence that the spaces $\hone$ and $\hzero$ are dual.\todo{write as lemma - no proof}
...
\end{align*}\todo{quantifiers on $i$?}
...
\end{split}
\end{equation}
Now if $\infty$ is a branch point, then we see that $x^{i-j}dx$ has a pole of order one at $P_\infty$ if and only if $i-j = -1$ (recall that in this case the ramification divisor $R$ has a $[P_\infty]$ term with coefficient one) and hence $\res_{P_\infty}\left(x^{i-j}dx\right)  = \res_{P_\infty}\left(x^{-1}dx\right) \neq 0$.
...
In this case we see that there are still no poles in $U_0\backslash \pi^{-1}(\infty)$ and hence $\sum_{P \in U_0}\res_P(x^{-1}dx) = \res_{P_\infty}(x^{-1}dx) + \res_{P_\infty'}(x^{-1}dx)$.\todo{adapt}
...
Finally, since $i$ ranges from $0$ to $g-1$ we see that for $1\leq j \leq g$ the residue classes of $yx^{-j}$ form a basis of $\hone$.\todo{phrasing}
...
Hence $\res_{P_0} \left( \frac{yx^{i-j}}{H(x)}dx \right) \neq \res_{P_0'} \left( \frac{yx^{i-j}}{H(x)}dx \right)$ if and only if $i-j=-1$.
Hence $\left\langle \frac{x^i}{H(x)}dx, \frac{y}{x^j} \right \rangle = \res_{P_0} \left( \frac{yx^{i-j}}{H(x)}dx \right) +\res_{P_0'} \left( \frac{yx^{i-j}}{H(x)}dx \right) \neq 0$ if and only $j = i+1$.

Finally, if zero is a branch point then \todo{consider this case}
\end{proof}
...
\end{split}
\end{equation}\todo{reference this later}
...
Clearly $\Phi_i(x,y) = \Phi_i^x(x) + y\Phi_i^y(x)$ and $\Psi_i (x,y)= \Psi_i^x(x) + y\Psi_i^y(x)$, and we will use these splittings to show that $\frac{ \Phi_i(x) }{x^{i+1}H(x)}dx$ and $\frac{\Psi_i(x) }{x^{i+1}H(x)}dx$ are regular on $U_\infty$ and $U_0$ respectively.\todo{note that some stuff was removed before this para}
...
We now use this along with Proposition \ref{divyp=2} and the polynomials we defined in the preceding paragraphs to complete the proof.
