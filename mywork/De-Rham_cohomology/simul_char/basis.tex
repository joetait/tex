...
This is equivalent to checking that the residue is non-zero at the points in $X\backslash U_1$ (of which there are only two --- $P_0$ and $P_0'$).
We can see from the divisor that both of these points are also poles when $i-j=-1$, so we need to compute both of these.
