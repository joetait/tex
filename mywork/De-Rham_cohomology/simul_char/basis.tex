...
We now suppose that $p=2$;
We will use the fact that we can change signs as convenient.
We see that
\begin{align*}
...
Hence it only remains to show that for each $i\in {1, \ldots, g}$ the differentials $\frac{\Phi_i}{x^{i+1}H}dx$ and $\frac{\Psi_i}{x^{i+1}H}dx$ are regular on $U_\infty$ and $U_0$ respectively.\todo{consider wording}


We define ${}_iA_{j+1} \in k$ for $0 \leq j \leq 2g+2$, and ${}_iB_k \in k$ for $1\leq k \leq g+1$ by the equation
\[
...
In particular we remark that the $x^i$ term of $xH'(x) + iH(x)$ is exactly zero, since it is precisely $ib_ix^{i-1}\cdot x + ib_i x^i = 2ib_ix^i = 0$.
...
\end{align*}
From \eqref{divyp=2} $y$ only has poles at points in $\pi^{-1}(\infty)$, and hence this completes the proof that $\frac{\Phi_i dx}{x^{i+1}H}$ is regular on $U_\infty = X \backslash \pi^{-1}(\infty)$.
