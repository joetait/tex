...
\[
{\cal K}_X/\cO_X \cong \oplus_{P\in X} i_*(K_X/\cO_P).
\]\todo{give explicit isomorphism}

It is now clear that tensoring the short exact sequence above by $\Omega_X$ gives us
\[
0 \rightarrow \Omega_X \rightarrow \Omega_X \otimes {\cal K}_X \rightarrow \oplus_{P\in X} i_*(\Omega_K/\Omega_P) \rightarrow 0,
\]
...
\begin{equation}label{dualitydiagram}\begin{array}{ccccccccc}
0 & \rightarrow & \Omega_X(U_0)\times \Omega_X(U_\infty) & \rightarrow & \Omega_K\times \Omega_K & \rightarrow & \left( \oplus_{P \in U_0} \Omega_K/\Omega_P\right) \times \left( \oplus_{P\in U_\infty} \Omega_K / \Omega_P \right) & \rightarrow & 0 \\
~ & ~ & d_1 \downarrow & ~ & d_2 \downarrow & ~ & d_3 \downarrow & ~ & ~ \\
0 &\rightarrow & \Omega_X(U_0 \cap U_\infty) & \rightarrow & \Omega_K & \rightarrow & \bigoplus_{P \in U_0 \cap U_\infty} \Omega_K/\Omega_P & \ra & 0.
\end{array}
...
\[
0 \rightarrow \hzero \ra \Omega_K \ra \oplus_{P \in X}\Omega_K/\Omega_P \ra \hone \ra 0.
\]
...
Lastly, $d_3$ is similarly defined by $d_3\colon (\omega, \psi) \mapsto \omega|_{U_0 \cap U_\infty} - \psi|_{U_0 \cap U_\infty}$.\todo{explain what the kernel is}

Given a point $P\in X$ we have the a map $\res_P \colon \Omega_K \ra k$\todo{define properly}.
This also gives a well defined map on the quotient $\res_P \colon \omega_K/\Omega_P \ra k$.\todo{show this, sort out dual notation for res}
Since we know that $\sum_{P\in X}\res_P(\omega) = 0$ for any $\omega \in \Omega_K$ (by \cite[Chap. III, Thm. 7.14.2]{hart}) the map $\res\colon \bigoplus_{P \in X} \ra k$ given by $\sum_{P\in X} \res_P(\omega_P)$ vanishes on the the image of $\Omega_K$.\todo{check that the res map is defined properly}
...
This then injects in to $\Omega_K$, and since $d_2$ is surjective\todo{justify this earlier using Serre's affinenes} we know it is in the image of $d_2$, and hence can choose an element of $\Omega_K \times \Omega_K$ mapping to $f$.
