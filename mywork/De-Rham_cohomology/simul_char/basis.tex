...
Indeed, if we suppose that some $\omega\in \hzero$ maps to $0 \in \derhamhone$ then we must be able to find some $f_i\in \cO_X(U_i)$ such that $(\omega|_{U_0}, \omega_{U_\infty}, 0) = (df_0, df_\infty, f_0|_{U_0\cap U_\infty} - f_\infty|_{U_0\cap U_\infty})$.
...
Hence $\omega_{U_0} = df_0 = df|_{U_0} = 0$, and similarly $\omega_{U_\infty} = 0$.
...
\end{align*}\todo{should I mention anything to do with $i^{-1}$ for first equality?}
Hence for each $P\in X$ we can map $f \in \underline{\Omega}_{K(X)}/\Omega_X$ to $i_*i^*(f) \in i_*\left( \Omega_{K(X)}/\Omega_{X,P}\right)$\todo{check if I can write $f$ for $i_*i^*(f)$}.
...
Serre's duality theorem \cite[Thm. 2, Chap. II]{algebraicgroupsandclassfields} states that $\psi$ and $\theta$ are isomorphisms\todo{add "of the hom sets"?} and are dual to each other, and hence that the spaces $\hone$ and $\hzero$ are dual.
...
It will then follow from the above discussion of Serre duality that $\left. \frac{y}{x^j}\right|_{U_0\cap U_\infty}$ forms a basis for $j=1, \ldots g$.
...
Hence the elements $\frac{y}{x^j}$ form a (dual) basis for $j \in \{1, \ldots , g\}$ if $\infty$ is a branch point.
...
Then since $\sigma \left( \frac{yx^{i-j}}{H(x)}dx \right) = \frac{(y+H(x))x^{i-j}}{H(x)}dx = \frac{yx^{i-j}}{H(x)}dx + x^{i-j}dx$, it follows that $\res_{P_0} \left( \frac{yx^{i-j}}{H(x)}dx \right) = \res_{P_0'} \left( \frac{yx^{i-j}}{H(x)}dx \right)$ if and only if $\res_{P_0}(x^{i-j}dx) = 0$.
Since $x$ is a uniformising parameter at $P_0$ then $\res_{P_0}(x^{i-j}dx) \neq 0$ if and only if $i-j = -1$.
Hence $\res_{P_0} \left( \frac{yx^{i-j}}{H(x)}dx \right) \neq \res_{P_0'} \left( \frac{yx^{i-j}}{H(x)}dx \right)$ if and only $i-j=-1$, and since we are assuming $\cha(k) = 2$ this implies that $\res_{P_0} \left( \frac{yx^{i-j}}{H(x)}dx \right) +\res_{P_0'} \left( \frac{yx^{i-j}}{H(x)}dx \right) \neq 0$.
\end{proof}
...
\begin{equation*}
\sigma \left( \frac{\Phi_i(x,y)}{x^{i+1}H(x)} dx \right)  = \frac{\Phi_i(x,y)}{x^{i+1}H(x)} dx + d \left( \frac{{}_iH_{2,i}(x)}{x^i} \right).
\end{equation*}
...
Clearly $\Phi_i(x,y) = \Phi_i^x(x) + y\Phi_i^y(x)$ and $\Psi_i (x,y)= \Psi_i^x(x) + y\Psi_i^y(x)$, and we will use these splittings to show that $\frac{ \Phi_i(x) }{x^{i+1}H}dx$ and $\frac{\Psi_i(x) }{x^{i+1}H}dx$ are regular on $U_\infty$ and $U_0$ respectively.

We start by computing the divisor of $\frac{1}{x^{i+1}H}dx$, since it is a common component to all the differentials we need to look at.
This yields
\begin{align}
\di \left( \frac{1}{x^{i+1}H}dx \right) & = \di(dx) - \di (x^{i+1}) - \di (H) \nonumber \\
...
Firstly,
\begin{align*}
\di \left( \frac{\Phi_i^x }{x^{i+1} H}dx \right)  = &  \di(\Phi_i^x) -(i+1)D_0 + (g+i)D_\infty\\
...
\end{align*}
From this we see that the differential $\frac{\Phi_i^x}{x^{i+1}H}dx$ is clearly regular on $U_\infty = X \backslash \pi^{-1}(\infty)$.
...
\end{align*}
From \eqref{divyp=2} we see that $y$ only has poles at points in $\pi^{-1}(\infty)$, and hence this completes the proof that $\frac{\Phi_i }{x^{i+1}H}dx$ is regular on $U_\infty = X \backslash \pi^{-1}(\infty)$.

Now we complete the same computations on $\Psi_i$, starting with $\Psi_i^x$:
\begin{align*}
\di\left( \frac{\Psi_i^x }{x^{i+1}H}dx \right)  & =   \di(\Psi_i^x)  - (i+1)D_0 + (g+i)D_\infty \\
...
\begin{align*}
\di\left(\frac{y\Psi_i^y }{x^{i+1}H}dx \right)  =  & \di_0(y) - (2g+1)[P_\infty] + \di(\Psi_i^y) - (i+1)D_0 + (g+i)[P_\infty] \\
...
\begin{align*}
\di\left(\frac{y\Psi_i^y }{x^{i+1}H}dx \right)  =  & \di(y) + \di(\Psi_i^y) - (i+1)D_0 + (g+i)D_\infty \\
= & \di(y) + \di_0(\Psi_i^y) - (i+1)D_0 + (g+i - d_\Psi^y)D_\infty \\
\geq & \di(y) + \di_0(\Psi_i^y) - (i+1)D_0 + (g+1)D_\infty. \\
\end{align*}
Since we know from \eqref{divyp=2} that $y$ cannot have a pole of order greater $g+1$ at $P_\infty$ or $P_\infty'$, and only has poles at these points, it follows that the differential $\frac{y\Psi_i^y }{x^{i+1}H}dx$ is regular on $U_0 = X \backslash \pi^{-1}(0)$.
