...
If we fix a non-zero natural number $i$ then $\frac{y}[x^i}$, considered as an element of $\cO_X(U_1\cap U_2)$, is mapped to $\frac{y}{x^i} + \frac{H(x)}{x^i}$. CONTINUE
Then we know that $\sigma$ acts trivially on $H^1(X,\cO_X)$ since it acts trivially on the dual
