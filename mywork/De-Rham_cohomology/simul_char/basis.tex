...
\todo{check this area especially}
...
\end{align*}
where the first line makes use of \eqref{pnot2divisors}.\todo{check reference is right}
Since $i \leq g$ clearly the differential is regular on $U_2 = X\backslash \{\pi^{-1}(\infty)\}$.\todo{remove braces?}
...
\end{align*}
Since $y$ and $\frac{\Phi_i^y}{x^{i+1}}$ both only have poles at points in $\pi^{-1}(\infty)$, this completes the proof that $\Phi_i$ is regular on $U_2 = X \backslash \pi^{-1}(0)$.
...
\end{align*}
Since we know from \todo{reference} that $y$ cannot have a pole of order greather $g+1$ at $P_\infty$ or $P_\infty'$, and only has poles at these points, it follows that the differential is regular on $U_1 = X \backslash \pi^{-1}(0)$, and we have completed the proof.
