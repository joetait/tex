...
In general, the divisor of $x^{i-j}dx$ (when not resctricted to $U_1$) when $p\neq 2$ is
...
\begin{cor}
The action of $G$ on $\derhamhone$ is faithful unless both $p=2$.
\end{cor}
...
\[
\left( {}_iH_1'dx, {}_iH_2'dx, x^{-i}(y + H(x) \right),
\]
which clearly satisfies the conditions of \eqref{quotient} and hence is zero.
So the actio of the involution is trivial.
We can then see from the proof of the aforementioned theorem in \cite{faithfulaction} that the entire action is trivial, and hence not faithful.
\begin{comment}
Hence the action is trivial if and only if ${}_iH_1'(x)dx = {}_iH_2'(x)dx =0$.\todo{should probably add some notation to index the i on these polynomails}
