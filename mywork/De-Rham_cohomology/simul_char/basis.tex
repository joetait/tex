...
\end{align*}
\todo{reference second line}Now if $i$ is even the second term in the last line is $\frac{x{}_iH_1'(x)}{x^{i+1}} = \frac{ {}_iH_1'(x)}{x^i}$.
If $i$ is odd then when differentiating $\frac{ {}_iH_1(x)}{x^{i}}$ we expect all the odd powers of ${}_iH_1(x)$ to disappear, since after dividing by $x^i$ they will become even.\todo{rephrase}
