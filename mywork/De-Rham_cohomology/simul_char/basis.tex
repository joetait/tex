...
\end{lem}
\begin{proof}
We use Serre duality and the fact that we already know a basis of $\hzero$.\todo{introduce Serre dual}
Recall that if $p\neq 2$ then for $i \in \{0,\ldots, g-1\}$ the element $\omega_i = \frac{x^i}{y}dx$ form a basis of $\hzero$.
Then for $j\in \{1, \ldots, g\}$  we consider the pair $\langle x^iy^{-1}dx, yx^{-j}|_{U_1}\rangle$, which maps to the residue of $x^{i-j}dx|_{U_1}$.\todo{introduce angle brackets. check restriction is right}
...
We note that $t=y-\sqrt{f(0)}$ and $s=y+\sqrt{f(0)}$ are uniformising parameters at $P_0$ and $P_0'$ respectively.\todo{define p0 and p0' earlier}
...
\]
Now the LHS has a pole at $P_0$ and the only term on the right hand side that can also have a pole at $P_0$ is 
\[
...
\]
Using the same argument as before we dispense of the $y$ term, and hence $d_{-1}$ is determined uniquely by the $\frac{1}{x}$ terms in $\fracP2d_{-1}f' \cdot f}{4(f^2-f\cdot f(0)}$.
Since $c_{-1}$ and $d_{-1}$ are determined by the same *equation*\todo{not an equation. Property?}, they must take the same value. 
...
Also, even if $\infty$ is not a branch point it still suffices to compute the residue on $X\U_1$\todo{check}, and we see that if 0 is a branch point this is non-zero.
So we again need only consider the case when neither $\infty$ or 0 are branch points.
\end{proof}
