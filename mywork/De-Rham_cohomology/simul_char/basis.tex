...
In particular, if $\omega = x^{-1}dx$ and $P = P_\infty$, then we see that $\res_{P_\infty} (x^{-1}dx) = \res_{P_\infty'}(x^{-1}dx)$, since $\sigma (x^{-1}dx) = x^{-1}dx$.
...
Next we recall from \cite[\S 6]{faithfulaction} that in this case a basis of $\hzero$ is given by $\frac{x^i dx}{H(x)}$ for $i \in \{ 0, \ldots, g-1\}$.
...
Then since $\sigma \left( \frac{yx^{i-j}dx}{H(x)} \right) = \frac{(y+H(x))x^{i-j}dx}{H(x)} = \frac{yx^{i-j}dx}{H(x)} + x^{i-j}dx$, it follows that $\res_{P_0} \left( \frac{yx^{i-j}dx}{H(x)} \right) = \res_{P_0'} \left( \frac{yx^{i-j}dx}{H(x)} \right)$ if and only if $\res_{P_0}(x^{i-j}dx) = 0$.
Since $x$ is a uniformising parameter at $P_0$ then $\res_{P_0}(x^{i-j}dx) \neq 0$ if and only if $i-j = -1$.
Hence $\res_{P_0} \left( \frac{yx^{i-j}dx}{H(x)} \right) \neq \res_{P_0'} \left( \frac{yx^{i-j}dx}{H(x)} \right)$ if and only $i-j=-1$, and since we are assuming $\cha(k) = 2$ this implies that $\res_{P_0} \left( \frac{yx^{i-j}dx}{H(x)} \right) +\res_{P_0'} \left( \frac{yx^{i-j}dx}{H(x)} \right) \neq 0$.
\end{proof}
