...
Hence $\ker(d_3) \cong \bigoplus_{P \in X} \left( \Omega_{K(X)}/ \Omega_{X,P}\right)$.\todo{improve}
...
This also gives a well defined map on the quotient $\res_P \colon \Omega_K/\Omega_P \ra k$, since by definition elements of $\Omega_P$ have zero residue at $P$, and so $\Omega_P \subseteq \ker \left(\res_P\right)$.
...
Since $\omega$ is regular on $U_0 \cap U_\infty$, and $0$ is regular on $U_\infty$, then $\res_P(\psi) = 0$, except possibly for $P \in \pi^{-1}(\infty)$.
So to compute $t(\bar \omega)$ is suffices to compute the resiude of $\omega$ at all points in $\pi^{-1}(\infty)$.
...
In particular, element since both $\omega_0$ and $\omega_\infty$ are regular on $U_0 \cap U_\infty$ for all such pairs, so are all elements of $\hzero$.
...
Moreover, if $f \in \cO_X(U_1 \cap U_\infty)$ is a representative of zero in $\hone$, i.\ e.\ $f = f_0 - f_\infty$ for $f_i \in \cO_X(U_i)$, and $\omega  \in \hzero$ then $f \omega|_{\U_0 \cap U_\infty} = f_0\omega|_{U_0 \cap U_\infty} - f_\infty \omega|_{U_0 \cap U_\infty}$, and $f_i \omega|_{U_0\cap U_\infty} \in \Omega_X(U_i)$ for $i\in \{0, \infty}$.
So the map is well defined on the quotients.
