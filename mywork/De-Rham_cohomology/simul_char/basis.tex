...
Finally, if zero is a branch point then $[P_0]$ has coefficient of one in the ramification divisor $R$, and hence $\ord_{P_0}(x^{-1}dx) = -1$ (recall that $\di(dx) = R - 2D_\infty$ from \eqref{differentialdivisor}).
Also, if $i-j > -1$ then clearly $\ord_{P_0}(x^{i-j}dx) \geq 0$ and hence the residue at $P_0$ is zero.
This leaves the case $i-j < -1$.
\todo{Finish this case. Check i's and j's are correct way round. Possibly reference divisor.}
