...
y^2 = f(x)
\end{equation}
for some polynomial $f(x) \in k[x]$, of degree $2g+1$ or $2g+2$ \todo{reference}.
...
Here we require that $\deg(H(x)) \leq g+1$, with equality if and only $\infty$ is not a branch point, and $\deg(F(x)) \leq 2g+2$, and in particular $\deg(F(x)) = 2g+1$ if $\infty$ is a branch point. \todo{reference and check whether if and only if for F}
...
\di(y) = \begin{cases}
 ...
 \di_0(y) +(g+1-deg(f))[P_\infty] - (g+1)[P_\infty] & \text{otherwise.}
\end{cases}
\en{equation*}
\todo{finished here}
...
\begin{align*}
\di\left(\frac{y\Phi_i^y dx}{x^{i+1}H} \right)  = & (\di_0(y) - (2g+1)D_\infty) + \di(\Phi_i^y) -(i+1)D_0 + (g+i)D_\infty\\
 = & \di_0(y) + \di(\Phi_i^y) -(i+1)D_0 + (2i -1)D_\infty \\
 = & \di_0(y) + \di_0(\Phi_i^y) - d_\Phi^yD_\infty - (i+1)D_0 + (2i-1)D_\infty \\
 = & \di_0(y) + \di_0\left(\frac{\Phi_i^y}{x^{i+1}} \right) + (2i-2d_\Phi^y -1)D_\infty.
\end{align*}
