...
\end{align*}
\todo{double check the signs here, also, missing a factor fo two}Note that the function before $dx$ here is precisely $\omega_{11}\left(\frac{s_i(x)}{2yx^i} \right)$.

This shows that
\[
\left( \omega_{11} \left( \left( \frac{s_i(x)}{2yx^i} \right) dx \right) , 0, \omega_{11}( x^{-i}y)\right)
\]
is the zero element in $\derhamhone$.

We will now finally show that the basis of $\hone$ in $\derhamhone$ is not stable under the group action --- in particular, it is not stable under the action of $\omega_{11}$.
Now we fix $i = g-1$.
...
Since $g =2$, we can compute this by \todo{$s_i$ should only go up to order 2g+1 - check this has been done}
\begin{note}
