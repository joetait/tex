...
\]
i.e.\ elements of the form $(\omega, \omega)$, which we will just write as $\omega$.
Hence we can define a map $\hzero \ra \derhamhone$ given by
\[
\omega \mapsto (\omega, \omega, 0).
\]
This is in fact an injection.
Indeed, if we suppose that some $\omega\in \hzero$ maps to $0 \in \derhamhone$ then we must be able to find some $f_i\in \cO_X(U_i)$ such that $(\omega, \omega, 0) = (df_0, df_\infty, f_0|_{U_0\cap U_\infty} - f_\infty|_{U_0\cap U_\infty})$.
Since a non-zero rational function can only have finitely many zeroes, we conclude that $f_0 = f_\infty$ everywhere.
We then conclude that $f_0 = f_\infty$ is a constant function, since $f_0$ and $f_\infty$ are regular on $U_0$ and $U_\infty$ respectively, and these sets cover $X$.
Finally, we conclude that $\omega = df_0 = df_\infty = 0$, since the differential of a constant function is always zero.
...
Then if we consider the direct sum of these sheaves over $X$ we have the following isomorphism
...
\end{equation}
We prove the above isomorphism exists by first constructing a map from $\underline{\Omega}_{K(X)}/\Omega_{X,P}$ in to the product $\prod_{P \in X} i_*\left(\Omega_{K(X)}/\Omega_{X,P}\right)$, and then showing that this map has finite support.

Given $i\colon \{P\} \hookrightarrow X$, we see that
\begin{align}
i^*\left(\underline{\Omega}_{K(X)}/\Omega_X\right) & = \left(\underline{\Omega}_{K(X)}/\Omega_X\right)_P \\
& = \underline{\Omega}_{K(X),P}/\Omega_{X,P} \\
& = \Omega_{K(X)}/\Omega_{X,P}.
\end{align}
Hence for each $P\in X$ we can map $f \in \underline{\Omega}_{K(X)}/\Omega_X$ to $i_*i^*(f) \in i_*\left( \Omega_{K(X)}/\Omega_{X,P}\right)$\todo{check if I can write $f$ for $i_*i^*(f)$}, and this map is an isomorphism.
...
Now we can take the \cech complex of this over our cover $\cU$, which is
\begin{equation}\label{dualitydiagram}\begin{array}{ccccc}
 \Omega_X(U_0)\times \Omega_X(U_\infty) & \hookrightarrow & \Omega_{K(X)}\times \Omega_{K(X)} & \twoheadrightarrow & \left( \bigoplus \limits_{P \in U_0} \Omega_{K(X)}/\Omega_{X,P}\right) \times \left( \bigoplus \limits_{P\in U_\infty} \Omega_{K(X)} / \Omega_{X,P} \right) \\
d_1 \downarrow & ~ & d_2 \downarrow & ~ & d_3 \downarrow \\
\Omega_X(U_0 \cap U_\infty) & \hookrightarrow & \Omega_{K(X)} & \twoheadrightarrow & \bigoplus\limits_{P \in U_0 \cap U_\infty} \Omega_{K(X)}/\Omega_{X,P}.
\end{array}
\end{equation}
...
\]
We know that the cohomology vanishes after $\hone$ by Serre's affineness criteria.
The fact that $\hone \cong \ker(d_1)$ and $\hone \cong {\rm coker}(d_1)$ follows from the definition of $\hone$ and $\hzero$ in terms of \cech  cohomology given above.
The map $d_2\colon \Omega_{K(X)} \times \Omega_{K(X)} \ra \Omega_2$ is the \cech differential given by $(\omega_1,\omega_2) \mapsto \omega_1 - \omega_2$, and hence the kernel is formed precisely by the elements $(\omega, \omega)$.
Hence the map $(\omega, \omega) \mapsto \omega$ gives an isomorphism from $\ker(d_2)$ to $\Omega_{K(X)}$.
Lastly, $d_3$ is similarly defined by $d_3\colon (\omega_1, \omega_2) \mapsto \omega_1|_{U_0 \cap U_\infty} - \omega_2|_{U_0 \cap U_\infty}$.
Hence the kernel of $d_3$ is formed of pairs of differentials which agree on $U_0 \cap U_\infty$.
Moreover, if $(\omega_1, \omega_2 ) \in \ker(d_3)$ then $\omega_1$ can take any value on $\pi^{-1}(\infty)$, independently of $\omega_2$, and similarly $\omega_2$ can take any value on $\pi^{-1}(0)$; i.\ e.\ we have unique differential at each point $P \in X$.
Hence $\ker(d_3) \cong \bigoplus_{P \in X} \left( \Omega_{K(X)}/ \Omega_{X,P}\right)$.

Given a point $P\in X$ we have the a map $\res_P \colon \Omega_K \ra k$, as defined in \cite[Chap III, Thm. 7.14.1]{hart}.
This also gives a well defined map on the quotient $\res_P \colon \Omega_K/\Omega_P \ra k$, since by definition elements of $\Omega_P$ have zero residue at $P$.
Therefore $\Omega_P$ is contained in the kernel of $\res_P\colon \Omega_K \ra k$.
Since we know that $\sum_{P\in X}\res_P(\omega) = 0$ for any $\omega \in \Omega_K$ (by \cite[Chap. III, Thm. 7.14.2]{hart}) the map $t \colon \bigoplus_{P \in X} \Omega_K/\Omega_P \ra k$ given by $\sum_{P\in X} \res_P(\omega_P)$ vanishes on the the image of $\Omega_K$.
...
Given a cocycle in $\hone$ we take a representative $f \in \Omega_X(U_0 \cap U_\infty)$.
This then injects in to $\Omega_K$, and since $d_2$ is surjective we know it is in the image of $d_2$, and hence can choose an element of $\Omega_K \times \Omega_K$ mapping to $f$.
In particular, we could choose $(f,0)$.
This then maps to $(f|_{U_0}, 0) \in \left( \bigoplus_{P \in U_0} \Omega_{K(X)}/\Omega_{X,P}\right) \times \left( \bigoplus_{P \in U_\infty} \Omega_{K(X)}/\Omega_{X,P} \right)$.
By commutativity of the diagram this maps to zero in $\bigoplus_{P \in U_0 \cap U_\infty} \Omega_{K(X)}/\Omega_{X,P}$, and hence by exactness there exists an element of the kernel mapping to $(f|_{U_0},0)$.
So $f|_{U_0}$ agrees with zero in the quotient $\Omega_{K(X)}/\Omega_{X,P}$ at all points $P \in U_0$ except $P \in \pi^{-1}(\infty)$.
Hence $\res_P(f) = 0$ except at $P \in \pi^{-1}(\infty)$, and so to compute $t(P)$ is suffices to compute the resiude of $P$ at all points in $\pi^{-1}(\infty)$.

We now further recall that we can use this trace map to show that $\hzero$ and $\hone$ are dual.
Given some $\omega \in \hzero$ and $f\in\hone$ we define
\[
\langle \omega, f\rangle := \sum_{P \in X} \res_P (f\omega).
\]
Thus if we fix any $\omega \in \hone$ we produce a map $\theta(\omega)\colon \hone \ra k$, given by $\theta(\omega)(f) = \langle \omega , f\rangle$.
Similarly, if we fix any $f \in \hone$ we get a map $\psi(f) \colon \hzero \ra k$.
Finally, Serre's duality theorem \cite[Thm. 2, Chap. II]{algebraicgroupsandclassfields} states that $\psi$ and $\theta$ are isomorphisms and are dual to each other, and hence that the spaces $\hone$ and $\hzero$ are dual.
...
\begin{lem}\label{basish1}
 Via the isomorphism \eqref{cechhone} the residue classes of $\frac{y}{x}, \ldots , \frac{y}{x^g} \in K(X)$, restricted to $U_0 \cap U_\infty$, form a basis of $H^1(X,\cO_X)$.\todo{check over this letter}
\end{lem}
\begin{proof}
...
We use Serre duality and the fact that for $i= 1, \ldots, g$ the elements $\omega_i = x^iy^{-1}dx$ form a basis of $\hzero$ (see, for example, \cite[Chap 7, Prop. 4.26]{liu}).
...
We described in the paragraph preceding this theorem that Serre duality gives us a dual map $\langle, \rangle \colon \hone \times \hzero \ra k$.\todo{check that this has been done}
...
We see from the divisor above that if $i-j \leq -2$ then there is zero at $P_\infty$, and hence the residue at $P_\infty$ is zero.
On the other hand, if $i-j \geq 0$ then there is still a pole at $P_\infty$, though the order is less than $-1$.
However, the residue is still zero, since $i-j > -1$ implies there is no pole, and hence zero residue, at the point(s) in $\pi^{-1}(0)$.
It then follows from the residue theorem and the fact that we can see from its divisor that $x^{i-j}dx$ cannot have any other poles, that the residue on $U_\infty$, which equals the residue at $P_\infty$, is zero.
...
\]\todo{change notation, maybe delete middle equality}
...
Hence $\langle x^iy^{-1}dx, \left. yx^{-(i+1)}\right|_{U_0} \rangle \neq 0$ and so for $1 \leq j \leq g$ the residue classes of $\frac{y}{x^j}$  form a basis dual to $\frac{x^i}{y}dx$ for $0 \leq i \leq g-1$.
...
\sum_{i=1}^l m_i[Q_i] -iD_0 -(2g+1 - 2i)[P_\infty] & \text{if $\infty$ is a branch point} \\
\sum_{i=1}^l m_i[Q_i] - iD_0 +(g+1-\deg(F(x)) + i)[P_\infty] - (g+1-i)[P_\infty'] & \text{otherwise}.
\end{cases}
\end{equation*}
Note that this makes use of \eqref{divyp=2} and \eqref{divxp=2}, and is clearly positive on $U_0 \cap U_\infty$.

Next we recall from \cite[\S 6]{faithfulaction} that in this case a basis of $\hone$ is given by $\frac{x^i dx}{H(x)}$ for $i \in \{ 1, \ldots, g\}$.
We then compute the divisor of $\left( \frac{y}{x^j} \cdot \frac{x^i dx}{H(x)}\right)$ using \eqref{divxp=2}, \eqref{differentialdivisor} \eqref{divisorofH} and \eqref{divyp=2}, which is
\begin{align*}
...
If $j=i+1$ the divisor is positive everywhere else on $U_0$\todo{check that this is the right set, and that the right set is used throughout}, and hence $\langle \frac{y}{x^j}, \frac{x^idx}{H(x)} \rangle \neq =0$\todo{check entries in right order}\todo{remove dx from top of fraction?}. 

We also need to check that if $j \neq i+1$ then $\langle \frac{y}{x^j}, \frac{x^idx}{H(x)} \rangle = 0$.
Indeed, if $j-i \geq 2$ then clearly we do not have a pole at $P_\infty$.
On the other hand, if $j-i \leq 0$ then the differential is regular at all $P \in \pi^{-1}(0)$.
Hence it's residue on $U_\infty$ is zero, and it then follows from the residue theorem \todo{citation} that the residue at $P_\infty$ is also zero.
Finally, since $i$ ranges from $0$ to $g-1$ we see that for $1\leq j \leq g$ the residue classes of $\frac{y}{x^j}$ form a basis of $\hone$.

If $\infty \in \mathbb P_k^1$ is not a branch point then using \eqref{divxp=2}, \eqref{differentialdivisor} \eqref{divisorofH} and \eqref{divyp=2}, the divisor of $\frac{y}{x^j} \cdot \frac{x^i dx}{H(x)}$ is
\begin{align*}
\di\left( \frac{yx^{i-j}dx}{H(x)} \right) & = \di(y) + \di(x^{i-j}) + \di( dx) - \di(H(x)) \\
& = \sum_{i=1}^l m_i[Q_i] + (g+1-\deg(F(x)))[P_\infty] - (g+1)[P_\infty'] + (i-j)D_0 \\
& \qquad - (i-j)D_\infty + R - 2D_\infty - R + (g+1)D_\infty \\
& = \sum_{i=1}^l m_i[Q_i] + (j+2g-i-\deg(F(x)))[P_\infty] + (j - i -2)[P_\infty'] + (i-j)D_0.
\end{align*}
...
As noted earlier, we then know that $\res_P(\delta^*(\omega) = \res_{\delta(P)}(\omega)$ for any $P \in X$ and $\omega$.
We use this at $P_0$, since the residue on $\pi^{-1}(\infty)$ is precisely the negative of the residue on $U_\infty$, and we see from the divisor that the only poles occur at $P_0$ and $P_0' = \delta (P_0)$.
Then since $\delta \left( \frac{yx^{i-j}dx}{H(x)} \right) = \frac{(y+H(x))x^{i-j}dx}{H(x)} = \frac{yx^{i-j}dx}{H(x)} + x^{i-j}dx$, it follows that $\res_{P_0} \left( \frac{yx^{i-j}dx}{H(x)} \right) = \res_{P_0'} \left( \frac{yx^{i-j}dx}{H(x)} \right)$ if and only if $\res_{P_0}(x^{i-j}dx) = 0$.
Since $x$ is a uniformising parameter at $P_0$ the clearly $\res_{P_0}(x^{i-j}dx) \neq 0$ if and only if $i-j = -1$.
Hence $\res_{P_0} \left( \frac{yx^{i-j}dx}{H(x)} \right) \neq \res_{P_0'} \left( \frac{yx^{i-j}dx}{H(x)} \right)$ if and only $i-j=-1$, and since we are in characteristic two this implies that $\res_{P_0} \left( \frac{yx^{i-j}dx}{H(x)} \right) +neq \res_{P_0'} \left( \frac{yx^{i-j}dx}{H(x)} \right) \neq 0$.
...
\end{align*}
where the second equality makes use of \eqref{dixp=2} and \eqref{pnot2divisors}.
