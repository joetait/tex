...
In order to define this we need to distinguish the zeroes of $F(x)$.
...
\ord_P(y) = \ord_P\left( \frac{F(x)}{y + H(x)} \right) = \ord_P(F(x)) - \ord_P(y+H(x)) = m_i.
\end{equation}
...
Note that we can extend this embedding to $\left. f \right|_U$ for any $f\in K(X)$ and any open subset $U \subseteq X$ by letting $r_P = f$ if $P \in U$, and $r_P = 1$ otherwise.
