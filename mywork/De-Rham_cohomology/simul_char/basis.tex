...
By a generalisation of Leray's theorem --- \cite[Cor. 12.4.7]{EGA0III} --- we know that the $\derhamhone$ is isomorphic to the first cohomology of the total complex of \eqref{bicomplex}.\todo{change}
...
\di\left(\frac{y\Phi_i^y dx}{x^{i+1}H} \right)  = & \di(y) + \di(\Phi_i^y) -(i+1)D_0 + (g+i)D_\infty\\
 ...
 = & \di(y) + \di_0\left(\frac{\Phi_i^y}{x^{i+1}} \right). 
\end{align*}
\todo{check above computations}
