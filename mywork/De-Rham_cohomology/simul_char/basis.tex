...
Note that the second case is not symmetric, which implies a choice with respect to $P_\infty'$ and $P_\infty$, but this will not affect future computations.
...
\end{lem}
\begin{proof}
We first check that the functions are indeed regular on $U_0 \cap U_\infty$ by computing their divisors. 
For $p \neq 2$ this is
\begin{align*}
...
\end{align*}
and since $R$ is a positive divisor this is clearly positive on $U_0 \cap U_\infty$.
...
Next we recall from \cite{faithfulaction} that a basis of $\hone$ is given by $\frac{x^i dx}{H(x)}$.
We then compute the divisor of $\frac{y}{x^j} \cdot \frac{x^i dx}{H(x)}$, which is
\begin{align*}
\di\left( \frac{yx^{i-j}dx}{H(x)} \right) & = \di(y) + \di(x^{i-j}) + \di( dx) - \di(H(x)) \\
& = \di_0(y) - (2g+ 1 )[P_\infty] + (i-j)D_0 - (i-j)D_\infty + R - 2D_\infty - R + dD_\infty
\end{align*}
\todo{finish this}
...
\sigma \left( \left( \left(\frac{-\Psi_i(x,y)}{x^{i+1}H(x)}\right) dx, \left( \frac{\Phi_i(x,y)}{x^{i+1}H(x)} \right) dx, x^{-i}y \right)\right) = \\
 \left( \left(\frac{-\Psi_i(x,y)}{x^{i+1}H(x)} \right) dx + \frac{{}_iH'_1(x)}{x^i}dx,  \left( \frac{\Phi_i(x,y)}{x^{i+1}H(x)} \right) dx+ \frac{{}_iH'_2(x)}{x^i} dx, x^{-i}(y+H(x)) \right).
\end{multline}
So the action on the basis elements in \eqref{three} is equivalent to adding 
\[
\left( \frac{{}_iH_1'}{x^i}dx, \frac{{}_iH_2'}{x^i}dx, x^{-i}H(x) \right),
\]
