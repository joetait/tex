...
We let $U_a = X \backslash \pi^{-1}(a)$ for any $a \in \mathbb P_k^1$ and we let ${\cal U}$ be the affine cover of $X$ formed by\todo{by or from} $U_0$ and $U_\infty$.
...
Since $X$ is a curve it has no differentials of degree greater than one.
...
Then let $\Omega[1]$ denote the complex obtained from shifting $\Omega$ by one, i.e. $\Omega[1]^{n+1} = \Omega^n$.\todo{define $\Omega^n$? Use different notation?}
Then we obtain the following short exact sequence of complexes 
\[
\Omega[1] \hookrightarrow T \twoheadrightarrow \cO,
\]
which gives rise to the following long exact sequence\todo{check use of which}
...
We can show surjectivity directly from
...
Throughout we let $P_a$ and $P_a'$ denote the unique elements of $\pi^{-1}(a)$ for any point $a \in \mathbb P_k^1$ which\todo{check use of which} is not a branch point.
...
y^2 - H(x)y = F(x)
\end{equation}\todo{should there be a comma at the end of equation?}
...
\end{equation}
Finally, we need to describe the divisor of $y$ when $p=2$.
...
\end{prop}
\begin{rem}
Note that the if $\infty$ is not a branch point divisor of $y$ does not have the symmetry of the other divisors we have considered.
...
Hence $\tilde F(x) := (x- \gamma_i)^{-1}F(x)$ is a unit at $P$.
...
Finally, we will compute the divisor of $dy$.\todo{haven't actually computed the divisor?}
...
Since the map $\underline{\Omega}_{K(X)}/\Omega_X \cong \bigoplus_{P \in X}i_*(\Omega_{K(X)}/\Omega_{X,P}$ given by $f \mapsto (i_*i^*(f))_{P \in X}$ is an isomorphism of stalks it follows that it is also an isomorphism at the level of sheaves, as in \eqref{sheafisomorphism}.
...
This also gives a well defined map on the quotient $\Omega_K(X)/\Omega_P$, since elements of $\Omega_P$ have zero residue at $P$, and so $\Omega_P \subseteq \ker \left(\res_P\right)$.
Since we know that $\sum_{P\in X}\res_P(\omega) = 0$ for any $\omega \in \Omega_K(X)$ by the residue theorem \cite[Chap. III, Thm. 7.14.2]{hart},  it follows that the map $t \colon \bigoplus_{P \in X} \Omega_K(X)/\Omega_P \ra k$ given by $(\omega_P)_{P \in X} \mapsto \sum_{P\in X} \res_P(\omega_P)$ vanishes on the image of $\Omega_{K(X)}$.
...
\end{equation}
where the product $f \omega|_{U_0\cap U_\infty}$ is just the usual product of a function and a differential, but restricted to the appropriate set.
...
\[
\hzero \times \hone \ra k, \ \langle \omega, f\rangle \mapsto  t(f\omega|_{U_0 \cap U_\infty}.
\]
...
\begin{lem}
The $k$-vector spaces $\hone$ and $\hzero$ are dual.
\end{lem}
\begin{proof}
...
\end{align}
Since $R$ is a positive divisor this is non-negative on $U_0 \cap U_\infty$ if and only if $i\in \{0, \ldots, g-1\}$.
...
We will show that the product $\langle x^iy^{-1}dx, yx^{-j} \rangle = \sum_{P \in X}\res_P(x^{i-j}dx)$ is non-zero if and only if $j = i+1$.
It will then follow from the above discussion of Serre duality that the collection of elements $\{ yx^{-j}|_{U_0\cap U_\infty}\}_{ 1 \leq j \leq g}$ form a basis of $\hone$.
...
\end{split}
\end{equation}
If $\infty$ is a branch point we see that $x^{i-j}dx$ has a pole of order one at $P_\infty$ if and only if $i-j = -1$ (recall that in this case the ramification divisor $R$ has a $[P_\infty]$ term with coefficient one).
Hence $\res_{P_\infty}\left(x^{i-j}dx\right)  \neq 0$ if and only $i-j=-1$, so $\langle x^iy^{-1}dx, yx^{-(i+1)} \rangle \neq 0$ for $0 \leq i \leq g-1$.
...
But since $i-j$ is non-negative there are no poles on $U_\infty$, and hence $\sum_{P \in U_\infty} \res_P(x^{i-j}dx) = 0$.
By the residue theorem $\res_{P_\infty} (x^{i-j}dx) = -\sum_{P \in U_\infty}\res_P(x^{i-j}dx)$, so $\res_{P_\infty}(x^{i-j}dx) = 0$.
Hence the elements $yx^{-1}, \ldots, yx^g$ form a (dual) basis if $\infty$ is a branch point.


If $\infty$ is not a branch point then we see from \eqref{residuedivisor} that $x^{i-j}dx$ has order one poles at both $P_\infty$ and $P_\infty'$ if and only if $i-j=-1$.
In this case we see from \eqref{divisorofyoverx} that $yx^{-1}$ has no poles on $U_0 \backslash \pi^{-1}(\infty)$.
...
On the other hand, if $j-i \leq 0$ then the differential is regular on $U_\infty$.
...
To show that the sum of the residues of $\frac{yx^i}{H(x)}dx$ is non-zero for every $i \in \{ 1, \ldots, g\}$ (i.e. that the image of each element under the trace map is non-zero) it suffices to show that the sum of the residues at $P_\infty$ and $P_\infty'$ are non-zero.
...
Finally, we define $\Phi_i(x,y) = \Phi_i^x(x) + y \Phi^y_i(x)$ and $\Psi_i(x,y) = \Psi_i^x(x) + y \Psi_i^y(x)$.
