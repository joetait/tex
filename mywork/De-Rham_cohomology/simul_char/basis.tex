...
From this condition we can, as noted in \cite[\S 6]{faithfulaction}\todo{more accurate citation, reference 18 month. check if needed}, deduce that $F(x)$ is not divisible by $x^2$.
...
\begin{align*}
    ...
    & = \di_0(y) -2i[P_0] - ( 2g + 1 -2i)[P_\infty].
\end{align*}
...
\begin{cor}
If $G$ acts faithfully on $X$ then $G$ acts faithfully on $\derhamhone$ if and only if $p \neq 2$.
\end{cor}
...
\todo{the above is not all necessary, but why the action is trivial is included since it may be useful later, and is vaguely relevant here}
\end{proof}


\todo{need to check that statement of corollary matches proof}
...
Now it remains to show that $\frac{\phi_i}{2yx^i}dx$ and $\frac{-\psi_i}{2yx^i}dx$ are regular on $U_1$ and $U_2$ respectively.
...
\todo[inline]{remove d psi, remove last line? - will keep for now in case useful later}
...
Since $i \leq g$ clearly the differential is regular on $U_2 = X\backslash \{P_\infty\}$.
...
\end{align*}
Again, the first line uses \eqref{pnot2divisors}, and since $i\geq 1$ we conclude that $\frac{\psi_i(x)}{2yx^i}dx$ is regular on $U_1 = X \backslash \{P_0\}$, concluding the $p=\neq 2$ case.
...
If $i$ is even then $B_g^ix^g + \ldots + B_1^ix = H'x$ and so the coefficient of $x^i$ in $H'x$ is $ib_i = 0$ (recall that we defined $b_i$ to be the coefficients of $H$ in \eqref{hcoefficients}).
...
& = (R-4[P_\infty]) - 2((i+1)[P_0] - (i+1)[P_\infty]) - (R - 2(g+1)[P_\infty]) \nonumber \\
& = 2(g+i)[P_\infty] - 2(i+1)[P_0],
\end{align}
...
Firstly,
\begin{align*}
\di \left( \frac{\Phi_i^x dx}{x^{i+1} H} \right)  = &  \di(\Phi_i^x) -2(i+1)[P_0] + 2(g+i)[P_\infty]\\
...
\begin{align*}
\di\left(\frac{y\Phi_i^y dx}{x^{i+1}H} \right)  = & (\di_0(y) - (2g+1)[P_\infty]) + \di(\Phi_i^y) -2(i+1)[P_0] + 2(g+i)[P_\infty]\\
...
\begin{align*}
\di\left( \frac{\Psi_i^x dx}{x^{i+1}H} \right)  = &  \di(\Psi_i^x)  - 2(i+1)[P_0] + 2(g+i)[P_\infty] \\
 \geq &  \di_0(\Psi_i^x ) - 2i[P_\infty] - 2(i+1)[P_0] + 2(g+i)[P_\infty] \\
 = &  \di_0(\Psi_i^x) - 2i[P_0] + 2(g+1)[P_\infty],
\end{align*}
and it is clear that the divisor is positive on $U_1 = X \backslash \{P_\infty\}$.

Similarly
\begin{align*}
\di\left(\frac{y\Psi_i^y dx}{x^{i+1}H} \right)  =  & \di_0(y) - (2g+1)[P_\infty] + \di(\Psi_i^y) - 2(i+1)[P_0] + 2(g+i)[P_\infty] \\
...
\todo[inline]{check that capital h has been used, check that I have been consistent with f(x) or just f}
\begin{lem}
...
\begin{align*}
\di(t) & = \di_0(y) - (2g+1)[P_\infty] -(2g+2)[P_0] + (2g+2)[P_\infty] \\
& = \di_0(y)-(2g+2)[P_0] + [P_\infty],
\end{align*}
...
\begin{multline*}
\di\left(\frac{yx^{i-j}}{H(x)}dx \right) = \di_0(y) + (i-j)D_0 + (g-1-(i-j))D_\infty \\ + (g+1-\deg(f))[P_\infty] -(g+1)[P_\infty'].
\end{multline*}
