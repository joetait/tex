...
\end{align*}
Hence for each $P\in X$ we can map $f \in \underline{\Omega}_{K(X)}/\Omega_X$ to $i_*i^*(f) \in i_*\left( \Omega_{K(X)}/\Omega_{X,P}\right)$.
...
Hence $f \mapsto i_*i^*(f)$ is an isomorphism at the level of stalks, and so the above yields an isomorphism at the level of sheaves in \eqref{sheafisomorphism}.\todo{check more}
