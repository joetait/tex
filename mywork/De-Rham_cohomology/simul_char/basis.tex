...
Since we assumed that $\infty$ is a branch point $f(x)$ is necessarily of degree $2g+1$ (see, for example, \cite[Prop. 7.4.24]{liu}).
...
From this condition we can, as noted in \cite[\S 6]{faithfulaction}\todo{more accurate citation - may be best to reference 18 month}, deduce that $F(x)$ is not divisible by $x^2$.
...
\end{align}
Here $\di_0(f)$ denotes the divisor of zeroes of the function $f$.
...
Clearly $\Phi_i(x) = \Phi_i^x(x) + y\Phi_i^y(x)$ and $\Psi_i (x)= \Psi_i^x(x) + y\Psi_i^y(x)$, and we will use these splittings of to show that $\Phi_i(x)$ and $\Psi_i(x)$ are regular on $U_2$ and $U_1$ respectively.\todo{not acutally phi and psi but the associated differentials}
