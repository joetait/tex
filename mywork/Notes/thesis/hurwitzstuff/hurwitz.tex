...
This is sheaf theoretic notation, which would not normally be used in function field theory, but we use it for consistency with the rest of the thesis.\todo{change this sentence after changing notation}
...
Recall that $\cO_{X,P}$ is an integrally closed subring of $K(X)$ for any $P\in X$, and that $K(X)$ is the field of fractions of $\cO_{X,P}$.
    ...
    Since $\phi(T)$ is minimal over $K(Y)$, then there exists some $\psi(T)\in K(Y)[T]$ such that $ \xi(T) = \phi(T)\cdot \psi(T)$.\todo{Could add proof of this - consider factorisation in the $K(X)$ adjoin the roots}
...
\section{Relations to classical algebraic geometry}
