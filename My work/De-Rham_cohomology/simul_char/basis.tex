...
Hence such an $f_1$ cannot exist, and the image of $x^{-i}y$ in $\check{H}^1(\cU,\cO_X)$ is non-zero (for all $i$).
 ...
 Suppose that $\omega_s$ form a basis of $H^0(X,\cO_X)$, for $i \in \{ 1,\ldots , g\}$.
To prove that the $\frac{y}{x^i}$ form a basis of $H^1(X,\cO_X)$ it is sufficient to show that ${\rm Res} \left( \omega_i, \frac{y}{x^i}\right)$ is non-zero for each $i$, by Serre duality. \todo{explain serre duality}
