...
We can now give a basis for $\cechhone$.\\

\begin{prop}\label{basis}

Let $i=1,\ldots, g$. If $p\neq 2$ then a basis of $\cechhone$ as a $k$ vector space is given by
...
Then any group $G$ acting faithfully on $X$ also acts faithfully on $\cechhone$.
...
We see from the above proposition that the third entry in part \eqref{three} of the basis of $\cechhone$ given by \cech cohomology is $x^{-i}y$ for $1 \leq i \leq g$.
...
It is clear that \eqref{two} and \eqref{four} satisfy the conditions in \eqref{derhamconditions}, and it follows \cite[Thm 6.1]{faithfulaction} that they are the image of $H^0(X,\Omega_X)$ in $\cechhone$.
