...
It is this map which gives us the differential map in the \cech cochain
\begin{equation*}
...
H(x) = \prod_{i=1}^d (x-a_i)^{n_i} = b_dx^d + \ldots + b_1x
\end{equation}
for some $d \leq g$, $a_i, b_i \in  k$ and $n_i \in \mathbb N$.
...
In order to state the basis of $\derhamhone$, as well as to shorten the proof of the following proposition, we define the following polynomial and differentials. \todo{change all de rham cohomology symbols} 
...
\todo{sort out spacing around theorems etc}
...
It is clear that \eqref{two} and \eqref{four} is an element of \eqref{derhamconditions}, and it follows from \cite[Thm 6.1]{faithfulaction} that they are the image of the basis of $H^0(X,\Omega_X)$ in $\derhamhone$.

Moreover it is obvious that the elements \eqref{one} and \eqref{three} map to the basis of $\hone$ give in Lemma \ref{basish1} if they belong to \eqref{derhamconditions}.
...
Whenever they are non-zero we denote their degrees as polynomials in $x$ by $d_\phi$ and $d_\psi$ respectively. From the definition of $\phi_i$ and $\psi_i$ we know that $0 < d_\psi \leq g$ and $g < d_\phi \leq 2g$ respectively, for all $1 \leq i \leq g$
...
\end{align*}
Again, the first line uses \eqref{pnot2divisors}, and again since $i\geq 1$ we conclude that $\frac{\psi_i}{2yx^i}dx$ is regular on $U_1 = X \backslash \{P_0\}$.
