...
\newcommand{\derhamone}{H_{\textrm{dR}}^1(X/k)}
...
To do this we will compute a basis of the two outer terms of the short exact sequence
\begin{equation}\label{ses}
0 \ra H^0(X,\Omega_X) \ra \derhamone \ra H^1(X,\cO_X) \ra 0.
\end{equation}\todo[inline]{find reference for ses}
...
\section{Basis of $\derhamone$}

The bases of the $k$-vector spaces in \ref{ses} will be given in terms of defining equations, so we describe these now.

If $p \neq 2$ then the extension $K(X)$ of $k(x)$ is $k(x,y)$ where $y$ satisfies
\[
y^2 = f(x)
\]
...
Any group $G$ acting faithfully on $X$ also acts faithfully on $\derhamone$.
...
It is clear that \eqref{two} and \eqref{four} are elements of \eqref{derhamconditions}, and it follows from \cite[Thm 6.1]{faithfulaction}\todo{add refence to thesis when writign up} that they are the image of $H^0(X,\Omega_X)$ in $\derhamone$.
