...
Since our cover consists of only two elements, then \todo{which term - just the first?}{the \cech cohomology of} $\Omega_X^\bullet$ with respect to the cover $\cal U$ is precisely the cokernel of this map.

Hence to compute the \cech de-Rham cohomology, we need to compute the cohomology of the total complex of
\[ \begin{array}{ccccccc}
~ & ~ & 0 & ~ & 0 & ~ & ~ \\
~ & ~ & \downarrow & ~ & \downarrow & ~ & ~ \\
...
0 \ra H^0(X,\Omega_X) \ra H^1_{dR}(X/k) \ra H^1(X,\cO_X) \ra 0.
\end{equation}
The basis of $H^0(X,\Omega_X)$ is already known in the literature \todo{citation} so we need only compute the basis of $H^1(X,\cO_X)$.
...
Similarly, we let $\Phi_i$ and $\Psi_i$ be the monomials of $S_i(x,y)$ where the degree of the $x$ component is greater than or equal to $i$ and less than $i$ respectively.\todo{check if it should $\geq$ or $\leq$ i}
...
& = & dyx^{-i}.
\end{eqnarray*}
Hence it only remains to show that $\frac{\Phi_i}{x^{i+1}H}dx$ and $\frac{\Psi_i}{x^{i+1}H}dx$ are regular on $U_1$ and $U_2$ respectively.
