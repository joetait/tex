...
Since we assumed that $\infty$ is a branch point $f(x)$ is necessarily of degree $2g+1$.\todo{reference}

We let $R$ denote the ramification divisor of $\pi$.
We recall that $R = \di(y) + 2(g+1)[P_\infty]$ and that $\di( dx) = R - 4[P_\infty]$.\todo{citations}

On the other hand, if $p=2$, then the extension $K(X)$ of $k(x)$ will be $k(x,y)$, this time with $y$ defined by
\[
y^2 - H(x)y = F(x),
\]
for polynomials $H(x), F(x) \in k[x]$, where $H(x)$ and $H'(x)^2F(x) + F'(x)^2$ share no roots.
Also, as noted in \cite[\S 6]{faithfulaction}, we may assume that $F(x)$ is not divisible by $x^2$.
\todo[inline]{make citations above more precise and add statement re degree of F and H}

We again let $R$ denote the ramification divisor---it will be clear which characteristic we are in whenever we refer to $R$, so there will be no confusion caused.
...
Finally, the ramification divisor we suppose that $H(x) = \prod_{i=1}^d (x-a_i)^{n_i}$ for some $d \leq g$, $a_i \in k$ and $n_i \geq i$.
...
We now compute the basis of $H^1(X,\cO_X)$, before explicitly stating the basis of $\cechhone$.\\
...
In order to state the basis of $\cechhone$, as well as to shorten the proof of the following proposition, we define the following polynomial and differentials.
...
We decompose $s_i(x)$ in the form $s_i(x) = \phi_i(x) + \psi_i(x)$ with $\psi_i(x), \phi_i(x) \in k[x]$, such that $\deg(\psi_i(x) \leq g$ and $x^{g+1}$ divides $\phi_i(x)$.
Similarly we decompose $S_i(x,y)$ in the form $S_i(x,y) = \Phi_i(x,y) + \Psi_i(x,y)$ with $\Psi_i(x,y), \Phi_i(x,y) \in k[x] \oplus yk[x]$, such that the degree of $\pSi_i(x,y)$ is less than $i$ as a polynomial in $x$, and $x^i$ divides $\Phi_i(x,y)$.

We can now give a basis for $\derhamone$, as a quotient of \eqref{derhamconditions}.

\begin{thm}\label{basis}

If $p\neq 2$ then the residues of the elements 
\begin{equation}\label{one}
 \left( \left( \frac{\psi_i(x)}{2yx^i}\right) dx, \left(\frac{-\phi_i(x)}{2yx^i}\right) dx, x^{-i}y\right), i = 1, \ldots ,g,
\end{equation}
as well as the residues of
\begin{equation}\label{two}
 \left( \frac{x^{i}}{y} dx , \frac{x^{i}}{y} dx, 0 \right), i=0, \ldots ,g-1,
\end{equation}
form a basis of $\derhamone$.

If $p=2$ then the residues of 
\begin{equation}\label{three}
\left( \left(\frac{\Psi_i(x,y)}{x^{i+1}H}\right) dx, \left( \frac{-\Phi_i(x,y)}{x^{i+1}H} \right) dx, x^{-i}y \right), i = 1, \ldots, g,
\end{equation}
as well as the residues of 
...
Recall that $H^0(X,\Omega_X)$ injects in to $\cechhone$.
Then if $p \neq 2$, or if $G$ does not contain a hyperelliptic involution, it follows from the main theorem of \cite{faithfulaction} that $G$ acts faithfully on $H^0(X,\Omega_X)$, and hence $G$ acts faithfully on $\cechhone$.

We now suppose that $p=2$ and $G$ does contain a hyperelliptic involution $\sigma$.
We see from the above theorem that the third entry in part \eqref{three} of the basis of $\cechhone$ given by \cech cohomology is $x^{-i}y$ for $1 \leq i \leq g$.
...
We see that
\begin{align*}
\left( \left( \Psi_i}{x^{i+1}H} \right) - \left( \frac{- \frac{\Phi_i}{x^{i+1}H} \right) \right) dx & =  \frac{S_i}{x^{i+1}H}dx \\
...
\end{align*}
\todo[inline]{define superscript y terms}
