...
\begin{document}
\maketitle
\todo[inline]{in general, check for when p might divide the genus or i, for i between 0 and g-1}
...
Though we have not properly described the zero divisor of $y$, it should be recalled that since $P_0$ is a ramification, \todo{is this right?}{$\di_0(y)$ will have a coefficient of one for $[P_0]$.}
...
We start by splitting up $d(yx^{-i})$ as $\frac{1}{x^i} \left(iyh + xf' + yh'x \right) dx+ \frac{iy}{x^{i+1}} dx$, and we now compute the divisors of these differentials.\todo{wrong to assume that f(0)=0?}
...
\end{eqnarray*}
\todo{check this}{Similar computations will work when either $d$ or $n_1$ (or both) are even} and when we have split up $\frac{f'dx}{x^ih}$ we have two divisors that sum to $dx^iy$ and are regular on the appropriate sets whenever $i$ is even.
...
\begin{eqnarray*}
 \di \left( \frac{y \phi}{hx^{i+1}} dx \right) & = & \di_0(y) - (2g+1)[P_\infty] + \di( \phi) + R - 4[P_\infty] - 2(i+1)[P_0] + 2(i+1)[P_\infty] -R + 2(g+1)[P_\infty] \\
& = & \di_0(y) + \di(\phi) -(2g+1)[P_\infty] + R' + 2(g+1-d)[P_\infty] + 2n_1[P_0] - 2(i+1)[P_0] + 2(i-1)[P_\infty] -R' - 2n_1[P_0]- 2(g+1-d)[P_\infty] +2(g+1)[P_\infty] \\
...
Similarly 
\begin{eqnarray*}
\di \left( \frac{\psi y}{hx^{i+1}} dx \right) & = & \di_0(y) -(2g+1)[P_\infty] + \di(\psi) +R -4[P_\infty] - 2(i+1)[P_0] + 2(i+1)[P_\infty] - R + 2(g+1)[P_\infty] \\
