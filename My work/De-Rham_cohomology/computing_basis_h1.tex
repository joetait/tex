...
\todo{find citation}{By definition}  $\check{H}_{dR}^1(U)$ is the quotient of the $k$-vector space 
\[
...
We now wish to write $\frac{y}{xh(x)}dx$ as $r(x,y)dt$ for some function $r$.
...
We now suppose that we can write $f = \alpha_{2g+1} x^{2g+1} + \ldots + \alpha_1x +\alpha_0$, and hence we have $f' = \alpha_{2g+1}x^{2g} + \ldots + \alpha_1$.
We fix $i$.
