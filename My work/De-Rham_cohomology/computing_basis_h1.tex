...
We now want to find, given $yx^{-i}$ for some $1\leq i \leq g$, two differentials $\omega_1$ and $\omeg_2$, satisfying the following:
\begin{list}
...
First we compute that $dyx^{-1} = x^{-1}dy + ydx^{-i} = x^{-i}dy + iyx^{-i-1}dx$.
The order of $y$ at $P_0$ is 1, so $y$ is a uniformising parameter at $P_0$.
On the other hand $\ord_{P_0}}(x) =2$, so we can write $x = ct^2 + {\rm higher order terms}$.
Hence $iyx^{-1-i}dx = \left( 2ciyx^{-1-i}t + {\rm higher order terms}\right) dt$.
We then conclude that $\ord_{P_0}(dyx^{-i}) \geq \{ \ord_{P_0}(x^-i}, 2ciyx^{-1-i}t\} = -i$.
